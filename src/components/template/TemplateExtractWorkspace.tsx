...
export type TemplateExtractWorkspaceProps = {
  ...
  showPreview?: boolean;
  autoSaveOnExtract?: boolean;
  onDraftReady?: (draft: TemplateEditWorkspaceInitialDraft) => void;
  onAutoSaveComplete?: (result: TemplateExtractApproveResult) => void;
...
export function TemplateExtractWorkspace({
  ...
  React.useEffect(() => {
    if (autoSaveOnExtract || !onDraftReady || !currentDraft || !currentDraftHtml.trim()) {
      return;
    }
  ...
  }, [autoSaveOnExtract, currentDraft, currentDraftHtml, extractConfigKey, onDraftReady, selectedFile, templateName]);
  ...
  const approveDraftDetail = React.useCallback(
    async (
      detail: TemplateExtractDetailResult,
      draftHtml: string,
      options: {
      ...
      const normalizedDraftId = detail.draft.id.trim();
      const normalizedDraftHtml = draftHtml.trim();
      const normalizedTemplateName = templateName.trim() || toBaseFileName(selectedFile) || '새 템플릿 초안';

      if (!normalizedDraftId || !normalizedDraftHtml) {
        throw new Error('저장할 초안이 없습니다.');
      }

      if (!normalizedTemplateName) {
        throw new Error('템플릿 관리 저장 이름을 입력하세요.');
      }

      const saveDraftHtml = sanitizeExtractDraftHtmlForTemplateSave(normalizedDraftHtml);
      ...
      setMessage(options.successMessage || '템플릿 관리 저장을 완료했습니다.');
      onAutoSaveComplete?.(result.data);
      return result.data;
    },
    [onAutoSaveComplete, selectedFile, templateName]
  );
  ...
  const handleRunPhysicalStage = React.useCallback(async () => {
    ...
    try {
      ...
      if (autoSaveOnExtract) {
        await approveDraftDetail(valueStageDraft, physicalHtml, {
          successMessage: '물리적 보정 후 템플릿 저장을 완료했습니다.',
        });
      } else {
  ...
  }, [approveDraftDetail, autoSaveOnExtract, commitDraft, valueStageDraft, valueStageHtml]);
  ...
  const handleRunFullExtract = React.useCallback(async () => {
    ...
    try {
      ...
      setValueStageHtml(extractedHtml);
      if (autoSaveOnExtract) {
        await approveDraftDetail(valueDetail, physicalHtml, {
          successMessage: `전체 추출 후 템플릿 저장을 완료했습니다. (${frameGroupVersion} -> ${currentTextVersion})`,
        });
      } else {
  ...
  }, [approveDraftDetail, autoSaveOnExtract, commitDraft, currentTextVersion, frameGroupVersion, requestExtractDraft]);
  ...
  const handleApprove = React.useCallback(async () => {
    ...
    try {
      await approveDraftDetail(currentDraft, currentDraftHtml);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : '템플릿 저장에 실패했습니다.');
    } finally {
      setLoading(false);
    }
  }, [approveDraftDetail, currentDraft, currentDraftHtml, templateName]);
