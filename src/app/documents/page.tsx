...
export default function DocumentsPage() {
  ...
  const [quickPhotoLocationText, setQuickPhotoLocationText] = React.useState('');
  const [quickPhotoLatitude, setQuickPhotoLatitude] = React.useState<number | null>(null);
  const [quickPhotoLongitude, setQuickPhotoLongitude] = React.useState<number | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (!activeDocument) {
      ...
      setQuickPhotoCapturedAt('');
      setQuickPhotoLocationText('');
      setQuickPhotoLatitude(null);
      setQuickPhotoLongitude(null);
  ...
  const handleQuickPhotoFileChange = React.useCallback(async (file: File | null) => {
    ...
    setQuickPhotoCapturedAt(detectedMetadata.capturedAt || '');
    setQuickPhotoLocationText(detectedMetadata.capturedLocationText || '');
    setQuickPhotoLatitude(detectedMetadata.capturedLatitude);
  ...
  const handleUploadAndConnectPhotoEvidence = async () => {
    ...
    try {
      ...
      formData.append('capturedLocationText', quickPhotoLocationText);
      formData.append('capturedLatitude', quickPhotoLatitude !== null ? String(quickPhotoLatitude) : '');
      formData.append('capturedLongitude', quickPhotoLongitude !== null ? String(quickPhotoLongitude) : '');
                            ...
                            <>
                              ...
                              <div className="space-y-2">
                                ...
                                <p className="text-xs text-slate-500">
                                  사진 메타데이터에 시간이 있으면 파일 선택 즉시 먼저 채우고, 필요하면 수정합니다.
                                </p>
                              </div>
                              <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-800">촬영 위치</label>
                                <Input
                                  value={quickPhotoLocationText}
                                  onChange={(event) => setQuickPhotoLocationText(event.target.value)}
                                  placeholder="예: 위도 37.123456, 경도 127.123456"
                                />
                                <p className="text-xs text-slate-500">
                                  사진 메타데이터의 GPS가 있으면 파일 선택 즉시 먼저 채우고, 필요하면 수정합니다.
                                </p>
                              </div>
                              <div className="space-y-2">
                              ...
                              <div className="space-y-2">
                                ...
                                <input
                                  ...
                                  accept="image/*"
                                  onChange={(event) => {
                                    void handleQuickPhotoFileChange(event.target.files?.[0] || null);
                                  }}
                                ...
                                <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-500">
                                  {quickPhotoFile ? (
                                    <>
                                      ...
                                      <p>자동 입력 촬영 시각: {quickPhotoCapturedAt || '-'}</p>
                                      <p>자동 입력 촬영 위치: {quickPhotoLocationText || '-'}</p>
                                    </>
                                  ) : (
                                    <p>아직 선택된 파일이 없습니다.</p>
                                  )}
                              ...
                              {latestQuickPhoto ? (
                                <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                                  ...
                                  <p>촬영 시각: {latestQuickPhoto.photo.capturedAt || '-'}</p>
                                  <p>촬영 위치: {latestQuickPhoto.photo.capturedLocationText || '-'}</p>
