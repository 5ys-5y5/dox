...
export default function PhotosPage() {
  ...
  const [capturedLocationText, setCapturedLocationText] = React.useState('');
  const [capturedLatitude, setCapturedLatitude] = React.useState<number | null>(null);
  const [capturedLongitude, setCapturedLongitude] = React.useState<number | null>(null);
  ...
  const handleSelectedFileChange = React.useCallback(async (file: File | null) => {
    setSelectedFile(file);

    if (!file) {
      setCapturedAt('');
      setCapturedLocationText('');
      setCapturedLatitude(null);
      setCapturedLongitude(null);
      return;
    }

    const detectedMetadata = await extractPhotoExifMetadata(file);
    setCapturedAt(detectedMetadata.capturedAt || '');
    setCapturedLocationText(detectedMetadata.capturedLocationText || '');
    setCapturedLatitude(detectedMetadata.capturedLatitude);
    setCapturedLongitude(detectedMetadata.capturedLongitude);
  }, []);
  ...
  const handleUploadPhoto = async () => {
    ...
    try {
      ...
      formData.append('capturedLocationText', capturedLocationText);
      formData.append('capturedLatitude', capturedLatitude !== null ? String(capturedLatitude) : '');
      formData.append('capturedLongitude', capturedLongitude !== null ? String(capturedLongitude) : '');
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-slate-500">사진 메타데이터에 시간이 있으면 먼저 채우고, 없으면 직접 입력합니다.</p>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-800">촬영 위치</label>
                  <Input
                    value={capturedLocationText}
                    onChange={(event) => setCapturedLocationText(event.target.value)}
                    placeholder="예: 위도 37.123456, 경도 127.123456"
                  />
                  <p className="text-xs text-slate-500">사진 메타데이터의 GPS가 있으면 먼저 채우고, 필요하면 수정합니다.</p>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <input
                    ...
                    accept="image/*"
                    onChange={(event) => {
                      void handleSelectedFileChange(event.target.files?.[0] || null);
                    }}
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                ...
                <p>자동 입력 촬영 시각: {capturedAt || '-'}</p>
                <p>자동 입력 촬영 위치: {capturedLocationText || '-'}</p>
              </div>
            ...
            <CardContent className="space-y-3 text-sm text-slate-700">
              {latestCreated ? (
                <>
                  ...
                  <p>촬영 위치: {latestCreated.photo.capturedLocationText || '-'}</p>
            ...
            photos.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm text-slate-600">촬영 위치: {item.photo.capturedLocationText || '-'}</p>
                  <p className="text-sm text-slate-600">
