...
export default function BulkOpsPage() {
  ...
  const [signatureSignerName, setSignatureSignerName] = React.useState('홍길동');
  const [signatureAuthRequestId, setSignatureAuthRequestId] = React.useState<string | null>(null);
  const [pendingAuthenticationId, setPendingAuthenticationId] = React.useState<string | null>(null);
  const [signatureAuthStatus, setSignatureAuthStatus] = React.useState<string | null>(null);
  ...
  const selectedSignatureDocument = React.useMemo(() => {
    if (!hasSignatureLabels || selectedDocumentIds.length !== 1) {
      return null;
    }

    return documents.find((item) => item.document.id === selectedDocumentIds[0]) || null;
  }, [documents, hasSignatureLabels, selectedDocumentIds]);
  ...
  React.useEffect(() => {
    if (!hasSignatureLabels) {
      setSelectedAuthenticationId('');
      setSignatureAuthRequestId(null);
      setPendingAuthenticationId(null);
      setSignatureAuthStatus(null);
      return;
    }
  ...
  const getSelectedDocumentDetail = async (): Promise<DocumentDetailResult> => {
    if (!selectedSignatureDocument) {
      throw new Error('서명용 본인확인은 문서 1개를 선택한 상태에서만 진행할 수 있습니다.');
    }

    const response = await fetch(`/api/documents/${encodeURIComponent(selectedSignatureDocument.document.id)}`, {
      cache: 'no-store',
    });
    const result = await response.json();

    if (!result.success) {
      throw new Error(result.message || '선택한 문서 상세 조회에 실패했습니다.');
    }

    return result.data as DocumentDetailResult;
  };

  const buildSignatureConsentText = (documentTitle: string) =>
    `${signatureSignerName}님, ${documentTitle} 문서의 서명 항목 변경을 진행하기 위한 본인확인에 동의합니다.`;

  const handleCreateSignatureAuthentication = async () => {
    if (!selectedSignatureDocument) {
      setMessage('서명 항목은 문서 1개를 선택한 상태에서만 본인확인을 시작할 수 있습니다.');
      return;
    }

    if (!signatureSignerName.trim()) {
      setMessage('서명자 이름을 입력하세요.');
      return;
    }
    ...
    try {
      const detail = await getSelectedDocumentDetail();
      const latestVersion = detail.latestVersion;

      if (!latestVersion) {
        throw new Error('선택한 문서의 최신 버전이 없어 본인확인을 시작할 수 없습니다.');
      }

      const requestResponse = await fetch('/api/sign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'REQUEST',
          documentId: selectedSignatureDocument.document.id,
          documentContent: latestVersion.htmlCanonical,
          signerInfo: {
            name: signatureSignerName.trim(),
      ...
      if (!requestResult.success) {
        throw new Error(requestResult.message || '서명 요청 생성에 실패했습니다.');
      }

      const requestId = requestResult.data.id as string;
      const authResponse = await fetch('/api/sign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'AUTH_REQUEST',
          ...
          provider: 'toss',
          documentContent: latestVersion.htmlCanonical,
          consentText: buildSignatureConsentText(selectedSignatureDocument.document.title),
          termsVersion: 'bulk-ops-v1',
        }),
      });
      const authResult = await authResponse.json();

      if (!authResult.success) {
        throw new Error(authResult.message || '본인확인 요청 생성에 실패했습니다.');
      }

      setSignatureAuthRequestId(requestId);
      setPendingAuthenticationId(authResult.data.authentication.id || null);
      setSignatureAuthStatus(authResult.data.authentication.auth_status || 'requested');
      setSelectedAuthenticationId('');
      setMessage('1단계 완료: 서명용 본인확인 요청을 만들었습니다. 바로 `검증 완료 처리`를 누르세요.');
    } catch (error) {
      const nextMessage = error instanceof Error ? error.message : '서명용 본인확인 요청 생성에 실패했습니다.';
      setMessage(nextMessage);
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifySignatureAuthentication = async () => {
    if (!selectedSignatureDocument) {
      setMessage('서명 항목은 문서 1개를 선택한 상태에서만 본인확인을 검증할 수 있습니다.');
      return;
    }

    if (!signatureAuthRequestId || !pendingAuthenticationId) {
      setMessage('먼저 본인확인 요청을 생성하세요.');
      return;
    }
    ...
    try {
      ...
      if (!latestVersion) {
        throw new Error('선택한 문서의 최신 버전이 없어 본인확인을 검증할 수 없습니다.');
      }

      const verificationResponse = await fetch('/api/sign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'AUTH_VERIFY',
          requestId: signatureAuthRequestId,
          authenticationId: pendingAuthenticationId,
          documentContent: latestVersion.htmlCanonical,
          consentText: buildSignatureConsentText(selectedSignatureDocument.document.title),
          signerName: signatureSignerName.trim(),
          birthdate: '1990-01-01',
          phone: '01012345678',
          ci: `CI-${selectedSignatureDocument.document.id}`,
          di: `DI-${selectedSignatureDocument.document.id}`,
          transactionId: `bulk-ops-${Date.now()}`,
          signedData: {
            documentId: selectedSignatureDocument.document.id,
            documentTitle: selectedSignatureDocument.document.title,
      ...
      if (!verificationResult.success) {
        throw new Error(verificationResult.message || '본인확인 검증 완료 처리에 실패했습니다.');
      }

      setSignatureAuthStatus(verificationResult.data.authentication?.auth_status || 'verified');
      await loadAuthentications();
      setPendingAuthenticationId(null);
      setSelectedAuthenticationId(verificationResult.data.authentication?.id || '');
      setMessage('2단계 완료: 본인확인 검증이 끝났습니다. 아래에서 검증 완료 인증이 자동 선택됐는지 확인한 뒤 `미리보기 계산`을 누르세요.');
    } catch (error) {
      const nextMessage = error instanceof Error ? error.message : '본인확인 검증 완료 처리에 실패했습니다.';
      setMessage(nextMessage);
    } finally {
      setLoading(false);
    }
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">BULK-EDIT-06</Badge>
          ...
          <p className="max-w-3xl text-xs text-slate-500">
            서명 항목은 한 번에 한 문서만 바꿀 수 있습니다. 화면에 보이는 1단계부터 3단계까지 순서대로 진행하면 됩니다.
          </p>
        </div>
          ...
          <CardContent className="space-y-4">
            ...
            {hasSignatureLabels ? (
              <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-4">
                <label className="text-sm font-medium text-slate-800">서명용 본인확인 3단계</label>
                  ...
                  <>
                    <div className="rounded-lg border border-slate-200 bg-white p-4">
                      <div className="grid gap-4 md:grid-cols-[1fr_auto_auto]">
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-slate-800">1. 서명자 이름 확인</label>
                          <Input
                            value={signatureSignerName}
                            onChange={(event) => setSignatureSignerName(event.target.value)}
                            placeholder="본인확인을 진행할 서명자 이름"
                          />
                          <p className="text-xs text-slate-500">
                            이 이름으로 본인확인 요청을 만들고, 같은 이름 기준으로 검증을 완료합니다.
                          </p>
                        </div>
                        <div className="flex items-end">
                          <Button type="button" variant="outline" onClick={handleCreateSignatureAuthentication} disabled={loading}>
                            2. 본인확인 시작
                          </Button>
                        </div>
                        <div className="flex items-end">
                          <Button type="button" variant="outline" onClick={handleVerifySignatureAuthentication} disabled={loading}>
                            3. 검증 완료 처리
                          </Button>
                        </div>
                      </div>
                    ...
                    <div className="rounded-lg border border-slate-200 bg-white p-4">
                      <p className="text-sm font-medium text-slate-900">진행 상태</p>
                      {signatureAuthStatus ? (
                        <p className="mt-2 text-sm text-slate-700">
                          현재 인증 상태: {signatureAuthStatus}
                          {signatureAuthRequestId ? ` / 요청 ID: ${signatureAuthRequestId}` : ''}
                        </p>
                      ) : (
                        <p className="mt-2 text-sm text-slate-600">
                          아직 서명용 본인확인 요청을 시작하지 않았습니다.
                        </p>
                      )}
                      <p className="mt-2 text-xs text-slate-500">
                        순서: 본인확인 시작 → 검증 완료 처리 → 아래 인증 선택 확인 → 미리보기 계산
                      </p>
                    </div>

                    <div className="space-y-2">
                      <label className="text-sm font-medium text-slate-800">검증 완료된 인증 선택 확인</label>
                      <EntityPicker
                        value={selectedAuthenticationId}
                        options={authOptions}
                        onChange={setSelectedAuthenticationId}
                        placeholder="검증 완료 처리 후 여기서 자동 선택됩니다"
                        emptyMessage="아직 이 문서용 검증 완료 인증이 없습니다. 위에서 `본인확인 시작`과 `검증 완료 처리`를 먼저 누르세요."
                      />
                      <p className="text-xs text-slate-500">
                        서명 항목은 이 문서용으로 검증이 끝난 인증만 사용할 수 있습니다. 추가 승인자는 현재 `확정자` 값으로 기록됩니다.
                      </p>
                    </div>
                  </>
                )}
