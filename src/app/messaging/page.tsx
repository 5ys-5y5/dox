...
export default function MessagingPage() {
  ...
  const [emailSiteId, setEmailSiteId] = React.useState('a242f858-ea43-4191-878e-6324ea2e4b5d');
  ...
  const [recentEmailRequestLinks, setRecentEmailRequestLinks] = React.useState<RecentRequestLinkListItem[]>([]);
  const [selectedEmailRequestLinkId, setSelectedEmailRequestLinkId] = React.useState('');
  const [latestEmailDispatchExecution, setLatestEmailDispatchExecution] = React.useState<EmailDispatchHistoryItemDto | null>(
    null
  );
  ...
  const emailRequestLinkOptions = React.useMemo(
    () =>
      recentEmailRequestLinks.map((item) => ({
        id: item.requestLink.id,
        label: item.documentTitle,
        meta: `${item.documentTypeKey} / ${item.maskedRecipientTarget}`,
      })),
    [recentEmailRequestLinks]
  );
  const selectedEmailRequestLink = React.useMemo(
    () => recentEmailRequestLinks.find((item) => item.requestLink.id === selectedEmailRequestLinkId) || null,
    [recentEmailRequestLinks, selectedEmailRequestLinkId]
  );
  ...
  const loadRecentEmailRequestLinks = React.useCallback(async () => {
    try {
      const response = await fetch(`/api/request-links?siteId=${encodeURIComponent(emailSiteId)}&limit=12`, {
        cache: 'no-store',
      });
      const result = await response.json();

      if (!result.success) {
        throw new Error(result.message || '이메일 요청 링크 목록 조회 실패');
      }

      const items = (result.data as RecentRequestLinkListItem[]).filter(
        (item) => item.requestLink.recipientChannel === 'email'
      );
      setRecentEmailRequestLinks(items);
      setSelectedEmailRequestLinkId((previous) => {
      ...
      setRecentEmailRequestLinks([]);
      setSelectedEmailRequestLinkId('');
    }
  }, [emailSiteId]);

  React.useEffect(() => {
    void loadRecentEmailRequestLinks();
  }, [loadRecentEmailRequestLinks]);
  ...
  const handleSendEmail = async () => {
    if (!selectedEmailRequestLinkId || !selectedEmailRequestLink) {
      setMessage('먼저 이메일 발송할 요청 링크를 선택하세요.');
      return;
    }

    if (selectedEmailRequestLink.requestLink.recipientChannel !== 'email') {
      setMessage('이메일 발송은 recipientChannel 이 email 인 요청 링크에서만 사용할 수 있습니다.');
      return;
    }

    const subject = `[MEJAI] 문서 입력 요청: ${selectedEmailRequestLink.documentTitle || '문서 입력 요청'}`;
    ...
    try {
      const requestLinkResponse = await fetch(
        `/api/request-links?dispatchUrlFor=${encodeURIComponent(selectedEmailRequestLinkId)}`,
        {
          cache: 'no-store',
        }
      );
      ...
      const htmlBody = [
        '<div>',
        `<p>안녕하세요${selectedEmailRequestLink.requestLink.recipientName ? `, ${selectedEmailRequestLink.requestLink.recipientName}님` : ''}.</p>`,
        `<p>${selectedEmailRequestLink.documentTitle || '문서'} 관련 요청 링크를 전달드립니다.</p>`,
      ...
      const response = await fetch('/api/messaging/email/send', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          requestLinkId: selectedEmailRequestLinkId,
          requestedBy: 'messaging-ops-user',
          subject,
          htmlBody,
        }),
      });
      ...
      const historyItem: EmailDispatchHistoryItemDto = result.data;
      setLatestEmailDispatchExecution(historyItem);
      await loadMessagingData();
      await loadRecentEmailRequestLinks();
      setMessage(
        `이메일 발송 상태: ${historyItem.dispatch.status} / 대상 ${historyItem.dispatch.recipientCount}건 / 성공 ${historyItem.dispatch.sentCount}건 / 실패 ${historyItem.dispatch.failedCount}건`
      );
    } catch (error) {
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">MESSAGING-15</Badge>
          ...
          <p className="max-w-3xl text-sm text-slate-600">
            ...
            최근 발송 이력과 상태 동기화, 이메일 실행 같은 운영 작업을 담당합니다.
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-slate-200">
            <CardHeader>
              ...
              <CardDescription>
                요청 링크를 선택해 이 화면에서도 바로 이메일을 발송합니다. 발송 상세 이력은 아래 카드에서 계속 확인합니다.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-800">요청 링크 기준 현장</label>
                <EntityPicker
                  value={emailSiteId}
                  options={siteOptions}
                  onChange={setEmailSiteId}
                  placeholder="현장을 선택하세요"
                  emptyMessage="저장된 현장이 없습니다."
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-800">이메일 요청 링크</label>
                <EntityPicker
                  value={selectedEmailRequestLinkId}
                  options={emailRequestLinkOptions}
                  onChange={setSelectedEmailRequestLinkId}
                  placeholder="최근 이메일 요청 링크를 선택하세요"
                  emptyMessage="선택한 현장에 이메일 요청 링크가 없습니다."
                />
              </div>
              {selectedEmailRequestLink ? (
                <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                  <p>수신자: {selectedEmailRequestLink.requestLink.recipientName || '-'}</p>
                  <p>수신 이메일: {selectedEmailRequestLink.requestLink.recipientTarget}</p>
                  <p>만료 시각: {selectedEmailRequestLink.requestLink.expiresAt}</p>
                </div>
              ) : null}
              <Button variant="outline" onClick={handleSendEmail} disabled={loading || !selectedEmailRequestLinkId}>
                이메일 발송
              </Button>
              {latestEmailDispatchExecution ? (
                <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-700">
                  <p className="font-medium text-slate-900">방금 실행한 이메일 발송</p>
                  <p className="mt-2">상태: {latestEmailDispatchExecution.dispatch.status}</p>
                  <p>대상 수신자: {latestEmailDispatchExecution.dispatch.recipientCount}건</p>
                  <p>성공: {latestEmailDispatchExecution.dispatch.sentCount}건</p>
                  <p>실패: {latestEmailDispatchExecution.dispatch.failedCount}건</p>
                  <div className="mt-3 space-y-2 border-t border-slate-200 pt-3">
                    {latestEmailDispatchExecution.targets.map((target) => (
                      <div key={target.id} className="text-xs text-slate-600">
          ...
          <Card className="border-slate-200">
            <CardHeader>
              ...
              <CardDescription>
                request-links 와 messaging 에서 실행한 이메일 발송을 같은 정본 이력으로 확인합니다.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 text-sm text-slate-700">
