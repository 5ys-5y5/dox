...
const MEASUREMENT_TEXT_FIT_SCRIPT = `(() => {
  ...
  const applyFit = () => {
    const elements = Array.from(document.querySelectorAll('[data-template-fit-target-width]'));
    ...
    document.documentElement.setAttribute('data-template-fit-applied', 'true');
  };
  const scheduleApplyFit = () => requestAnimationFrame(() => requestAnimationFrame(applyFit));
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', scheduleApplyFit, { once: true });
  } else {
    scheduleApplyFit();
  }
})();`;
...
<html>
  ...
  <body>${sanitizedHtml}<script>${MEASUREMENT_TEXT_FIT_SCRIPT}</script></body>
</html>`;
};
  ...
  await client.send(
    'Runtime.evaluate',
    {
      expression: `new Promise((resolve) => {
        const hasTargets = document.querySelector('[data-template-fit-target-width]');
        if (!hasTargets) {
          resolve(true);
          return;
        }
        ...
        const tick = () => {
          if (document.documentElement.getAttribute('data-template-fit-applied') === 'true') {
            resolve(true);
            return;
          }
