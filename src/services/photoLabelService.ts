...
type PhotoRegistryRow = {
  ...
  captured_location_text: string | null;
...
const toPhotoRecordDto = (row: PhotoRegistryRow): PhotoRecordDto => ({
  ...
  capturedLocationText: row.captured_location_text,
  capturedLatitude: row.captured_latitude,
  capturedLongitude: row.captured_longitude,
  status: row.status,
...
const PHOTO_STORAGE_ALLOWED_MIME_TYPES = ['image/jpeg', 'image/png', 'image/webp', 'image/heic', 'image/heif'];
const PHOTO_STORAGE_FILE_SIZE_LIMIT = 10485760;

const ensurePhotoStorageBucket = async (client: ReturnType<typeof getSupabase>) => {
  ...
  if (typeof storageApi.getBucket === 'function') {
    const existingBucket = await storageApi.getBucket(PHOTO_STORAGE_BUCKET);
    ...
    if (!shouldCreateBucket) {
      throw new Error(
        `사진 업로드 실패: Storage 버킷 상태를 확인할 수 없습니다. (${existingBucket.error.message || '알 수 없는 오류'})`
      );
    }
  }

  if (typeof storageApi.createBucket !== 'function') {
    throw new Error('사진 업로드 실패: Storage 버킷 자동 생성을 지원하지 않는 환경입니다.');
  }

  const createdBucket = await storageApi.createBucket(PHOTO_STORAGE_BUCKET, {
    public: true,
    fileSizeLimit: PHOTO_STORAGE_FILE_SIZE_LIMIT,
    allowedMimeTypes: PHOTO_STORAGE_ALLOWED_MIME_TYPES,
  });

  if (createdBucket.error && !createdBucket.error.message?.includes('already exists')) {
    throw new Error(
      `사진 업로드 실패: Storage 버킷을 자동 생성할 수 없습니다. (${createdBucket.error.message || '알 수 없는 오류'})`
    );
  }
};

const isPhotoRegistrySchemaCacheError = (message?: string | null) =>
  Boolean(message?.includes('schema cache') && message?.includes('photo_registry'));
...
const insertPhotoRecord = async (params: PhotoCreateInput): Promise<PhotoCreateResult> => {
  const fullInsertPayload = {
    ...
    captured_location_text: params.capturedLocationText?.trim() || null,
    captured_latitude:
      typeof params.capturedLatitude === 'number' && Number.isFinite(params.capturedLatitude)
        ? params.capturedLatitude
        : null,
    captured_longitude:
      typeof params.capturedLongitude === 'number' && Number.isFinite(params.capturedLongitude)
        ? params.capturedLongitude
        : null,
    status: 'active' as const,
  };

  let { data, error } = await photosSchema().from('photo_registry').insert([fullInsertPayload]).select('*').single();

  if (error && isPhotoRegistrySchemaCacheError(error.message)) {
    const legacyInsertPayload = {
      site_id: fullInsertPayload.site_id,
      photo_url: fullInsertPayload.photo_url,
      storage_path: fullInsertPayload.storage_path,
      photo_title: fullInsertPayload.photo_title,
      description: fullInsertPayload.description,
      captured_at: fullInsertPayload.captured_at,
      status: fullInsertPayload.status,
    };

    ({ data, error } = await photosSchema()
      .from('photo_registry')
...
export const PhotoLabelService = {
  ...
  async uploadPhoto(params: PhotoUploadInput): Promise<PhotoUploadResult> {
    ...
    const uploadToStorage = async () =>
      client.storage.from(PHOTO_STORAGE_BUCKET).upload(storagePath, params.fileBytes, {
    ...
    if (uploadError?.message?.includes('Bucket not found')) {
      await ensurePhotoStorageBucket(client);
    ...
    try {
      return await insertPhotoRecord({
        ...
        capturedLocationText: params.capturedLocationText?.trim() || null,
        capturedLatitude: params.capturedLatitude ?? null,
        capturedLongitude: params.capturedLongitude ?? null,
      });
    } catch (error) {
