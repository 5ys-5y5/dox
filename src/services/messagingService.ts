...
import type {
  ...
  SmsDocumentRecipientBindingDto,
  SmsDocumentSenderBindingDto,
...
type SmsDocumentSenderBindingRow = {
  id: string;
  document_id: string;
  sender_id: string;
  created_at: string;
...
type SmsDocumentRecipientBindingRow = {
  id: string;
  document_id: string;
  recipient_id: string;
  created_at: string;
...
const toDocumentSenderBindingDto = (
  row: SmsDocumentSenderBindingRow,
  sender: SmsSenderRow
): SmsDocumentSenderBindingDto => ({
  id: row.id,
...
const toDocumentRecipientBindingDto = (
  documentId: string,
  recipients: SmsRecipientRow[]
): SmsDocumentRecipientBindingDto => ({
  documentId,
  recipientIds: recipients.map((recipient) => recipient.id),
  recipients: recipients.map((recipient) => toRecipientDto(recipient)),
});

const toSettingsDto = (
  row: SmsSettingsRow | null,
  documentSenderBindings: SmsDocumentSenderBindingDto[] = [],
  documentRecipientBindings: SmsDocumentRecipientBindingDto[] = []
): SmsSettingsDto => ({
  defaultSenderPhoneNumber: row?.default_sender_phone_number || process.env.REQUEST_LINK_SMS_DEFAULT_SENDER?.trim() || null,
  documentSenderBindings,
  documentRecipientBindings,
...
export const MessagingService = {
  async getSmsSettings(): Promise<SmsSettingsDto> {
    ...
    const { data: senderBindingRows, error: senderBindingError } = await messagingSchema()
      .from('sms_document_sender_registry')
      .select('*')
      .order('created_at', { ascending: false });

    if (senderBindingError) {
      if (
        senderBindingError.message.includes('schema cache') ||
        senderBindingError.message.includes('sms_document_sender_registry') ||
        senderBindingError.message.includes('relation')
      ) {
        return toSettingsDto((data as SmsSettingsRow | null) || null, [], []);
      }

      throw new Error(`문서별 기본 발신번호 조회 실패: ${senderBindingError.message}`);
    }

    const { data: recipientBindingRows, error: recipientBindingError } = await messagingSchema()
      .from('sms_document_recipient_registry')
      .select('*')
      .order('created_at', { ascending: false });

    if (recipientBindingError) {
      if (
        recipientBindingError.message.includes('schema cache') ||
        recipientBindingError.message.includes('sms_document_recipient_registry') ||
        recipientBindingError.message.includes('relation')
      ) {
        return toSettingsDto((data as SmsSettingsRow | null) || null, [], []);
      }

      throw new Error(`문서별 기본 수신번호 조회 실패: ${recipientBindingError.message}`);
    }

    const senderIds = Array.from(
      new Set((senderBindingRows || []).map((row) => String((row as SmsDocumentSenderBindingRow).sender_id)))
    );
    let senderMap = new Map<string, SmsSenderRow>();

    if (senderIds.length > 0) {
      const { data: senderRows, error: senderError } = await messagingSchema()
      ...
      if (senderError) {
        throw new Error(`문서별 기본 발신번호 발신번호 조회 실패: ${senderError.message}`);
      }

      senderMap = new Map((senderRows || []).map((row) => [String((row as SmsSenderRow).id), row as SmsSenderRow]));
    }

    const documentSenderBindings = (senderBindingRows || [])
      .map((row) => {
        const bindingRow = row as SmsDocumentSenderBindingRow;
        const sender = senderMap.get(bindingRow.sender_id);
        return sender ? toDocumentSenderBindingDto(bindingRow, sender) : null;
      })
      .filter((item): item is SmsDocumentSenderBindingDto => Boolean(item));

    const recipientIds = Array.from(
      new Set((recipientBindingRows || []).map((row) => String((row as SmsDocumentRecipientBindingRow).recipient_id)))
    );
    let recipientMap = new Map<string, SmsRecipientRow>();

    if (recipientIds.length > 0) {
      const { data: recipientRows, error: recipientError } = await messagingSchema()
      ...
      if (recipientError) {
        throw new Error(`문서별 기본 수신번호 수신번호 조회 실패: ${recipientError.message}`);
      }

      recipientMap = new Map((recipientRows || []).map((row) => [String((row as SmsRecipientRow).id), row as SmsRecipientRow]));
    }

    const documentRecipientGroups = new Map<string, SmsRecipientRow[]>();

    for (const row of recipientBindingRows || []) {
      const bindingRow = row as SmsDocumentRecipientBindingRow;
      const recipient = recipientMap.get(bindingRow.recipient_id);

      if (!recipient) {
        continue;
      }

      const bucket = documentRecipientGroups.get(bindingRow.document_id) || [];
      bucket.push(recipient);
      documentRecipientGroups.set(bindingRow.document_id, bucket);
    }

    const documentRecipientBindings = Array.from(documentRecipientGroups.entries()).map(([documentId, recipients]) =>
      toDocumentRecipientBindingDto(documentId, recipients)
    );

    return toSettingsDto((data as SmsSettingsRow | null) || null, documentSenderBindings, documentRecipientBindings);
  },
  ...
  async updateSmsSettings(input: SmsSettingsUpdateInput): Promise<SmsSettingsDto> {
    ...
    const normalizedDocumentSenderBindings = Array.isArray(input.documentSenderBindings)
      ? Array.from(
          new Map(
            input.documentSenderBindings
              .map((value) => ({
                documentId: value.documentId?.trim() || '',
                senderId: value.senderId?.trim() || null,
              }))
              .filter((value) => value.documentId)
              .map((value) => [value.documentId, value])
          ).values()
        )
      : input.documentSenderBindings;
    const normalizedDocumentRecipientBindings = Array.isArray(input.documentRecipientBindings)
      ? Array.from(
          new Map(
            input.documentRecipientBindings
              .map((value) => ({
                documentId: value.documentId?.trim() || '',
                recipientIds: Array.from(
                  new Set((value.recipientIds || []).map((recipientId) => recipientId?.trim() || '').filter(Boolean))
                ),
              }))
              .filter((value) => value.documentId)
              .map((value) => [value.documentId, value])
    ...
    const senderIdsToValidate = Array.isArray(normalizedDocumentSenderBindings)
      ? Array.from(
          new Set(
            normalizedDocumentSenderBindings
              .map((value) => value.senderId)
              .filter((value): value is string => Boolean(value))
          )
        )
      : [];
    const recipientIdsToValidate = Array.isArray(normalizedDocumentRecipientBindings)
      ? Array.from(
          new Set(
            normalizedDocumentRecipientBindings.flatMap((value) => value.recipientIds).filter((value): value is string => Boolean(value))
          )
        )
      : [];

    if (senderIdsToValidate.length > 0) {
      const { data: senderRows, error: senderError } = await messagingSchema()
        .from('sms_sender_registry')
        .select('id')
        .in('id', senderIdsToValidate)
        .eq('is_active', true);

      if (senderError) {
        throw new Error(`문자 발송 설정 검증 실패: ${senderError.message}`);
      }

      const foundSenderIds = new Set((senderRows || []).map((row) => String((row as { id: string }).id)));
      const missingSenderIds = senderIdsToValidate.filter((value) => !foundSenderIds.has(value));

      if (missingSenderIds.length > 0) {
        throw new Error(`문자 발송 설정 저장 실패: 문서별 기본 발신번호를 찾을 수 없습니다. (${missingSenderIds.join(', ')})`);
      }
    }

    if (recipientIdsToValidate.length > 0) {
      const { data: recipientRows, error: recipientError } = await messagingSchema()
        .from('sms_recipient_registry')
        .select('id')
        .in('id', recipientIdsToValidate)
        .eq('is_active', true);

      if (recipientError) {
        throw new Error(`문자 발송 설정 검증 실패: ${recipientError.message}`);
      }

      const foundRecipientIds = new Set((recipientRows || []).map((row) => String((row as { id: string }).id)));
      const missingRecipientIds = recipientIdsToValidate.filter((value) => !foundRecipientIds.has(value));

      if (missingRecipientIds.length > 0) {
        throw new Error(
          `문자 발송 설정 저장 실패: 문서별 기본 수신번호를 찾을 수 없습니다. (${missingRecipientIds.join(', ')})`
        );
      }
    }
    ...
    let settingsRow = existingData as SmsSettingsRow | null;
    ...
    if (!existingData) {
      ...
      settingsRow = data as SmsSettingsRow;
    }
    ...
    if (Object.keys(patch).length > 0) {
      const { data, error } = await messagingSchema()
        .from('sms_service_settings')
        .update(patch)
        .eq('id', (settingsRow as SmsSettingsRow).id)
        .select('*')
        .single();
    ...
    if (Array.isArray(normalizedDocumentSenderBindings)) {
      const documentIds = normalizedDocumentSenderBindings.map((item) => item.documentId);
      const bindingsToDelete = normalizedDocumentSenderBindings.filter((item) => !item.senderId).map((item) => item.documentId);
      const bindingsToUpsert = normalizedDocumentSenderBindings.filter(
        (item): item is { documentId: string; senderId: string } => Boolean(item.senderId)
      );

      if (bindingsToDelete.length > 0) {
        const { error: deleteError } = await messagingSchema()
          .from('sms_document_sender_registry')
          .delete()
          .in('document_id', bindingsToDelete);

        if (deleteError) {
          throw new Error(`문서별 기본 발신번호 삭제 실패: ${deleteError.message}`);
        }
      }

      if (bindingsToUpsert.length > 0) {
        const { error: upsertError } = await messagingSchema()
          .from('sms_document_sender_registry')
          .upsert(
            bindingsToUpsert.map((item) => ({
        ...
        if (upsertError) {
          throw new Error(`문서별 기본 발신번호 저장 실패: ${upsertError.message}`);
        }
      }
    }

    if (Array.isArray(normalizedDocumentRecipientBindings)) {
      const documentIds = normalizedDocumentRecipientBindings.map((item) => item.documentId);
      ...
      if (documentIds.length > 0) {
        ...
        if (deleteError) {
          throw new Error(`문서별 기본 수신번호 삭제 실패: ${deleteError.message}`);
        }
      }

      const bindingsToInsert = normalizedDocumentRecipientBindings.flatMap((item) =>
        item.recipientIds.map((recipientId) => ({
          document_id: item.documentId,
      ...
      if (bindingsToInsert.length > 0) {
        const { error: insertError } = await messagingSchema().from('sms_document_recipient_registry').insert(bindingsToInsert);

        if (insertError) {
          throw new Error(`문서별 기본 수신번호 저장 실패: ${insertError.message}`);
        }
      }
    }
  ...
  async listEmailDispatches(limit = 12): Promise<EmailDispatchHistoryItemDto[]> {
    const safeLimit = Number.isFinite(limit) ? Math.max(1, Math.min(Math.trunc(limit), 50)) : 12;
    ...
    if (dispatchError) {
      throw new Error(`이메일 발송 이력 조회 실패: ${dispatchError.message}`);
    }

    const dispatches = (dispatchData || []).map((row) => toEmailDispatchDto(row as EmailDispatchRow));
    ...
    if (targetError) {
      throw new Error(`이메일 발송 이력 target 조회 실패: ${targetError.message}`);
    }

    const groupedTargets = new Map<string, EmailDispatchTargetDto[]>();

    for (const row of targetData || []) {
      const target = toEmailDispatchTargetDto(row as EmailDispatchTargetRow);
      const bucket = groupedTargets.get(target.dispatchId) || [];
      bucket.push(target);
      groupedTargets.set(target.dispatchId, bucket);
    }
