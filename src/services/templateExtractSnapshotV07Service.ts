...
const TemplateExtractSnapshotV07Layout = {
...
const PDF_LAYOUT_EXTRACT_SCRIPT_V07 = `import Foundation
import PDFKit
...
const normalizeWhitespaceV07 = (value: string) => value.replace(/\s+/g, ' ').trim();
const escapeRegExpV07 = (value: string) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

const normalizePdfFallbackTextV07 = (value: string) => {
  const withLineBreaks = value.replace(/\r/g, '').replace(/\n{3,}/g, '\n\n');
  const labelPattern = new RegExp(
    `(${PDF_FALLBACK_LABELS_V07.map((label) => escapeRegExpV07(label)).join('|')})\\s*`,
    'g'
  );
  ...
  return marked
    .split('\n')
    .map((line) => normalizeWhitespaceV07(line))
    .filter(Boolean)
    .join('\n');
};

const extractPdfLayoutV07 = async (fileName: string, bytes: Uint8Array): Promise<TemplateExtractPdfLayoutModel> => {
  const tempDir = await mkdtemp(join(tmpdir(), 'template-extract-pdf-v07-'));
  const tempFilePath = join(tempDir, fileName || 'upload.pdf');
  const tempScriptPath = join(tempDir, 'extract-pdf-layout-v07.swift');

  try {
    await writeFile(tempFilePath, bytes);
    await writeFile(tempScriptPath, PDF_LAYOUT_EXTRACT_SCRIPT_V07);
...
export const TemplateExtractSnapshotV07Service = {
  async extractPdfSource(fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> {
    const layout = await extractPdfLayoutV07(fileName, bytes);
    ...
    const clonedHtml = TemplateExtractSnapshotV07Layout.buildCloneHtml(sourceTitle, rawText, layout);
    ...
    return {
      sourceTitle,
      sourceKind: 'text',
      sourceContent: normalizePdfFallbackTextV07(rawText),
      originalFileName: fileName,
      originalMimeType: 'application/pdf',
    };
  },
