...
const buildCloneStyleHtml = () => `    <style>
      ...
      .template-clone--pdf-form-v12 h1 {
        margin: 0 0 12px;
        text-align: center;
        font-size: 24px;
        font-weight: 700;
      ...
      .template-clone--pdf-form-v12 table {
        width: 100%;
      ...
      .template-clone--pdf-form-v12 th,
      .template-clone--pdf-form-v12 td {
...
const buildStructuredCloneHtml = (structured: StructuredWorkOrder, layout?: TemplateExtractPdfLayoutModel | null) => `<section data-template-extract-draft="true" data-template-clone="pdf-form-v12">
  <div class="template-clone template-clone--pdf-form-v12">
${buildCloneStyleHtml()}
    <h1>작업지시서</h1>
...
export const TemplateExtractSnapshotV12Layout = {
  // BUILDHTML-PDF-12
  // current 경로는 v11 form clone을 유지하면서
  // 1) status actor 를 CE/PM 고정이 아니라 동적으로 렌더링하고
  // 2) OCR fallback 으로 텍스트 레이어가 없는 PDF 도 structured parse 로 진입하게 합니다.
...
export const TemplateExtractSnapshotV12Service = {
  async extractPdfSource(fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> {
    ...
    const clonedHtml = TemplateExtractSnapshotV12Layout.buildCloneHtml(sourceTitle, rawText, layout);
