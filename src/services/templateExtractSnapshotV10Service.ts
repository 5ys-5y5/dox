...
const buildCloneStyleHtml = () => `    <style>
      ...
      .template-clone--pdf-form-v10 h1 {
        margin: 0 0 12px;
        text-align: center;
        font-size: 24px;
        font-weight: 700;
      ...
      .template-clone--pdf-form-v10 table {
        width: 100%;
      ...
      .template-clone--pdf-form-v10 th,
      .template-clone--pdf-form-v10 td {
      ...
      .template-clone__section-table th.template-clone__section-heading {
        text-align: left;
...
const buildSingleSectionTable = (section: PdfSectionBlock, hint: SectionHeadingHint | null | undefined) => {
  ...
  const rows = valueLines
    .map((line, index) => {
      const headingCell =
        index === 0
          ? `<th class="template-clone__section-heading" rowspan="${valueLines.length}">${buildSectionHeadingHtml(section)}</th>`
          : '';
  ...
  const rows = Array.from({ length: rowCount })
    .map((_, index) => {
      const leftHeadingCell =
        index === 0
          ? `<th class="template-clone__section-heading" rowspan="${rowCount}">${buildSectionHeadingHtml(leftSection)}</th>`
          : '';
      const rightHeadingCell =
        index === 0
          ? `<th class="template-clone__section-heading" rowspan="${rowCount}">${buildSectionHeadingHtml(rightSection)}</th>`
          : '';
...
const buildStructuredCloneHtml = (structured: StructuredWorkOrder, layout?: TemplateExtractPdfLayoutModel | null) => `<section data-template-extract-draft="true" data-template-clone="pdf-form-v10">
  <div class="template-clone template-clone--pdf-form-v10">
${buildCloneStyleHtml()}
    <h1>작업지시서</h1>
...
export const TemplateExtractSnapshotV10Layout = {
...
export const TemplateExtractSnapshotV10Service = {
  async extractPdfSource(fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> {
    ...
    const clonedHtml = TemplateExtractSnapshotV10Layout.buildCloneHtml(sourceTitle, rawText, layout);
