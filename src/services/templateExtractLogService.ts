...
const buildMeasurementNote = (
  qualityReport: TemplateExtractReplicaQualityReport | null | undefined,
  visualSimilarityReport: TemplateExtractVisualSimilarityReport | null | undefined
) => {
  if (visualSimilarityReport?.measured) {
    const frameScore = visualSimilarityReport.frameScore ?? visualSimilarityReport.overallScore;
    const combinedScore = visualSimilarityReport.combinedScore;
    const combinedSuffix =
      typeof combinedScore === 'number' ? ` / 전체 ink 중첩률: ${(combinedScore * 100).toFixed(2)}%` : '';

    return `${visualSimilarityReport.measurementMode} 기준 1px 허용 오차 프레임 중첩률: ${(
      frameScore * 100
    ).toFixed(2)}%${combinedSuffix}`;
  }
...
- measurementNote: ${buildMeasurementNote(input.qualityReport, input.visualSimilarityReport)}
...
${toMarkdownJson(buildMeasurementStatus(input.qualityReport, input.visualSimilarityReport))}
