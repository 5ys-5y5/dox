...
export type TemplateExtractPdfTableFragmentFrameBuildOptions = {
  includeMaskEdges?: boolean;
...
const MASK_RULE_SNAP_TOLERANCE_PX = 3.25;
...
const buildMaskCandidateSegments = (
  ...
  const horizontal = maskPage.rects
    .filter((rect) => rect.height <= MAX_MASK_RULE_THICKNESS && rect.width >= minimumLength)
    .map(
      (rect) =>
        ({
          orientation: 'h' as const,
          left: clampCoordinate(rect.x * scaleX, 0, page.width),
          top: clampCoordinate((rect.y + rect.height / 2) * scaleY, 0, page.height),
          width: Number(Math.max(1, rect.width * scaleX).toFixed(2)),
        }) satisfies TemplateExtractPdfTableFragmentFrameSegment
    )
    .sort((left, right) => left.top - right.top || left.left - right.left);
  const vertical = maskPage.rects
    .filter((rect) => rect.width <= MAX_MASK_RULE_THICKNESS && rect.height >= minimumLength)
    .map(
      (rect) =>
        ({
          orientation: 'v' as const,
          left: clampCoordinate((rect.x + rect.width / 2) * scaleX, 0, page.width),
          top: clampCoordinate(rect.y * scaleY, 0, page.height),
          height: Number(Math.max(1, rect.height * scaleY).toFixed(2)),
        }) satisfies TemplateExtractPdfTableFragmentFrameSegment
    )
    .sort((left, right) => left.left - right.left || left.top - right.top);

  return mergeSegments([...horizontal, ...vertical]).filter((segment) => {
    if (segment.orientation === 'h') {
      return segment.width >= minimumLength;
    }

    return segment.height >= minimumLength;
  });
};
...
const snapAxis = (value: number, axes: number[], tolerance = MASK_RULE_SNAP_TOLERANCE_PX) => {
  const nearest = findNearestAxis(value, axes, tolerance);
...
const buildMaskSnappedFrameSegments = (
  page: TemplateExtractPdfPage,
  maskPage: TemplateExtractPdfMaskVectorPage | null | undefined,
  grid: RuleGrid,
  minimumLength = MIN_MASK_RULE_LENGTH
) => {
  if (!maskPage) {
  ...
  const candidates = buildMaskCandidateSegments(page, maskPage, minimumLength);
  const segments: TemplateExtractPdfTableFragmentFrameSegment[] = [];
  const seen = new Set<string>();

  for (const candidate of candidates) {
    if (candidate.orientation === 'h') {
      const snappedY = snapAxis(candidate.top, grid.rowRules);
      const candidateRight = candidate.left + candidate.width;
      const snappedLeft = snapAxis(candidate.left, grid.columnRules, 5);
      const snappedRight = snapAxis(candidateRight, grid.columnRules, 5);
      const tableOverlap = overlapLength(candidate.left, candidateRight, grid.outerLeft, grid.outerRight);
      const isFullWidth = tableOverlap >= (grid.outerRight - grid.outerLeft) * 0.82;
      const isRowRule = snappedY != null && tableOverlap >= Math.max(24, candidate.width * 0.55);
      const isLocalDivider =
        snappedY == null &&
        candidate.top >= grid.outerTop - MASK_RULE_SNAP_TOLERANCE_PX &&
        candidate.top <= grid.outerBottom + MASK_RULE_SNAP_TOLERANCE_PX &&
        snappedLeft != null &&
        snappedRight != null &&
        snappedRight > snappedLeft &&
        snappedRight - snappedLeft >= Math.max(24, candidate.width * 0.72);

      if (!isRowRule && !isLocalDivider) {
        continue;
      }

      const left = isFullWidth
        ? grid.outerLeft
        : clampCoordinate(snappedLeft ?? Math.max(candidate.left, grid.outerLeft), grid.outerLeft, grid.outerRight);
      const right = isFullWidth
        ? grid.outerRight
        : clampCoordinate(snappedRight ?? Math.min(candidateRight, grid.outerRight), grid.outerLeft, grid.outerRight);

      if (right - left < Math.max(12, minimumLength * 0.5)) {
        continue;
      }
      ...
      appendSegment(segments, seen, {
        orientation: 'h',
        left,
        top: snappedY ?? candidate.top,
        width: Number((right - left).toFixed(2)),
      });
      continue;
    }

    const snappedX = snapAxis(candidate.left, grid.columnRules);
    ...
    const top = clampCoordinate(candidate.top, grid.outerTop, grid.outerBottom);
    const bottom = clampCoordinate(candidateBottom, grid.outerTop, grid.outerBottom);

    if (bottom - top < Math.max(10, minimumLength * 0.45)) {
      continue;
    }
...
const buildRuleBandedSegments = (
  ...
  explicitSegments: TemplateExtractPdfTableFragmentFrameSegment[],
  grid: RuleGrid,
  options: TemplateExtractPdfTableFragmentFrameBuildOptions = {}
  ...
  if (options.includeMaskEdges) {
    for (const maskSegment of buildMaskSnappedFrameSegments(page, options.maskPage, grid, options.maskRuleMinLength)) {
      appendSegment(segments, seen, maskSegment);
    }
  }
  ...
  return {
    segments: merged,
    source: options.includeMaskEdges && options.maskPage ? 'mixed' : geometryRows.length > 0 ? 'rule_geometry' : 'rule_only',
...
export const TemplateExtractPdfTableFragmentFrameService = {
  buildFrameSegments(
    ...
    if (ruleGrid) {
      return buildRuleBandedSegments(page, geometryRows, explicitSegments, ruleGrid, options);
    }
