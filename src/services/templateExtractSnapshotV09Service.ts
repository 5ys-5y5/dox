...
const buildAbsoluteCloneStyleHtml = () => `    <style>
      ...
      .template-clone__pdf-page-image {
        position: absolute;
        inset: 0;
        width: 100%;
        height: 100%;
      ...
      .template-clone__pdf-page-overlay {
        position: absolute;
        inset: 0;
      }
      ...
      .template-clone__pdf-mask--inline {
        min-width: 56px;
        min-height: 14px;
      }

      .template-clone__pdf-mask--block {
        min-width: 72px;
        min-height: 16px;
      }
    </style>`;
...
const renderInlineSegments = (
  ...
  const normalizedLineLength = Math.max(normalizeWhitespace(lineText).length, 1);
  let consumedLength = 0;

  return segments
    .map((segment) => {
      const normalizedSegmentLength = Math.max(normalizeWhitespace(segment.text).length, segment.text.length, 1);

      if (!segment.label) {
        consumedLength += normalizedSegmentLength;
        return '';
      }

      const leftOffset = Math.max(
        Math.round((consumedLength / normalizedLineLength) * lineWidth),
        0
      );
      const proportionalWidth = Math.max(
        Math.round((normalizedSegmentLength / normalizedLineLength) * lineWidth),
        segment.block ? Math.round(lineWidth * 0.72) : 56
      );
      const boundedWidth = Math.min(proportionalWidth, Math.max(Math.round(lineWidth), 56));
      const maskWidth = segment.block ? boundedWidth : Math.max(boundedWidth, 56);
      const styleAttribute = ` style="left:${leftOffset}px;width:${maskWidth}px;"`;
      consumedLength += normalizedSegmentLength;

      return `<span data-template-value="${escapeAttribute(segment.label)}" class="template-clone__pdf-mask template-clone__pdf-mask--${
        segment.block ? 'block' : 'inline'
      }"${styleAttribute}>${escapeHtml(segment.text)}</span>`;
    })
    .join('');
};
...
const buildGenericLineHtml = (
  ...
  if (matched) {
    return {
      html: renderInlineSegments(text, line.width, [
        { text: '양식명(코드) : ' },
  ...
  if (pendingFieldLabel) {
    ...
    if (shouldStopPending) {
      ...
      return {
        html: renderInlineSegments(text, line.width, [{ text, label: pendingFieldLabel, block: true }]),
        nextSectionLabel: previousSectionLabel,
  ...
  if (
    ...
    return {
      html: renderInlineSegments(text, line.width, [{ text, label: nextSectionMatch.label, block: true }]),
  ...
  if (isLikelyValueContinuation(line, effectiveSectionLabel)) {
    return {
      html: renderInlineSegments(text, line.width, [{ text, label: effectiveSectionLabel!, block: true }]),
  ...
  const pagesHtml = layout.pages
    .map((page) => {
      ...
      return `    <div class="template-clone__pdf-page" data-page="${page.pageNumber}" style="width:${page.width.toFixed(
        ...
        (page as TemplateExtractPdfPageWithImage).backgroundImageDataUrl
          ? `      <img class="template-clone__pdf-page-image" src="${escapeAttribute((page as TemplateExtractPdfPageWithImage).backgroundImageDataUrl || '')}" alt="PDF page ${page.pageNumber}" />\n`
          : ''
      }      <div class="template-clone__pdf-page-overlay">\n${linesHtml}\n      </div>\n    </div>`;
    })
    .join('\n');
...
type TemplateExtractPdfPageWithImage = TemplateExtractPdfLayoutModel['pages'][number] & {
  backgroundImageDataUrl?: string | null;
};

const TemplateExtractSnapshotV09Layout = {
...
const PDF_LAYOUT_EXTRACT_SCRIPT_V09 = `import Foundation
import PDFKit
...
const normalizeWhitespaceV09 = (value: string) => value.replace(/\s+/g, ' ').trim();
const escapeRegExpV09 = (value: string) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

const normalizePdfFallbackTextV09 = (value: string) => {
  const withLineBreaks = value.replace(/\r/g, '').replace(/\n{3,}/g, '\n\n');
  const labelPattern = new RegExp(
    `(${PDF_FALLBACK_LABELS_V09.map((label) => escapeRegExpV09(label)).join('|')})\\s*`,
    'g'
  );
  ...
  return marked
    .split('\n')
    .map((line) => normalizeWhitespaceV09(line))
    .filter(Boolean)
    .join('\n');
};

const extractPdfLayoutV09 = async (fileName: string, bytes: Uint8Array): Promise<TemplateExtractPdfLayoutModel> => {
  const tempDir = await mkdtemp(join(tmpdir(), 'template-extract-pdf-v09-'));
  const tempFilePath = join(tempDir, fileName || 'upload.pdf');
  const tempScriptPath = join(tempDir, 'extract-pdf-layout-v09.swift');

  try {
    await writeFile(tempFilePath, bytes);
    await writeFile(tempScriptPath, PDF_LAYOUT_EXTRACT_SCRIPT_V09);
...
export const TemplateExtractSnapshotV09Service = {
  async extractPdfSource(fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> {
    const [layout, pageImages] = await Promise.all([
      extractPdfLayoutV09(fileName, bytes),
      TemplateExtractPdfRenderService.renderPageImages(fileName, bytes).catch(() => []),
    ]);
    ...
    const layoutWithImages = {
      ...layout,
      pages: layout.pages.map((page, index) => ({
        ...page,
        backgroundImageDataUrl: pageImages[index] || null,
      })),
    } as TemplateExtractPdfLayoutModel;

    const clonedHtml = TemplateExtractSnapshotV09Layout.buildCloneHtml(sourceTitle, rawText, layoutWithImages);
    ...
    return {
      sourceTitle,
      sourceKind: 'text',
      sourceContent: normalizePdfFallbackTextV09(rawText),
      originalFileName: fileName,
      originalMimeType: 'application/pdf',
    };
  },
