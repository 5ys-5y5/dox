...
const countSemanticCells = (html: string) => {
  let semanticCellCount = 0;

  for (const match of html.matchAll(/<(td|th)\b[^>]*>/gi)) {
    const attrs = parseAttributesFromTag(match[0]);
    const className = attrs.class || '';

    if (/\btemplate-clone__(pdf|work-order)-empty\b/.test(className)) {
      continue;
    }
...
export const TemplateExtractReplicaHtmlNormalizerService = {
  normalizeReplicaHtml(html: string): NormalizedReplicaHtml {
    ...
    const semanticCellCount = countSemanticCells(html);
  ...
  embedPipelineTrace(html: string, pipelineTrace: TemplateExtractPdfPipelineTrace) {
    return this.upsertRootDataAttributes(html, {
      ...
      'data-template-frame-policy': pipelineTrace.frameDiagnostics?.policy || null,
      'data-template-frame-source': pipelineTrace.frameDiagnostics?.source || null,
      'data-template-frame-segments': pipelineTrace.frameDiagnostics
        ? [
            `all${pipelineTrace.frameDiagnostics.segmentCount}`,
            `h${pipelineTrace.frameDiagnostics.horizontalSegmentCount}`,
            `v${pipelineTrace.frameDiagnostics.verticalSegmentCount}`,
          ].join('-')
        : null,
      'data-template-frame-mask-enabled': pipelineTrace.frameDiagnostics
        ? pipelineTrace.frameDiagnostics.maskVectorEnabled
          ? 'true'
          : 'false'
        : null,
      'data-template-frame-mask-pages': pipelineTrace.frameDiagnostics
        ? String(pipelineTrace.frameDiagnostics.maskVectorPageCount)
        : null,
      'data-template-frame-mask-min-length': pipelineTrace.frameDiagnostics?.maskRuleMinLength != null
        ? String(pipelineTrace.frameDiagnostics.maskRuleMinLength)
        : null,
      'data-template-frame-fragments': pipelineTrace.frameDiagnostics?.fragmentCount != null
        ? String(pipelineTrace.frameDiagnostics.fragmentCount)
        : null,
      'data-template-frame-giant-table-rejected': pipelineTrace.frameDiagnostics?.giantTableRejected != null
        ? pipelineTrace.frameDiagnostics.giantTableRejected
          ? 'true'
          : 'false'
        : null,
    });
  ...
  parsePipelineTraceFromHtml(html: string): TemplateExtractPdfPipelineTrace | null {
    ...
    return {
      ...
      frameDiagnostics:
        attrs['data-template-frame-policy'] && attrs['data-template-frame-source']
          ? {
              policy: attrs['data-template-frame-policy'] as NonNullable<
                TemplateExtractPdfPipelineTrace['frameDiagnostics']
              >['policy'],
              source: attrs['data-template-frame-source'] as NonNullable<
                TemplateExtractPdfPipelineTrace['frameDiagnostics']
              >['source'],
              segmentCount: Number(
                (attrs['data-template-frame-segments'] || '').match(/all(\d+)/)?.[1] || 0
              ),
              horizontalSegmentCount: Number(
                (attrs['data-template-frame-segments'] || '').match(/h(\d+)/)?.[1] || 0
              ),
              verticalSegmentCount: Number(
                (attrs['data-template-frame-segments'] || '').match(/v(\d+)/)?.[1] || 0
              ),
              maskVectorEnabled: toBoolean(attrs['data-template-frame-mask-enabled']),
              maskVectorPageCount: Number(attrs['data-template-frame-mask-pages'] || 0),
              maskRuleMinLength:
                attrs['data-template-frame-mask-min-length'] != null
                  ? toNumber(attrs['data-template-frame-mask-min-length'])
                  : null,
              fragmentCount:
                attrs['data-template-frame-fragments'] != null
                  ? toNumber(attrs['data-template-frame-fragments'])
                  : null,
              giantTableRejected:
                attrs['data-template-frame-giant-table-rejected'] != null
                  ? toBoolean(attrs['data-template-frame-giant-table-rejected'])
                  : null,
            }
          : null,
    };
