...
- 현재 특징:
  ...
  - position action, text size, text style는 각각 `workspace/panels/TemplatePositionActionOverlay.tsx`, `TemplatePositionBoxSizeOverlay.tsx`, `TemplatePositionTextStyleOverlay.tsx`로 분리되었다.
  - selection appearance overlay JSX는 `workspace/panels/TemplateSelectionAppearanceOverlay.tsx`로 이동했다.
  - 메인 컴포넌트 내부에는 대형 overlay render helper가 남아 있지 않고, orchestration과 DOM/editor 엔진 연결이 중심이다.
  - 총 LOC는 아직 원본 단일 파일보다 크다. 이번 완료 기준은 `분해 완료`이며, `총 LOC 절감`은 후속 중복 제거 단계다.
...
- `TemplateEditWorkspace` 내부에 대형 JSX render helper가 남지 않는다.
...
- 현재 진행 메모:
  ...
  - `src/components/template/workspace/panels/TemplatePositionActionOverlay.tsx`, `TemplatePositionBoxSizeOverlay.tsx`, `TemplatePositionTextStyleOverlay.tsx`로 position overlay를 기능별 파일로 추가 분리 완료
  - `src/components/template/workspace/panels/TemplateSelectionAppearanceOverlay.tsx`로 selection appearance overlay 이동 완료
  - 우측 캔버스 카드의 header/toolbar/legend 및 대형 overlay 조립은 메인 파일 밖으로 이동했다
- 상태: `완료`
...
- 현재 진행 메모:
  - `renderPositionActionOverlay`, `renderTextAutoSizeControls`, `renderTextPaddingControls`, `renderTextCanvasActionControls`, `renderSelectionAppearanceControls` 본체 제거 완료
  - 메인 파일 라인 수는 `30,403 -> 28,583`으로 감소
  - 브라우저 smoke는 이번 환경에서 다시 막혀 코드/번들 기준으로만 마감했다
- 상태: `완료`
...
| P7 | 완료 | canvas toolbar, 상단 header, persistence panel, position/text/style overlay shell 분리 완료 |
| P8 | 완료 | 문서/manifest 갱신, 대형 render helper 제거, 번들 검증 완료. 브라우저 smoke는 환경 차단 기록 |
...
다음 실행은 `분리 이후 중복 제거 / 총 LOC 절감` 또는 `localhost:3001`이 열린 환경에서 브라우저 smoke 재실행이 필요할 때만 진행한다.

## Run 2026-05-18 13:20:38

- phase: `P7 -> P8 stricter completion`
- purpose:
  - 메인 파일에 남아 있던 position/text/style overlay JSX를 `workspace/panels/**`로 이동
  - `TemplateEditWorkspace` 내부 대형 render helper 제거
- editable_files:
  - `src/components/template/TemplateEditWorkspace.tsx`
  - `src/components/template/workspace/types.ts`
  - `src/components/template/workspace/constants.ts`
  ...
  - `docs/tempsplit.md`
- backup_path:
  - `docs/applied/tempsplit-backups/2026-05-18_13-20-38`
- browser_checks_passed:
  - `blocked`: `localhost:3001` 미기동
  - `blocked`: sandbox에서 browser/devtools 연결 불가
- performance_notes:
  - `esbuild` bundle check 통과
  - 메인 파일 라인 수 `30,403 -> 28,583`
  - 메인 컴포넌트 내부 `render*` helper는 DOM-level helper만 남음
- issue_found:
  - 브라우저 smoke 재실행은 환경 차단으로 미수행
  - 총 모듈 LOC는 여전히 원본 단일 파일보다 큼
- fix_or_followup:
  - 실행 서버가 열리면 `/templates`, `/templates/edit` 상호작용 smoke 재실행
  - 후속 과제는 `총 LOC 절감` 목적의 중복 제거
- status: `done`

## Run 2026-05-18 13:57:55

- phase: `P7 follow-up`
- purpose:
  - `TemplatePositionSideOverlays.tsx`를 기능별 파일로 재분해
  - apply-status icon 중복 제거
- editable_files:
  - `src/components/template/workspace/panels/TemplatePositionSideOverlays.tsx`
  ...
  - `docs/tempsplit.md`
- backup_path:
  - `docs/applied/tempsplit-backups/2026-05-18_13-58-14`
- browser_checks_passed:
  - `blocked`: `localhost:3001` 미기동
- performance_notes:
  - `esbuild` bundle check 통과
  - `workspace/**` 총량 `12,756 -> 12,751`
  - position overlay 대형 파일 `936 lines`는 `534 / 252 / 137` lines로 분해
- issue_found:
  - 브라우저 smoke는 여전히 환경 차단
- fix_or_followup:
  - 실행 서버가 열리면 상호작용 smoke만 남음
- status: `done`
