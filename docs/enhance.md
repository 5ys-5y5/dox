...
이 문서와 후속 구현은 아래 순서를 절대 벗어나지 않는다.

1. `올바른 측정`
2. `측정에서 벗어나는 지점의 문제 정의`
3. `그 문제만 겨냥한 구현`
4. `구현 후 동일 측정으로 재검증`
5. 다시 `올바른 측정` 으로 돌아간다.

이 루프를 지키지 않는 수정은 금지한다.

특히 아래 원칙을 강제한다.

1. `휴리스틱 점수`, `row/cell 개수`, `token F1`, `topology summary` 는 디버깅 보조 자료일 뿐이며, **PDF와 HTML의 시각 유사도 측정으로 간주하지 않는다.**
2. 버전 승격, 기본값 교체, “개선되었다”는 판단은 **반드시 동일 조건에서 렌더된 PDF 결과와 HTML 결과를 비교한 측정** 에만 근거한다.
3. 사용자가 요구한 기준은 **“PDF 출력과 HTML 출력이 겹쳤을 때 1px 이내로 일치하는 영역 비율”** 이다. 이 기준과 직접 연결되지 않는 수치는 acceptance 근거가 될 수 없다.
4. 따라서 `offline quality` 는 앞으로도 있을 수 있지만, 그것은 **acceptance metric 이 아니라 anomaly locator / regression hint** 로만 쓴다.
5. “측정이 불완전한 상태에서 구현을 계속 밀어붙이는 것”은 금지한다. 먼저 측정이 맞는지부터 검증한다.
6. 어떤 구현이 v20보다 좋아 보인다고 주장하려면, 반드시 동일 샘플과 동일 측정 기준에서 v20 대비 개선 수치를 함께 남긴다.
7. 측정이 틀렸다면 구현보다 측정 수정이 우선이다.
8. 실제 픽셀 중첩률 측정이 붙기 전에는 `v21` 이 `v20` 보다 낫다는 이유로 후보 선택, 자동 승격, 기본값 교체를 하지 않는다.
9. 구조 진단값은 `selection hint` 로도 쓰지 않는다. 진단값으로 후보를 갈아끼우는 순간, 측정이 아닌 감으로 버전을 바꾸게 되므로 금지한다.
10. 로그, UI, HTML data attribute 어디에서도 구조 진단값을 `visual similarity`, `quality score`, `overall score` 같은 표현으로 포장하지 않는다.
11. 개선 판단은 반드시 `docs/*_template-extract-measurement_*.md` 에 기록된 실제 `시각 유사도 측정 결과` 값을 터미널에서 직접 읽고 내린다.
12. `sourceInkPixelCount` 대비 `replicaInkPixelCount` 가 커졌는데 `overlapInkPixelCount` 와 `overallScore` 가 내려가면, 그 변경은 `frame/text overdraw regression` 으로 간주하고 즉시 폐기한다.

## 현재 측정 체계에 대한 판정

2026-04-18 기준 현재 `src/services/templateExtractReplicaOfflineQualityService.ts` 는 **시각 유사도 측정기라고 볼 수 없다.**

이유는 아래와 같다.

1. PDF와 HTML을 같은 렌더 조건에서 이미지로 출력해 비교하지 않는다.
2. 대신 `token F1`, `row/cell count`, `topology summary`, `page width/height` 같은 구조 휴리스틱을 합산한다.
3. 따라서 이 값은 “문서 구조가 얼추 비슷한가”를 보는 참고치일 뿐, 사용자가 요구한 `1px 이내 출력 일치 비율`과 직접 연결되지 않는다.
4. 이 휴리스틱이 높은데도 실제 시각 유사도가 낮을 수 있으며, 이미 그런 사례가 발생했다.
5. 그러므로 현재 `offline quality` 는 **diagnostic only** 로만 사용하고, `v20` 대비 `v21` 우수성의 근거로 사용하지 않는다.

### 2026-04-19 추가 판정

실제 사용자 비교 결과, `offlineMetrics` 가 `0.95 ~ 1.0`에 가깝게 보여도 원문 PDF와 HTML을 병치하면 `50%`도 맞지 않는 사례가 확인됐다.

이 사실로부터 아래를 확정한다.

1. 현재 `offlineMetrics` 는 이름에 `score` 가 포함되어 있어도 시각 유사도와 무관하다.
2. `mismatchPixelRatio` 역시 실제 픽셀 차이 측정이 아니라 dimension 중심 진단값이므로 acceptance 용어로 쓰면 안 된다.
3. 따라서 `v21` 에서 이 값을 사용한 후보 선택 로직은 폐기한다.
4. 실제 픽셀 중첩률 측정기가 붙기 전까지 `v21` 은 `v20` HTML 생성 경로를 그대로 재사용하는 bridge 로만 유지한다.
5. 이후 구현은 반드시 `실제 측정기 구현 -> v20/v21 동일 샘플 비교 -> 문제 지점 식별 -> 그 지점만 수정` 순서를 따른다.

### 2026-04-19 브라우저 측정 경로

이번 턴부터 1차 실측 경로는 아래처럼 정의한다.

1. 서버는 `TemplateExtractPdfRenderService` 로 원본 PDF 페이지를 PNG data URL 로 렌더한다.
2. 클라이언트 브라우저는 추출된 output HTML 을 숨김 iframe 에 로드한다.
3. 각 페이지 노드를 `SVG foreignObject -> canvas` 로 캡처해 브라우저 실제 렌더 결과를 얻는다.
4. PDF PNG 와 HTML canvas 를 같은 크기로 맞춘 뒤, **비백색(ink) 픽셀 union 대비 1px 허용 오차 overlap 비율** 을 계산한다.
5. 이 값만 `visual similarity` 로 표기한다.
6. `offline quality` 는 계속 구조 진단용 보조 정보로만 남긴다.

### 2026-04-20 v26 구현 원칙

`v26` 부터는 외부 구현 방향도 아래 공식 자료 기준으로 고정한다.

1. Mozilla `PDF.js` 의 공식 문서는 프로젝트를 `core / display / viewer` 로 나누며, text layer 는 display/viewer 측 보조 레이어이지 최종 정답 레이아웃이 아님을 전제한다. 실제 이슈 기록에도 text layer misalignment 사례가 반복된다.
2. `pdf2htmlEX` 의 공식 소개는 목표를 `Native HTML text with precise font and location` 으로 밝힌다. 즉 목표는 점묘/프린트가 아니라 **정밀 위치의 native HTML text** 다.
3. Apple `PDFKit` 공식 문서는 `PDFPage.numberOfCharacters`, `PDFPage.selection(for: NSRange)`, `PDFPage.characterBounds(at:)`, `PDFSelection.bounds(for:)`, `PDFSelection.selectionsByLine()` 를 제공한다. 즉 현재 코드베이스에서는 PDFKit page-space 좌표와 page-global character index 를 authoritative source 로 삼는 것이 맞다.

따라서 `v26` visible output 은 아래 제약을 반드시 만족한다.

1. 가시 레이어는 `HTML div/span` 또는 필요시 최소한의 `SVG line/rect` 만 사용한다.
2. `mask_vector`, `dot cloud`, `bitmap print`, `background image`, `pointillism` 류 출력은 금지한다.
3. 텍스트는 문자 박스를 그대로 뿌리는 것보다 **run/word 단위 native HTML text** 를 우선한다.
4. 선은 page-space 좌표의 rule segment 를 그대로 `div` 또는 `rect` 로 옮긴다.
5. 위치 데이터는 추정 휴리스틱보다 `PDFKit page-space bounds` 를 우선 사용한다.
6. 자간 보정이 필요하면 `scale transform` 보다 `letter-spacing` 같은 native text metric 보정을 우선 사용한다.
7. page markup 은 반드시 `frame layer` 와 `text layer` 를 최상위 sibling 으로 분리한다. 검토자는 프레임 정합성과 텍스트 정합성을 각각 따로 눈으로 확인할 수 있어야 한다.
8. line selection 내부의 문자 추출은 range 중첩을 허용하지 않는다. 같은 page-global character index 가 두 번 들어오면 즉시 버린다.
9. browser rounding 오차를 줄이기 위해, 가시 HTML 은 필요시 내부 좌표계를 확대(`precision stage`)한 뒤 CSS `scale()` 로 축소할 수 있다. 이 단계는 editable HTML 을 유지한 상태에서만 허용한다.
10. `v27`부터 frame layer는 text bbox 폭 추정으로 선 길이를 정하지 않는다. `columnEdges`, `row top/bottom`, `rule row/column`을 우선 축으로 삼아 전역 그리드 선을 만든다.
...
### 2026-04-17 구현 메모 `ENHANCE-10`

1. `full-page image`, `bitmap background`, `pdf raster overlay`, `image-first hybrid` 는 최종 산출물 경로에서 전부 금지한다.
2. 금지 사유는 단순 시각 유사성 문제가 아니라, 후속 field tagging / bulk management / semantic ownership 을 깨뜨리기 때문이다.
3. `v21` 은 `v20` builder 철학을 유지한 `html-only bridge` 로 한정한다.
4. `v21` 은 `legacy/topology/generic/rule` 중 실제로 선, 셀, 텍스트를 HTML 로 만든 결과만 허용한다.
5. `작업지시서_사일동 주상복합.pdf`, `사업자등록증.pdf` 는 hybrid 회귀가 아니라 `html-only regression` 우선 샘플로 고정한다.
6. `95점`, `99점` 목표는 계속 유지하되, 평가 대상은 반드시 `HTML element + text + border + line` 으로 구성된 출력만 인정한다.

### 폐기된 접근

1. `TemplateExtractPdfHybridCloneService` 기반의 background image 삽입 방식
2. `TemplateExtractReplicaPixelSimilarityService` 를 hybrid bitmap 통과 게이트로 사용하는 방식
3. `hybrid bitmap/text-layer` 가 통과하면 다른 builder 보다 우선 채택하는 방식

### 2026-04-17 구현 메모 `ENHANCE-11`

1. `v21` 은 `html-only bridge` 를 유지하되, 후보 선택을 다시 단순 고정 builder 에 맡기지 않는다.
2. 디지털 문서는 `legacy`, `generic`, `work_order_topology` 후보를 모두 생성하고 offline quality 로 최고점을 선택한다.
3. 스캔 문서는 `generic_layout_scanned`, `certificate_family_scanned`, `work_order_family_topology_scanned`, `work_order_family_frame_scanned` 를 비교한다.
4. 모든 후보는 평가 전에 실제 page width/height 를 주입해서 `pageContractScore` 손실을 제거한다.
5. 2026-04-17 기준 offline 회귀 결과:
   - `작업지시서_사일동 주상복합.pdf`: `0.7271 -> 0.8769`, 선택 builder `generic_layout`
   - `사업자등록증.pdf`: `0.4993 -> 0.8714`, 선택 builder `generic_layout_scanned`
6. 위 개선은 모두 이미지 없이 `HTML element + text + border + line` 만으로 얻은 결과다.

### 2026-04-17 구현 메모 `ENHANCE-12`

1. `pdf-html-grid` 는 실제 페이지와 무관한 wrapper shadow, border, padding 을 제거하고 page-sized white sheet 에 가깝게 조정한다.
2. browserless quality 에서는 `template-clone__pdf-empty`, `template-clone__work-order-empty` 같은 helper cell 을 source cell 수로 계산하지 않는다.
3. text-layer 기반 topology summary 에서 `horizontal/vertical segment = 0` 인 경우, row/table 기반 HTML 은 `segment missing` 으로 감점하지 않는다.
4. quality gate 는 `1.0 exact equality` 대신 작은 epsilon 을 허용한다.
5. fresh offline 결과:
...
6. 따라서 현재 남은 우선순위는 디지털 work order 가 아니라 스캔형 문서의 OCR/token anchor 정합이다.
...
## 2026-04-20 V24 Anti-Pattern Record

- 금지 경로: `work_order_family_mask_vector_digital`
- 금지 사유: PDF 렌더 잉크를 사각형/점 집합으로 직접 옮긴 가시 레이어는 `1px` 중첩률은 높아질 수 있어도, 사용자가 수정 가능한 텍스트와 직선 기반 HTML이 아니다.
- 허용 기준:
  - visible layer 는 수정 가능한 텍스트 노드와 직선/사각형 선분으로만 구성한다.
  - intermediate 단계에서 bitmap/mask/vectorization 을 써도 되지만, 최종 출력에서 glyph pixel/ink pixel 을 직접 rect 로 찍는 방식은 금지한다.
  - `1px` 중첩률이 높아도 editable HTML 원칙을 위반하면 즉시 폐기한다.
