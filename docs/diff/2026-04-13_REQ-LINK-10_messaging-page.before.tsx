...
export default function MessagingPage() {
  ...
  const handleSaveDocumentSenderMappings = async () => {
    setLoading(true);
    setMessage(null);
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.message || '문서별 기본 발신번호 저장 실패');
      }
      ...
      await loadMessagingData();
      setMessage('문서별 기본 발신번호를 저장했습니다.');
    } catch (error) {
      setMessage(error instanceof Error ? error.message : '문서별 기본 발신번호 저장 실패');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      ...
      <Card className="border-slate-200">
        <CardHeader>
          <CardTitle>문서별 기본 발신번호</CardTitle>
          <CardDescription>
            문서마다 다른 기본 발신번호를 지정할 수 있습니다. `/request-links`에서는 이 값이 기본 적용되지만,
            사용자가 체크를 해제해 전역 기본 발신번호로 되돌릴 수 있습니다.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-800">현장 ID</label>
            <Input value={documentSiteId} onChange={(event) => setDocumentSiteId(event.target.value)} />
          </div>
                ...
                <div key={item.document.id} className="rounded-md border border-slate-200 bg-white px-3 py-3">
                  ...
                  <div className="mt-3">
                    <select
                      ...
                      className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm"
                    >
                      <option value="">문서 기본 발신번호 없음</option>
                      {senders.map((sender) => (
          ...
          <Button variant="outline" onClick={handleSaveDocumentSenderMappings} disabled={loading || documents.length === 0}>
            문서별 기본 발신번호 저장
          </Button>
        </CardContent>
      </Card>
