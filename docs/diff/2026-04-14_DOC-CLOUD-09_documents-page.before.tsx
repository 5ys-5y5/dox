...
export default function DocumentsPage() {
  ...
  React.useEffect(() => {
    ...
    if (!previewRoot || !latestVersion || !interactivePreviewHtml) {
      setPreviewFields([]);
      return;
    }
  ...
  }, [interactivePreviewHtml, selectedDocumentDetail?.latestVersion]);
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">DOC-CLOUD-08</Badge>
          <h1 className="text-3xl font-semibold text-slate-950">서류 클라우드 관리</h1>
          <p className="max-w-3xl text-sm text-slate-600">
            문서 한 건을 고른 뒤 문서를 보면서 바로 전송과 출력 작업을 끝내는 화면입니다. 개발자용 등록·편집 기능은 아래
            `고급 편집` 안으로 숨기고, 기본 화면은 선택, 상태 확인, 문서 클릭 기반 전송 설정, 출력 실행에만 집중합니다.
          </p>
        </div>
        ...
        <div className="space-y-6">
          <Card className="border-slate-200">
            <CardHeader>
              ...
              <CardDescription>이 문서에서 바로 끝낼 수 있는 작업만 먼저 보여줍니다.</CardDescription>
            </CardHeader>
                ...
                <>
                  <div className="rounded-xl border border-slate-200 p-4">
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm font-medium text-slate-900">요청 링크 만들고 이메일 보내기</p>
                        <p className="mt-1 text-xs text-slate-500">
                          왼쪽 문서에서 요청할 항목을 클릭하고, 오른쪽에서 받는 사람만 확인하면 이메일까지 한 번에 처리합니다.
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
                        <div className="space-y-3">
                          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                            문서 안에서 수정 요청할 값을 직접 클릭하세요. 다시 클릭하면 선택이 해제됩니다.
                          </div>
                          {interactivePreviewHtml ? (
                            <div
                              ...
                              className="max-h-[420px] overflow-auto rounded-lg border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-700 [&_h1]:mb-3 [&_h1]:text-xl [&_h1]:font-semibold [&_h2]:mb-2 [&_h2]:text-lg [&_h2]:font-semibold [&_p]:mb-2 [&_table]:mb-3 [&_table]:w-full [&_table]:border-collapse [&_th]:border [&_th]:border-slate-200 [&_th]:bg-slate-50 [&_th]:px-2 [&_th]:py-1 [&_th]:text-left [&_td]:border [&_td]:border-slate-200 [&_td]:px-2 [&_td]:py-1 [&_li]:mb-1"
                              dangerouslySetInnerHTML={{ __html: interactivePreviewHtml }}
                            />
                          ) : (
                            <div className="rounded-lg border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500">
                              최신 문서 본문이 없어 클릭할 항목을 표시할 수 없습니다.
                            </div>
                          )}
                        </div>
                  ...
                  <div className="rounded-xl border border-slate-200 p-4">
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm font-medium text-slate-900">출력본 바로 만들기</p>
                        <p className="mt-1 text-xs text-slate-500">
                          PDF와 DOCX는 바로 내려받고, HWP는 요청 상태만 먼저 남깁니다.
                        </p>
                      </div>
                  ...
                  <div className="rounded-xl border border-slate-200 p-4">
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm font-medium text-slate-900">사진 상태 확인</p>
                        <p className="mt-1 text-xs text-slate-500">
                          사진 증빙 상태만 여기서 보고, 업로드와 라벨링은 사진 화면에서 이어집니다.
                        </p>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-sm text-slate-700">
                          요구 {selectedDocumentDetail.photoEvidence.requirementCount}개 / 충족 {selectedDocumentDetail.photoEvidence.coveredCount}개 / 누락 {selectedDocumentDetail.photoEvidence.missingCount}개
                        </span>
                      </div>
