...
import type { SmsRecipientRecordDto, SmsSenderRecordDto, SmsSendResult, SmsSettingsDto } from '../../lib/messagingDtos';
...
export default function RequestLinksPage() {
  ...
  const handleSendSms = async () => {
    ...
    try {
      ...
      const messageText = `${prefixText}${recipient?.recipientName || '수신자'}님, 요청 링크를 확인하세요. ${requestLinkUrl}`;
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <Input
                  ...
                  onChange={(event) => setSmsDefaultSenderPhone(event.target.value)}
                  placeholder="기본 발신번호"
                />
                <Input
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                {smsSettings?.defaultSenderPhoneNumber ? (
                  <p className="text-xs text-slate-500">기본 발신번호: {smsSettings.defaultSenderPhoneNumber}</p>
                ) : null}
              </div>
              <div className="space-y-2">
