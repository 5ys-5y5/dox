...
- PostgREST 가 `signing`, `documents`, `sites`, `templates` 스키마를 읽을 수 있어야 `schema(...)` 기반 코드가 동작한다.
- UI에서 바로 반영되지 않으면 `ALTER ROLE authenticator SET pgrst.db_schemas = 'public, signing, documents, sites, templates';` 와 `NOTIFY pgrst, 'reload config';`, `NOTIFY pgrst, 'reload schema';` 를 사용한다.
