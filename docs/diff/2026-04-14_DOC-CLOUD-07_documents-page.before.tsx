    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">DOC-CLOUD-06</Badge>
          ...
          <p className="max-w-3xl text-sm text-slate-600">
            ...
            상세 조회, 버전 추가, 최신본 목록 확인과 한 문서 기준 운영 상태 통합 확인, 다음 작업 실행까지 다룹니다.
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-slate-200">
            <CardHeader>
              ...
              <CardDescription>
                문서를 선택해 최신 버전과 전체 이력을 조회하고, 같은 문서에 새 버전을 추가합니다.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {selectedDocumentDetail ? (
                <div className="space-y-4 rounded-xl border border-slate-200 p-4">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-slate-800">다음 작업 바로가기</p>
                    <div className="flex flex-wrap gap-2">
                      <Button asChild size="sm">
                        <Link
                          href={`/request-links?siteId=${encodeURIComponent(selectedDocumentDetail.document.siteId)}&documentId=${encodeURIComponent(selectedDocumentDetail.document.id)}`}
                        >
                          요청 링크 보내기
                        </Link>
                      </Button>
                      <Button asChild size="sm" variant="outline">
                        <Link
                          href={`/exports?siteId=${encodeURIComponent(selectedDocumentDetail.document.siteId)}&documentId=${encodeURIComponent(selectedDocumentDetail.document.id)}`}
                        >
                          출력본 만들기
                        </Link>
                      </Button>
                      ...
                      {selectedDocumentDetail.document.templateId ? (
                        <Button asChild size="sm" variant="outline">
                          <Link
                            href={`/templates?templateId=${encodeURIComponent(selectedDocumentDetail.document.templateId)}`}
                          >
                            템플릿 보기
                          </Link>
                        </Button>
                    ...
                    {!selectedDocumentDetail.document.templateId ? (
                      <p className="text-xs text-slate-500">연결된 템플릿이 없어서 템플릿 보기 버튼은 숨깁니다.</p>
                    ) : null}
                  </div>
