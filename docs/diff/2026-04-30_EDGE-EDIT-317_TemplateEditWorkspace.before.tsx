  ...
  const handlePreviewPointerMove = React.useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (resizeState && event.pointerId === resizeState.pointerId) {
      ...
      if (resizeState.edgeResizeTargets && resizeState.edgeResizeTargets.length > 0) {
        ...
        const liveResizeSnapshot =
          liveResizeRoot && resizeState.mutationEdgeIds?.length
            ? buildLiveEdgeTopologySnapshot(liveResizeRoot)
            : null;

        if (liveResizeRoot && liveResizeSnapshot && resizeState.mutationEdgeIds?.length) {
          const liveResizeTargets = resolveLiveEdgeResizeTargets(
            liveResizeRoot,
            liveResizeSnapshot,
          ...
          if (liveResizeTargets.length > 0) {
            resizeState.edgeResizeTargets = liveResizeTargets;
          }
        }
