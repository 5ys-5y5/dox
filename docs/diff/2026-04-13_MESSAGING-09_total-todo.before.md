...
- 날짜: 2026-04-13
  ...
  - 수정 내용:
    - `src/lib/messagingDtos.ts` 에 `SmsSyncInput`, `SmsSyncResult` 와 `status_synced` 이벤트 타입을 추가했다.
    ...
    - `src/services/messagingService.ts` 에 `syncSmsDispatchStatus()` 를 추가했다. dispatch target 의 `provider_message_id` 를 기준으로 상태를 다시 읽고, target/dispatch 집계를 갱신한 뒤 `status_synced` 이벤트를 저장한다.
