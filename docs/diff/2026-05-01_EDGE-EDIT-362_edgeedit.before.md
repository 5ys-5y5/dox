...
22. `status-history-1:left` 의 위치를 변경하면 `status-history-1:right` 가 right-chain(`band-5-cell-6:right` ~ `band-18-cell-2:right`)에서 미세하게 밀려나는 문제
   - 원인: left/right width drag 보정은 active edge cluster 기준으로만 수행되고 있었고, 같은 node의 passive opposite vertical edge는 baseline 으로 다시 고정하지 않았다. 그래서 `status-history-1:left` 를 움직인 뒤에도 `status-history-1:right` 는 선택 chain 밖의 passive edge라서 subpixel drift 가 남을 수 있었다.
   - 해결: width drag 후 `normalizePassiveOppositeVerticalEdges(...)` 를 추가하여, 움직이지 않아야 하는 같은 node의 반대편 vertical edge를 drag 시작 baseline 으로 다시 맞추도록 했다.
   - 결과: `status-history-1:left` drag 후에도 `status-history-1:right` 는 right-chain 과 같은 line coordinate 를 유지한다.
...
3. 재현 동작
   - 오른쪽으로 drag 하여 `status-history-1:left` 또는 `band-4-cell-2:right` line 근처로 접근
4. 기존 잘못된 결과
   - `band-5-cell-2:right / band-5-cell-3:left` exact physical peer 쌍이 drag 중간 또는 snap 직전에 분리됨
   - 이후 `band-5-cell-3:left` 가 `band-5-cell-2` 최소 너비 지점까지 강제로 이동하는 경로가 남음

#### 이번 턴 원인

1. `band-5-cell-3:left` 같은 exact physical boundary peer-only width drag 에도 multi-row vertical correction pass 가 동일하게 적용되고 있었다.
2. 이 correction pass 는 `status-history-1:left` 같은 다른 row vertical line 을 기준으로 active cluster 를 재정렬하는 목적에는 필요하지만, local peer-only drag 에서는 오히려 `band-5-cell-2:right / band-5-cell-3:left` 쌍을 mid-drag 에 갈라놓는 과보정이 된다.
3. pointerup 뒤에도 같은 경로의 finalize/normalize 를 다시 태워 local exact-boundary 결과를 재차 흔들 수 있었다.

#### 수정

1. `isSimpleExactPhysicalBoundaryVerticalDrag(...)` 를 추가했다.
   - 조건:
     - 가로(width) drag 여야 한다.
     - `selected_edge_clicked` 가 1개여야 한다.
     - `selected_edge_auto_multi` 가 없어야 한다.
     - 관련 edge 전체가 vertical 이어야 한다.
     - 관련 edge 전체가 `groupExactPhysicalBoundaryEdgeIds(...)` 기준 1개 exact physical boundary group 이어야 한다.
2. 위 조건에 해당하는 drag 는 pointermove 중 `stabilizeLiveVerticalEdgeTargetsToAppliedDelta / realignLiveVerticalEdgeTargets / live autosnap correction / normalizeLiveVerticalPhysicalPeersToDragDirection` 경로를 건너뛴다.
3. pointerup 뒤에도 같은 simple exact-boundary drag 는 `finalizeLiveVerticalEdgeTargets(...)` 대신 제한된 `normalizeLiveVerticalPhysicalPeers(...)` 만 수행한다.

#### 기대 효과

1. `band-5-cell-3:left` 같은 local peer-only drag 는 `band-5-cell-2:right` 와 끝까지 같은 선을 유지한다.
2. `status-history-1:left` 같은 multi-row chain 은 기존 baseline-lock / autosnap correction 경로를 그대로 유지한다.
3. 이미 해결한 아래 문제를 다시 열지 않는다.
...
1. `CHK-RIGHT-LINE-005`
   - `band-5-cell-3:left` local peer-only drag 에서 `band-5-cell-2:right` 와 분리되지 않아야 한다.
   - 상태: 구현 반영
2. `CHK-RIGHT-LINE-006`
   - `status-history-1:left` multi-row chain drag 는 기존 collapse fix/autosnap fix 가 유지되어야 한다.
   - 상태: 구현 반영

#### 이번 턴 파일/백업

1. 백업
   - `docs/diff/2026-05-01_EDGE-EDIT-356_TemplateEditWorkspace.before.tsx`
   - `docs/diff/2026-05-01_EDGE-EDIT-357_edgeedit.before.md`
2. 수정 파일
   - `src/components/template/TemplateEditWorkspace.tsx`
   - `docs/edgeedit.md`
