...
const extractPdfSourceV25 = async (fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> => {
  ...
  if (layoutDetection.sourceMode === 'digital' && layoutDetection.documentFamily === 'work_order') {
    ...
    const measuredFitOptions = {
      cloneId: 'pdf-rule-text-font-v25',
      usePdfFontMetrics: true,
      textLayoutMode: 'clustered_line_box' as const,
      embeddedFonts,
    };
    ...
    const measuredWidths =
      measurementSpecs.length > 0
        ? await TemplateExtractHtmlRenderService.measureTextRuns(
        ...
        : {};
    const measuredLetterSpacingByLine = Object.fromEntries(
      measurementSpecs
        .map((spec) => {
          const measuredWidth = measuredWidths[spec.id];
          const glyphCount = Math.max(1, Array.from(spec.text).length - 1);

          if (!measuredWidth || glyphCount < 1) {
            return null;
          }

          const letterSpacing = Number(((spec.targetWidth - measuredWidth) / glyphCount).toFixed(4));

          if (!Number.isFinite(letterSpacing) || Math.abs(letterSpacing) > 8) {
            return null;
          }

          return [spec.id, letterSpacing] as const;
        })
    ...
    if (fontAwareHtml) {
      ...
      const pipelineTrace = buildPipelineTrace(
        ...
        layoutDetection,
        topology,
        'work_order_family_rule_text_font_digital'
      );
      const qualityReport = buildOfflineDiagnosticReport(layout.rawText, layout, topology, html);
