...
- Supabase Dashboard > Project Settings > API > Exposed schemas 에 `signing` 을 추가해야 서비스 코드가 동작한다.
- Supabase Dashboard / PostgREST 설정에 `documents` 도 포함해야 `schema('documents')` 코드가 동작한다.
...
- API 계약:
  - `POST /api/sites`
    - 입력 DTO: `siteName`, `tradeKeys[]`, `openDate`
    - 출력 DTO: `siteId`, `generatedChecklistCount`
  - `POST /api/sites/checklist/rebuild`
    - 입력 DTO: `siteId`
...
- API 계약:
  - `POST /api/templates`
    - 입력 DTO: `templateName`, `sourceDocument`, `layoutResizePolicy`
    - 출력 DTO: `templateId`, `draftHtml`, `fieldCount`
  - `POST /api/templates/:templateId/fields`
    - 입력 DTO: `fields[]`
    - 출력 DTO: `templateId`, `savedFieldCount`
  - `GET /api/templates/:templateId`
    - 출력 DTO: `htmlLayout`, `fields[]`, `labelMap`
- 데이터 소유권:
  - 템플릿 정의
  - 템플릿 필드 스키마
  - 라벨 매핑
- 의존 서비스:
  - 서류 클라우드 관리에서 원본 문서 조회를 선택적으로 사용
- 분리 배포 시 필요한 최소 조건:
  - 템플릿 저장소
  - HTML 레이아웃 저장소
  - 필드 스키마 편집 API
- 체크리스트:
  - [ ] `TPL-REG-01` 원본 문서를 HTML 레이아웃 초안으로 변환한다.
...
- API 계약:
  - `POST /api/templates/extract`
    - 입력 DTO: `sourceDocument`, `similarTemplateIds?`
    - 출력 DTO: `draftId`, `confidenceSummary`, `fields[]`
  - `POST /api/templates/extract/:draftId/approve`
    - 입력 DTO: `reviewedFields[]`, `templateName`
    - 출력 DTO: `templateId`, `approvedFieldCount`
- 데이터 소유권:
  - 추출 초안
  - 필드별 confidence
  - 사용자 검토 결과
...
- API 계약:
  - `POST /api/photos`
    - 입력 DTO: `siteId`, `photoFile`, `capturedAt`
    - 출력 DTO: `photoId`, `status`
  - `POST /api/photos/:photoId/labels`
    - 입력 DTO: `labels[]`
    - 출력 DTO: `savedLabels[]`
  - `POST /api/photos/:photoId/suggest-labels`
    - 입력 DTO: `photoId`
    - 출력 DTO: `suggestedLabels[]`, `confidence[]`
- 데이터 소유권:
  - 현장 사진 메타데이터
  - 사진 라벨
  - 라벨 추천 결과
