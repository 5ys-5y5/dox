...
const romanizeHangulSyllable = (character: string) => {
  ...
  return `${HANGUL_INITIAL_ROMAJA[initialIndex] || ''}${HANGUL_MEDIAL_ROMAJA[medialIndex] || ''}${
    HANGUL_FINAL_ROMAJA[finalIndex] || ''
  }`;
};
...
const buildRequirementLabelKey = (labelName: string, providedLabelKey?: string | null) => {
  const provided = String(providedLabelKey || '').trim();
  const source = provided || labelName;
  const romanized = Array.from(source).map(romanizeHangulSyllable).join('');
  const normalized = romanized
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/_+/g, '_')
    .replace(/^_+|_+$/g, '');
  ...
  return `photo_${buildStableSuffix(labelName || source || 'requirement')}`;
};
...
const normalizeRequirements = (requirements: PhotoLabelRequirementInput[]) => {
  ...
  return requirements
    .map((requirement) => {
      ...
      const labelKey = buildRequirementLabelKey(labelName, requirement.labelKey);
      ...
      if (!labelName) {
        throw new Error('사진 요구 라벨 저장 실패: 사진 태그 이름은 필수입니다.');
      }
