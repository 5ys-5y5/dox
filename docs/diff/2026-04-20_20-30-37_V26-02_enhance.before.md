...
3. Apple `PDFKit` 공식 문서는 `PDFPage.characterBounds(at:)`, `PDFSelection.bounds(for:)`, `PDFSelection.selectionsByLine()` 를 제공한다. 즉 현재 코드베이스에서는 PDFKit page-space 좌표를 authoritative source 로 삼는 것이 맞다.
