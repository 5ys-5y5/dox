...
6. 텍스트 맞춤 확장은 줄임이나 주변 상자의 축소를 하지 않는다. 선택된 대상의 한 축만 커질 수 있으며, 다른 상자의 높이/너비를 줄여 공간을 만들지 않는다.
7. 텍스트 맞춤 확장에 영향을 받는 상자가 없다면 선택된 하나의 상자만 변경한다.
8. 예시 기준: `status-history-1`의 높이가 달라질 때, `크기 및 위치` 탭의 edge resize처럼 `band-5-cell-1`, `band-5-cell-2`, `band-5-cell-3`, `band-5-cell-4`, `band-5-cell-5`, `band-5-cell-6`의 높이를 줄이면 안 된다.
9. `크기 및 위치` 탭의 캔버스 액션 버튼은 선택 상태에 따라 출력 여부가 달라져야 한다. 비활성 버튼을 계속 보여주는 것이 아니라 조건에 맞는 버튼만 출력한다.
10. `그룹 만들기`: 복수의 상자 또는 그룹이 선택된 경우 출력한다.
11. `그룹 해제`: 단일 또는 복수의 그룹이 선택에 포함된 경우 출력한다. 선택에 상자가 함께 있어도 그룹만 해제한다.
12. `그룹에서 제외`: 단일 또는 복수의 그룹이 선택된 경우 출력한다. 클릭하면 각 그룹에서 제외할 상자 또는 하위 그룹을 선택하는 모드로 들어간다. 이 모드에서 선택된 상자나 그룹은 상위 그룹에서 해제된다.
13. `그룹에서 제외` 모드는 `Esc`, `x`, `q`, 또는 화면의 `x` 버튼으로 종료할 수 있어야 한다. 종료해도 모드 진입 전 선택 상태는 유지한다.
14. `그룹에 포함`: 단일 또는 복수의 상자나 그룹이 선택된 경우 출력한다. 상자와 그룹이 섞여 선택되어도 허용한다. 클릭하면 포함시킬 대상 그룹을 선택하는 모드로 들어간다.
15. `그룹에 포함` 모드는 `Esc`, `x`, `q`, 또는 화면의 `x` 버튼으로 종료할 수 있어야 한다. 종료해도 모드 진입 전 선택 상태는 유지한다.
16. `상자 생성`: 어떤 상자나 그룹도 선택되지 않은 상태에서만 출력한다.
17. `간격 설정`: 선택이 없는 상태와 선택이 있는 모든 상태에서 항상 출력한다.
...
- `applySelectionStylePatch`는 `width` 또는 `height` patch가 있으면 `applyRelativeAnchoredFrameRectsInRoot(root)`를 호출한다. 텍스트 탭 자동 확장은 이 경로를 그대로 쓰면 주변 relative/edge 관계가 재계산되어 다른 상자가 줄어들 수 있으므로 별도 경로가 필요하다.
...
- 텍스트 자동 확장은 선택 상자 크기 변경만 다루며, 위치 탭 edge resize 알고리즘을 재사용해 주변 상자를 줄이는 방식은 금지한다.
...
- 주변 상자의 크기 축소나 간격 재배치는 수행하지 않는다.
- `status-history-1` 같은 단일 상자에 적용할 때 주변 `band-5-cell-*` 높이를 줄이지 않는 결과를 반환한다.
...
- `applyRelativeAnchoredFrameRectsInRoot`와 직접 결합하지 않는다.
...
- 이 자동 확장 경로에서는 `applyRelativeAnchoredFrameRectsInRoot(root)`를 호출하지 않는다. 필요할 경우 선택 상자 자신의 stored rect/style만 갱신한다.
- 상대 위치 anchor 자체를 깨지 않기 위해, 상자 크기 값만 수정하고 주변 peer normalization은 호출하지 않는다.
...
- 변경 대상이 `status-history-1` 하나뿐인 경우 다른 상자의 width/height는 바뀌지 않아야 한다.
...
7. `UIUPDATE-TEXT-AUTOSIZE-04`: 단일 상자 독립 확장 경로를 구현하고 주변 상자 축소를 금지한다.
...
| `UIUPDATE-TEXT-AUTOSIZE-03` | 선택 상자만 현재 크기 이상으로 확장하고 주변 상자를 줄이지 않는 적용 경로를 구현한다. | `TemplateEditWorkspace.before.tsx` | 대기 |
| `UIUPDATE-TEXT-AUTOSIZE-04` | `status-history-1` 확장 시 `band-5-cell-1..6`이 줄어들지 않음을 검증한다. | 테스트 기록 | 대기 |
...
8. 실행 전후 `status-history-1`의 height와 `band-5-cell-1..6`의 height를 비교한다. `status-history-1`은 커질 수 있고, `band-5-cell-1..6`은 작아지면 실패다.
