...
2. sandbox 에서 `next dev` 가 `listen EPERM` 으로 막혀 `localhost:3001` 실페이지를 직접 띄울 수는 없었다
3. 대신 `file:///tmp/selected-edge-harness/index.html` 하니스에서 실제 `TemplateEditWorkspace` 를 구동했다
4. `band-3-cell-2:left` 1회 클릭 후 drag 시 `band-3-cell-2` 와 `band-4-cell-2` 가 각각 `24px` 이동했고 `band-10-cell-2` 는 `0px` 변화였다
5. 같은 edge 2회 클릭 후 drag 시 `band-3-cell-2` 만 `24px` 이동했고 `band-10-cell-2` 는 `0px` 변화였다
6. `band-3-cell-1:right` drag 시 `band-3` 내부 경계만 `24px` 이동했고 `band-10-cell-2` 는 `0px` 변화였다

### 13.3 대체 검증 경로

1. 코드 baseline 을 `d0a845...` 로 복원
2. 구조 분석 기반 설계 갱신
3. 후속 구현 후 파일 단위 타입체크, 서비스 번들, synthetic runtime 테스트 수행
4. `chrome-devtools` 에서 실제 click + pointer drag 를 직접 실행해 selection mode 와 rect delta 를 확인
