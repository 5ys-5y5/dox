...
const buildPipelineTrace = (
  engineVersion: Extract<TemplateExtractEngineVersion, '19' | '20' | '21' | '22'>,
  detection: TemplateExtractPdfFamilyDetectionResult,
...
const inferV22CloneBuilder = (detection: TemplateExtractPdfFamilyDetectionResult) => {
  if (detection.documentFamily === 'work_order') {
    return `work_order_family_positioned_${detection.sourceMode}`;
  }

  if (detection.documentFamily === 'certificate') {
    return `certificate_family_positioned_${detection.sourceMode}`;
  }

  return `generic_positioned_${detection.sourceMode}`;
};
...
const extractPdfSourceV22 = async (fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> => {
  ...
  const layoutDetection = TemplateExtractPdfFamilyService.detectFromLayout(sourceTitle, fileName, layout);
  const topology = TemplateExtractPdfTopologyService.buildFromDigitalLayout(layout);
  const positionedHtml = TemplateExtractPdfHtmlCloneService.buildPositionedCloneHtml(sourceTitle, layout);

  if (positionedHtml) {
    const html = TemplateExtractReplicaHtmlNormalizerService.upsertPageMetrics(positionedHtml, layout.pages);
    const pipelineTrace = buildPipelineTrace('22', layoutDetection, topology, inferV22CloneBuilder(layoutDetection));
...
export const TemplateExtractPdfService = {
  ...
  async extractPdfSource(
    fileName: string,
    bytes: Uint8Array,
    version: Extract<TemplateExtractEngineVersion, '19' | '20' | '21' | '22'> = '19'
