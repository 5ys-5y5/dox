...
const renderReplicaPagesToCanvases = async (html: string, iframe: HTMLIFrameElement) => {
...
const renderPdfPagesToCanvases = async (pdfPageDataUrls: string[]) => {
  const pageCanvases: Array<{ pageNumber: number; canvas: HTMLCanvasElement }> = [];
...
export const TemplateExtractVisualSimilarityClient = {
  async measure(input: MeasureVisualSimilarityInput): Promise<TemplateExtractVisualSimilarityReport> {
    ...
    const replicaPages = await renderReplicaPagesToCanvases(input.html, input.iframe);
    const sourcePages = await renderPdfPagesToCanvases(input.pdfPageDataUrls);
