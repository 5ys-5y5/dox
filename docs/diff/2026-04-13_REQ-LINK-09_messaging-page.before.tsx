...
export default function MessagingPage() {
  ...
  const [selectedDefaultRecipientIds, setSelectedDefaultRecipientIds] = React.useState<string[]>([]);
  ...
  const loadMessagingData = React.useCallback(async () => {
    try {
      ...
      if (settingsResult.success) {
        ...
        setSelectedDefaultRecipientIds(nextSettings.defaultRecipientIds || []);
    ...
    try {
      const response = await fetch('/api/messaging/sms/settings', {
        ...
        body: JSON.stringify({
          ...
          defaultRecipientIds: selectedDefaultRecipientIds,
          messagePrefix,
        }),
      });
  ...
  const toggleDefaultRecipientId = (recipientId: string) => {
    setSelectedDefaultRecipientIds((previous) =>
      previous.includes(recipientId)
        ? previous.filter((item) => item !== recipientId)
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">MESSAGING-13</Badge>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border-slate-200">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-800">기본 수신번호 그룹</label>
              <div className="rounded-md border border-slate-200 bg-white">
                {recipients.length > 0 ? (
                  <div className="max-h-44 overflow-auto">
                    {recipients.map((recipient) => {
                      const checked = selectedDefaultRecipientIds.includes(recipient.id);
                      ...
                      return (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={checked}
                            onChange={() => toggleDefaultRecipientId(recipient.id)}
                          />
              ...
              <p className="text-xs text-slate-500">
                `/request-links` 화면에서 문자 발송을 누르면 여기서 저장한 수신번호 그룹을 자동 사용합니다.
              </p>
            </div>
            ...
            {settings ? (
              <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                ...
                <p>저장된 기본 수신번호 수: {settings.defaultRecipientIds.length}건</p>
