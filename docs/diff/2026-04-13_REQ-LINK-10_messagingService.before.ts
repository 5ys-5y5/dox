...
export const MessagingService = {
  async getSmsSettings(): Promise<SmsSettingsDto> {
    ...
    const { data: bindingRows, error: bindingError } = await messagingSchema()
    ...
    if (bindingError) {
      if (
        bindingError.message.includes('schema cache') ||
        bindingError.message.includes('sms_document_sender_registry') ||
        bindingError.message.includes('relation')
      ) {
        return toSettingsDto((data as SmsSettingsRow | null) || null, []);
      }

      throw new Error(`문서별 기본 발신번호 조회 실패: ${bindingError.message}`);
    }

    const senderIds = Array.from(new Set((bindingRows || []).map((row) => String((row as SmsDocumentSenderBindingRow).sender_id))));
    ...
    const documentSenderBindings = (bindingRows || [])
      .map((row) => {
    ...
    return toSettingsDto((data as SmsSettingsRow | null) || null, documentSenderBindings);
  },
