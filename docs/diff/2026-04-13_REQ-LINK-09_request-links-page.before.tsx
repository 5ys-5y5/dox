...
export default function RequestLinksPage() {
  ...
  const handleSendSms = async () => {
    ...
    if (!smsSettings?.defaultSenderPhoneNumber || !smsSettings.defaultRecipientIds.length) {
      setMessage('기본 발신번호와 기본 수신번호 그룹을 먼저 준비하세요. 설정은 /messaging 에서 관리합니다.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/messaging/sms/send', {
        ...
        body: JSON.stringify({
          ...
          recipientIds: smsSettings.defaultRecipientIds,
          requestedBy,
          messageText,
        }),
      });
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">REQ-LINK-07</Badge>
          ...
          <p className="max-w-3xl text-sm text-slate-600">
            ...
            만료 검증, 1회성 사용, 제출 감사 로그를 유지하고, 문자 발송은 `/messaging`의 기본 발신번호를 자동 사용합니다.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border-slate-200">
          <CardHeader>
            ...
            <CardDescription>
              ...
              관리합니다. 이 페이지는 요청 링크 생성과 실제 발송 실행에만 집중합니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-slate-700">
        ...
        <Card className="border-slate-200">
          <CardHeader>
            ...
            <CardDescription>
              선택한 요청 링크를 문자로 전달합니다. 발신번호는 `/messaging`에 저장된 기본 발신번호를 자동 사용하고,
              기본 수신번호 그룹도 `/messaging`에서 자동으로 가져옵니다. 여기서는 전송 실행만 합니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-slate-700">
            <div className="space-y-2">
              <p className="text-xs text-slate-500">
                자동 사용 발신번호: {smsSettings?.defaultSenderPhoneNumber || '-'}
              </p>
              <p className="text-xs text-slate-500">
                자동 사용 기본 수신번호 그룹: {smsSettings?.defaultRecipientIds.length || 0}개
              </p>
            </div>
