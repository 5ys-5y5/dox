    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">TPL-EXT-05</Badge>
          <h1 className="text-3xl font-semibold text-slate-950">템플릿 추출</h1>
          <p className="max-w-3xl text-sm text-slate-600">
            값이 이미 들어간 문서를 읽어 템플릿 등록으로 넘길 수 있는 draft HTML과 후보 필드를 생성합니다.
          </p>
          <p className="max-w-3xl text-xs text-slate-500">
            UI는 단순하게 파일 업로드 또는 본문 붙여넣기만 제공합니다. 뒤에서는 파일 형식 판별, 본문 추출, 초안 생성이 순서대로 분리되어 동작합니다.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <Card className="border-slate-200">
          <CardHeader>
            <CardTitle>입력값이 들어간 원본 문서</CardTitle>
            <CardDescription>
              `txt`, `html`, `docx`, `pdf` 파일을 업로드하거나, 직접 본문을 붙여 넣어 초안을 생성합니다. 업로드 파일이 있으면 붙여넣기 본문보다 우선합니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
              ...
              <p>파일 업로드 시 sourceKind는 자동 판별되며, docx는 HTML 초안으로 정규화되고, pdf는 텍스트 레이어를 먼저 추출한 뒤 기존 규칙 기반 분석기로 넘깁니다.</p>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-800">원본 본문 붙여넣기</label>
              <textarea
                value={sourceContent}
