...
const stripDraftPreviewUiState = (html: string) =>
  html.replace(/\sdata-template-selected="true"/gi, '');
...
const markTemplateValueElementEdited = (element: HTMLElement) => {
  ...
  element.closest('.v201-choice-row')?.setAttribute('data-template-edited', 'true');
};
...
export default function TemplateExtractPage() {
  ...
  const getCurrentDraftPreviewHtml = React.useCallback(() => {
    ...
    const fallbackHtml = draftPreviewHtmlRef.current || draftDetail?.draft.generatedDraftHtml || '';

    return stripDraftPreviewUiState(liveHtml || fallbackHtml);
  }, [draftDetail?.draft.generatedDraftHtml]);

  React.useEffect(() => {
    draftPreviewHtmlRef.current = draftDetail?.draft.generatedDraftHtml || '';
  }, [draftDetail?.draft.id, draftDetail?.draft.generatedDraftHtml]);
  ...
  }, [draftDetail?.draft.generatedDraftHtml, reviewedFields, selectedCandidateKey]);
  ...
  const updatePreviewValueForField = (field: TemplateExtractReviewedFieldInput, nextValue: string) => {
    ...
    draftPreviewHtmlRef.current = stripDraftPreviewUiState(root.innerHTML);
  };
  ...
  const syncPreviewEditTarget = (target: EventTarget | null) => {
    ...
    const valueElement = targetElement?.closest<HTMLElement>('[data-template-value]');

    if (!valueElement) {
      const editableElement = targetElement?.closest<HTMLElement>('.v201-edit-text');
      const root = draftPreviewRef.current;
      if (editableElement) {
        markTemplateValueElementEdited(editableElement);
      }
      draftPreviewHtmlRef.current = root ? stripDraftPreviewUiState(root.innerHTML) : draftPreviewHtmlRef.current;
      return;
    }
  ...
  const handleDraftPreviewSelect = (target: EventTarget | null) => {
    const targetElement = target instanceof HTMLElement ? target : null;
    const choiceButton = targetElement?.closest<HTMLElement>('.v201-choice-box');
            ...
            <CardContent>
              <div className="min-h-[320px] rounded-xl border border-slate-200 bg-white p-6">
                {(draftDetail?.draft.generatedDraftHtml || '').trim() ? (
                  <div
                    ref={draftPreviewRef}
                    className="template-extract-draft-preview prose prose-sm max-w-none text-slate-800"
                    onClick={(event) => handleDraftPreviewSelect(event.target)}
                    onFocusCapture={(event) => handleDraftPreviewSelect(event.target)}
