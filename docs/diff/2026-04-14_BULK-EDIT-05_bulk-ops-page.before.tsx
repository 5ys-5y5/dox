    ...
    () =>
      authentications.map((authentication) => ({
        ...
        label:
          ...
          `${authentication.providerLabel} 인증`,
        meta: [
          authentication.documentTitle || authentication.documentId || authentication.requestId,
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-xs text-slate-500">
            `signature` 계열 라벨은 검증 완료된 본인확인 기록과 확정자 정보가 있을 때만 미리보기/반영이 허용됩니다.
          </p>
        </div>
            ...
            {hasSignatureLabels ? (
              <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                <EntityPicker
                  ...
                  placeholder="본인확인 기록을 선택하세요"
                  emptyMessage="verified 상태의 본인확인 기록이 없습니다."
                />
                <p className="text-xs text-slate-500">
                  서명 라벨 변경에는 verified 인증 기록이 필요합니다. 추가 승인자는 현재 `확정자` 값을 그대로 사용합니다.
                </p>
              </div>
            ) : null}
          </CardContent>
