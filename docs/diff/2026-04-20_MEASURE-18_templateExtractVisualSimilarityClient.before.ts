...
const CANVAS_SAFE_FONT_STACK = 'sans-serif';
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  let measurementCaptureMode: 'foreign_object' | 'manual_canvas' = 'manual_canvas';
  let manualCanvasReadError: unknown = null;

  try {
    canvas = captureWithManualCanvas();
    assertCanvasReadable(canvas);
  } catch (error) {
    manualCanvasReadError = error;
    measurementCaptureMode = 'foreign_object';
    ...
    try {
      ...
      const manualReason = formatUnknownError(manualCanvasReadError);
      const foreignObjectReason = formatUnknownError(foreignObjectError);

      throw new Error(
        `시각 유사도 측정 실패: manual canvas 캡처와 foreignObject 캡처가 모두 실패했습니다. manual=${manualReason}; foreignObject=${foreignObjectReason}.${hintMessage}`
      );
    }
  }
