...
const buildReplicaRuleSegments = (
  ...
  const segments: ReplicaRuleSegment[] = [];
  const seen = new Set<string>();
  const globalColumnEdges = dedupeReplicaRuleAxis(
    [...(geometryPage?.columnEdges || []), ...(rulePage?.columnRules || [])],
    page.width,
    1.25
  );
  const globalRowEdges = dedupeReplicaRuleAxis(
    [...(geometryPage?.rows.flatMap((row) => [row.top, row.top + row.height]) || []), ...(rulePage?.rowRules || [])],
    page.height,
    1.25
  );

  if (globalColumnEdges.length >= 2 && globalRowEdges.length >= 2) {
    const gridLeft = globalColumnEdges[0];
    const gridRight = globalColumnEdges[globalColumnEdges.length - 1];
    const gridTop = globalRowEdges[0];
    const gridBottom = globalRowEdges[globalRowEdges.length - 1];
    const gridWidth = Number((gridRight - gridLeft).toFixed(2));
    const gridHeight = Number((gridBottom - gridTop).toFixed(2));

    if (gridWidth > 8 && gridHeight > 8) {
      for (const rowEdge of globalRowEdges) {
        appendReplicaRuleSegment(segments, seen, {
          orientation: 'h',
          left: gridLeft,
          top: rowEdge,
      ...
      for (const columnEdge of globalColumnEdges) {
        appendReplicaRuleSegment(segments, seen, {
          orientation: 'v',
          left: columnEdge,
          top: gridTop,
