...
- 분리 배포 시 필요한 최소 조건:
  ...
  - 서명 영역 저장소
- 체크리스트:
  - [진행중] `TPL-REG-01` 원본 문서를 HTML 레이아웃 초안으로 자동 변환한다.
...
- 날짜: 2026-04-14
  ...
  - Chrome DevTools MCP:
    - `list_pages()` 호출 시 기존 chrome-profile 이 이미 실행 중이라는 오류로 이번 턴에도 검증하지 못했다.
  - 남은 위험:
    - service role 에 storage bucket 생성 권한이 없는 환경이라면 자동 복구가 다시 실패할 수 있다. 그 경우에는 현재 연결된 Supabase 프로젝트에 `docs/applied/run-this-supabase-photo-storage-bootstrap.sql` 을 수동 적용해야 한다.
- 날짜: 2026-04-14
  ...
  - 남은 위험:
    - 최종적으로는 현재 앱이 연결된 Supabase 프로젝트에 `docs/applied/run-this-supabase-photo-storage-bootstrap.sql` 과 schema reload 가 적용돼야 새 storage metadata 컬럼까지 정상 기록된다.
- 날짜: 2026-04-14
