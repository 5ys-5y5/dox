...
| `TEXT-AUTO-SIZE-MODE-DIRECTION-01` | 텍스트 탭의 자동 크기 선택을 `자동 높이 상자`, `자동 너비 상자`, `고정 상자` 3개 모드로 바꾼다. `자동 높이 상자`가 선택된 경우 아래에 `위로 확장`, `아래로 확장`, `왼쪽으로 확장`, `오른쪽으로 확장` 기준 버튼을 출력하고, 선택 기준은 `data-template-frame-auto-size-anchor`로 저장한다. | `docs/diff/2026-05-12_TEXT_AUTO_SIZE_MODE_DIRECTION-01_*` | 완료 |
