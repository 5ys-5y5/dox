...
${section.valueLines
  .map(
    (line) =>
      `          <li class="template-clone__value-item" data-template-value="${escapeHtml(section.label)}">${escapeHtml(
        line
      )}</li>`
  )
  .join('\n')}
...
${section.valueLines
  .map(
    (line) =>
      `        <p class="template-clone__value-line" data-template-value="${escapeHtml(section.label)}">${escapeHtml(
        line
      )}</p>`
  )
  .join('\n')}
