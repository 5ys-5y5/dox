...
4. 워크스페이스는 이후 selection token 을 직접 다시 해석하지 않고 이 intent 결과만 소비한다.

### `src/components/template/TemplateEditWorkspace.tsx`

1. edge pointerdown 에서 live snapshot + current selection 을 읽고 resize intent 를 먼저 계산한다.
2. click `pointerup` 도 다시 `resolveClick()` 하지 않고 `pointerdown` 에서 저장한 `predictedSelection` 을 그대로 commit 한다.
3. drag-start 에서는 `predictedSelection` 과 `targetEdgeIds` 만 사용하고, selection ref/state/UI 를 같은 경로로 commit 한다.
...
5. same-line 이지만 endpoint 가 닿지 않는 `band-10-cell-2:left` 는 `band-3-cell-2:left` cohort 에 포함되지 않았다.
