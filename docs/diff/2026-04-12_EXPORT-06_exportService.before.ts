...
const getExportDownloadInfo = (row: ExportJobRow) => {
  const downloadSupported = row.target_format === 'pdf';
...
export const ExportService = {
  ...
  async getExportDownloadPayload(exportJobId: string) {
    ...
    if (exportJob.target_format !== 'pdf') {
      throw new Error('다운로드 실패: 현재 2차 구현에서는 PDF만 실제 다운로드를 지원합니다.');
    }
    ...
    const bytes = ExportRendererService.renderPdf({
      documentTitle: document.title,
    ...
    const nextArtifactMetadata = {
      ...(artifact.metadata || {}),
      actualBinaryMode: 'on-demand-pdf-v1',
    ...
    return {
      fileName: buildExportFileName(document.title, version.version_number, 'pdf'),
