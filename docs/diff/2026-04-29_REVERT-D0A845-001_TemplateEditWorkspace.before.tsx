...
const FRAME_VISUAL_BOX_SELECTOR = '[data-template-frame-box="true"]';
...
const FRAME_LOCAL_LEFT_OFFSET_DATASET = 'templateFrameLocalLeftOffset';
const FRAME_LOCAL_TOP_OFFSET_DATASET = 'templateFrameLocalTopOffset';
const FRAME_LOCAL_WIDTH_DATASET = 'templateFrameLocalWidth';
const FRAME_LOCAL_HEIGHT_DATASET = 'templateFrameLocalHeight';
...
const readFrameLocalSize = (node: HTMLElement, axis: 'width' | 'height') => {
  const visualBox = resolveFrameVisualBox(node);
  const datasetKey = axis === 'width' ? FRAME_LOCAL_WIDTH_DATASET : FRAME_LOCAL_HEIGHT_DATASET;
  const stored = Number.parseFloat(visualBox.dataset[datasetKey] || '');
  ...
  return axis === 'width' ? contentRect.width : contentRect.height;
};

const readFrameLocalRect = (node: HTMLElement): FrameNodeRect | null => {
  const visualBox = resolveFrameVisualBox(node);
  const hasLocalOverride =
    visualBox.dataset[FRAME_LOCAL_WIDTH_DATASET] ||
    visualBox.dataset[FRAME_LOCAL_HEIGHT_DATASET] ||
    visualBox.dataset[FRAME_LOCAL_LEFT_OFFSET_DATASET] ||
    visualBox.dataset[FRAME_LOCAL_TOP_OFFSET_DATASET];

  if (!hasLocalOverride) {
    return null;
  }

  const nodeRect = readFrameElementRect(visualBox);
  return {
    left: nodeRect.left,
    top: nodeRect.top,
    width: Math.max(1, readFrameLocalSize(node, 'width')),
    height: Math.max(1, readFrameLocalSize(node, 'height')),
  };
};

const writeFrameLocalRect = (
  node: HTMLElement,
  nextRect: {
    width?: number;
    height?: number;
  ...
  visualBox.style.position = 'relative';

  if (typeof nextRect.leftOffset === 'number') {
    visualBox.dataset[FRAME_LOCAL_LEFT_OFFSET_DATASET] = String(nextRect.leftOffset);
    visualBox.style.left = `${Math.round(nextRect.leftOffset)}px`;
  }

  if (typeof nextRect.topOffset === 'number') {
    visualBox.dataset[FRAME_LOCAL_TOP_OFFSET_DATASET] = String(nextRect.topOffset);
    visualBox.style.top = `${Math.round(nextRect.topOffset)}px`;
  }

  if (typeof nextRect.width === 'number') {
    const safeWidth = Math.max(MIN_FRAME_SIZE_PX, nextRect.width);
    visualBox.dataset[FRAME_LOCAL_WIDTH_DATASET] = String(safeWidth);
    visualBox.style.width = `${Math.round(safeWidth)}px`;
    visualBox.style.minWidth = `${Math.round(safeWidth)}px`;
    visualBox.style.maxWidth = `${Math.round(safeWidth)}px`;
    contentTarget.style.width = `${Math.round(safeWidth)}px`;
    contentTarget.style.minWidth = `${Math.round(safeWidth)}px`;
    contentTarget.style.maxWidth = `${Math.round(safeWidth)}px`;
  }

  if (typeof nextRect.height === 'number') {
    const safeHeight = Math.max(MIN_FRAME_SIZE_PX, nextRect.height);
    visualBox.dataset[FRAME_LOCAL_HEIGHT_DATASET] = String(safeHeight);
    visualBox.style.height = `${Math.round(safeHeight)}px`;
    visualBox.style.minHeight = `${Math.round(safeHeight)}px`;
    visualBox.style.maxHeight = `${Math.round(safeHeight)}px`;
    contentTarget.style.height = `${Math.round(safeHeight)}px`;
    contentTarget.style.minHeight = `${Math.round(safeHeight)}px`;
    contentTarget.style.maxHeight = `${Math.round(safeHeight)}px`;
  }
};
...
const readFrameNodeRect = (node: HTMLElement): FrameNodeRect => {
  const localRect = readFrameLocalRect(node);
  ...
  if (visualBox !== node) {
    return readFrameElementRect(visualBox);
  }
...
const applyFrameResizeWithDirection = (
  ...
  if (visualBox !== node) {
    if (direction.includes('w')) {
      applyFrameLocalWidthDelta(node, 'left', nextRect.left - currentRect.left);
    } else if (direction.includes('e')) {
      applyFrameLocalWidthDelta(node, 'right', nextRect.width - currentRect.width);
    }

    if (direction.includes('n')) {
      applyFrameLocalTopDelta(node, nextRect.top - currentRect.top);
    } else if (direction.includes('s')) {
      applyFrameLocalHeightDelta(node, nextRect.height - currentRect.height);
    }
    return;
  }
...
const applyFrameLocalWidthDelta = (node: HTMLElement, side: 'left' | 'right', delta: number) => {
  if (Math.abs(delta) < 0.5) {
    return 0;
  }

  const currentRect = readFrameNodeRect(node);
  const currentLeftOffset = parseFramePx(node.style.left || node.dataset[FRAME_LOCAL_LEFT_OFFSET_DATASET] || '0');

  if (side === 'left') {
    const nextWidth = Math.max(MIN_FRAME_SIZE_PX, currentRect.width - delta);
    const appliedDelta = currentRect.width - nextWidth;
    writeFrameLocalRect(node, {
      leftOffset: currentLeftOffset + appliedDelta,
      width: nextWidth,
    });
    return appliedDelta;
  }
  ...
  const nextWidth = Math.max(MIN_FRAME_SIZE_PX, currentRect.width + delta);
  const appliedDelta = nextWidth - currentRect.width;
  writeFrameLocalRect(node, { width: nextWidth });
  return appliedDelta;
};

const applyFrameLocalTopDelta = (node: HTMLElement, delta: number) => {
  if (Math.abs(delta) < 0.5) {
    return 0;
  }

  const currentRect = readFrameNodeRect(node);
  const currentTopOffset = parseFramePx(node.style.top || node.dataset[FRAME_LOCAL_TOP_OFFSET_DATASET] || '0');
  const nextHeight = Math.max(MIN_FRAME_SIZE_PX, currentRect.height - delta);
  const appliedDelta = currentRect.height - nextHeight;
  writeFrameLocalRect(node, {
    topOffset: currentTopOffset + appliedDelta,
    height: nextHeight,
  });
  return appliedDelta;
};

const applyFrameLocalHeightDelta = (node: HTMLElement, delta: number) => {
  if (Math.abs(delta) < 0.5) {
    return 0;
  }
  ...
  const currentRect = readFrameNodeRect(node);
  const nextHeight = Math.max(MIN_FRAME_SIZE_PX, currentRect.height + delta);
  const appliedDelta = nextHeight - currentRect.height;
  writeFrameLocalRect(node, { height: nextHeight });
  return appliedDelta;
};
...
const resolveFrameVisualBox = (node: HTMLElement) => {
  const existing = node.querySelector<HTMLElement>(`:scope > ${FRAME_VISUAL_BOX_SELECTOR}`);

  if (existing) {
    return existing;
  }

  if (node.matches(FRAME_SELECTION_NODE_SELECTOR)) {
    return ensureFrameVisualBox(node);
  }

  return node;
};

const syncFrameVisualBoxContentStyles = (node: HTMLElement) => {
  const contentTarget = resolveFrameContentTarget(node);
  ...
  contentTarget.style.minWidth = '100%';
  contentTarget.style.maxWidth = '100%';
  contentTarget.style.height = '100%';
  contentTarget.style.minHeight = '100%';
  contentTarget.style.boxSizing = 'border-box';
  contentTarget.style.background = 'transparent';
  contentTarget.style.border = '0';
  contentTarget.style.margin = '0';
};

const ensureFrameVisualBox = (node: HTMLElement) => {
  const existing = node.querySelector<HTMLElement>(`:scope > ${FRAME_VISUAL_BOX_SELECTOR}`);

  if (existing) {
    syncFrameVisualBoxContentStyles(node);
    return existing;
  }

  const visualBox = document.createElement('div');
  visualBox.setAttribute('data-template-frame-box', 'true');
  visualBox.style.position = 'relative';
  visualBox.style.display = 'block';
  visualBox.style.width = '100%';
  visualBox.style.minWidth = '100%';
  visualBox.style.maxWidth = '100%';
  visualBox.style.height = '100%';
  visualBox.style.minHeight = '100%';
  visualBox.style.boxSizing = 'border-box';
  visualBox.style.overflow = 'hidden';
  visualBox.style.borderLeft = node.style.borderLeft;
  visualBox.style.borderRight = node.style.borderRight;
  visualBox.style.borderTop = node.style.borderTop;
  visualBox.style.borderBottom = node.style.borderBottom;
  visualBox.style.backgroundColor = node.style.backgroundColor;
  visualBox.style.borderRadius = node.style.borderRadius;

  const childNodes = Array.from(node.childNodes);
  childNodes.forEach((childNode) => {
    visualBox.appendChild(childNode);
  });
  node.appendChild(visualBox);
...
const applyFrameSelectionUi = (
  ...
  if (edgeSelectionState.tokens.length > 0 && edgeSnapshot) {
    ...
    edgeSelectionState.tokens.forEach((token) => {
      ...
      token.memberEdgeIds.forEach((edgeId) => {
        ...
        const host = resolveFrameVisualBox(node);
        host.setAttribute('data-template-edge-visual', 'true');
    ...
    anchorFrameGroupIds.forEach((frameGroupId) => {
      ...
      const host = resolveFrameVisualBox(node);
      host.setAttribute('data-template-edge-anchor-node', 'true');
      FRAME_RESIZE_DIRECTIONS.forEach((direction) => {
        host.appendChild(buildResizeHandle(direction));
      });
    });
  ...
  root.querySelectorAll<HTMLElement>(FRAME_SELECTION_NODE_SELECTOR).forEach((node) => {
    ...
    const host = resolveFrameVisualBox(node);
    host.setAttribute('data-template-selected', 'true');
    host.setAttribute('data-template-selection-order', String(selectionIndex + 1));

    if (selectionIndex === 0) {
      host.setAttribute('data-template-primary-selected', 'true');
    }
...
const applyFrameStylePatch = (
  ...
  if (typeof patch.width === 'number' && Number.isFinite(patch.width)) {
    if (visualBox !== node) {
      writeFrameLocalRect(node, { width: patch.width });
    } else {
  ...
  if (typeof patch.height === 'number' && Number.isFinite(patch.height)) {
    if (visualBox !== node) {
      writeFrameLocalRect(node, { height: patch.height });
    } else {
  ...
  if (patch.backgroundColor !== undefined) {
    visualBox.style.backgroundColor = patch.backgroundColor || '';
  }

  if (patch.borderRadius !== undefined) {
    visualBox.style.borderRadius = patch.borderRadius ? `${Number.parseFloat(patch.borderRadius)}px` : '';
  }
...
export default function TemplateEditWorkspace({ initialTemplateId = '' }: TemplateEditWorkspaceProps) {
  ...
  const syncDraftPreviewHtmlRef = React.useCallback(() => {
    ...
    normalizeFrameVisualBoxes(root);
  ...
  const bindPreviewRef = React.useCallback(
    (node: HTMLDivElement | null) => {
      ...
      window.requestAnimationFrame(() => {
        ...
        normalizeFrameVisualBoxes(node);
        applyPreviewEditPermissions(node);
        applyFrameSelectionUi(
          node,
          selectedFrameGroupIdsRef.current,
          reconcileLiveEdgeSelection(node, edgeSelectionStateRef.current),
          buildLiveEdgeTopologySnapshot(node)
        );
      });
    },
    [buildLiveEdgeTopologySnapshot, reconcileLiveEdgeSelection]
  ...
  const collectEdgeResizeTargets = React.useCallback(
    (
      ...
      compatibleTokens.forEach((token) => {
        const activeEdgeIds = token.mode === 'isolated' ? [token.anchorEdgeId] : token.memberEdgeIds;
        ...
        activeEdgeIds.forEach((edgeId) => {
          ...
          const alreadyTracked = targets.some((candidate) => candidate.edgeId === target.edgeId);
  ...
  const handlePreviewPointerMove = React.useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!resizeState && edgePressState && event.pointerId === edgePressState.pointerId) {
      ...
      applyRuntimeSelectionUi([], nextEdgeSelection);
      resizeStateRef.current = {
      ...
      if (resizeState.edgeResizeTargets && resizeState.edgeResizeTargets.length > 0) {
        ...
        resizeState.edgeResizeTargets.forEach((target) => {
          if ((target.side === 'left' || target.side === 'right') && Math.abs(nextDeltaX) >= 0.5) {
            const actualDelta = applyFrameLocalWidthDelta(target.node, target.side, nextDeltaX);
            if (
          ...
          if (target.side === 'top' && Math.abs(nextDeltaY) >= 0.5) {
            const actualDelta = applyFrameLocalTopDelta(target.node, nextDeltaY);
            if (
              Math.abs(actualDelta) >= 0.5 &&
          ...
          if (target.side === 'bottom' && Math.abs(nextDeltaY) >= 0.5) {
            const actualDelta = applyFrameLocalHeightDelta(target.node, nextDeltaY);
            if (
              Math.abs(actualDelta) >= 0.5 &&
              (Math.abs(recordedAppliedDeltaY) < 0.5 || Math.abs(actualDelta) < Math.abs(recordedAppliedDeltaY))
            ) {
  ...
  }, [applyRuntimeSelectionUi, getFrameNodes, resolveEdgeSelectionForResizeStart, collectEdgeResizeTargets]);
