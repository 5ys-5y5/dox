...
export const TemplateExtractPdfService = {
  ...
  // 2) 페이지 렌더 이미지를 생성해 원문 배경을 확보
  // 3) 좌표 기반 overlay placeholder 를 생성
  // 4) 실패 시 key-value text fallback
  // 순서로 처리합니다.
  async extractPdfSource(fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> {
    const [layout, pageImages] = await Promise.all([
      extractPdfLayout(fileName, bytes),
    ...
    const layoutWithImages: TemplateExtractPdfLayoutModel = {
    ...
    const clonedHtml = TemplateExtractPdfLayoutService.buildCloneHtml(sourceTitle, rawText, layoutWithImages);
