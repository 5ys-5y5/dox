...
export default function ExportsPage() {
  ...
  const handlePrepareHwpHandoff = async () => {
    ...
    try {
      ...
      setMessage(`hwp handoff payload 를 준비했습니다. (${result.data.externalJobId})`);
      await handleLoadJob();
    } catch (error) {
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-slate-600">
            ...
            `docx` 실제 다운로드, `hwp` 외부 handoff/callback 계약까지 다룹니다.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="space-y-6">
          ...
          <Card className="border-slate-200">
            <CardHeader>
              <CardTitle>HWP handoff / callback</CardTitle>
              <CardDescription>외부 변환기 연계 전, handoff payload와 callback 계약을 같은 화면에서 검증합니다.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-slate-700">
              ...
              {hwpHandoff ? (
                <>
                  <p>External Job ID: {hwpHandoff.externalJobId}</p>
                  <p>Callback URL: {hwpHandoff.callbackUrl}</p>
                  <p>Callback Token: {hwpHandoff.callbackToken}</p>
                  <pre className="overflow-x-auto rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                    {JSON.stringify(hwpHandoff.payload, null, 2)}
                  </pre>
                </>
              ) : (
                <p className="text-slate-500">아직 준비된 HWP handoff payload가 없습니다.</p>
              )}

              {hwpCallbackResult ? (
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                  <p>Callback 처리 상태: {hwpCallbackResult.status}</p>
                  <p>Export Job ID: {hwpCallbackResult.exportJobId}</p>
                  <p>Artifact ID: {hwpCallbackResult.artifact?.id || '-'}</p>
                </div>
              ) : null}
            </CardContent>
          </Card>
