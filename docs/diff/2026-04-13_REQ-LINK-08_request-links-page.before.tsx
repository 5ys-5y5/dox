...
export default function RequestLinksPage() {
  ...
  const loadSmsRecipients = React.useCallback(async () => {
    try {
      const recipientResponse = await fetch('/api/messaging/sms/recipients', { cache: 'no-store' });
  ...
  const handleSendSms = async () => {
    ...
    if (!smsSettings?.defaultSenderPhoneNumber || selectedSmsRecipientIds.length === 0) {
      setMessage('기본 발신번호와 수신번호를 먼저 준비하세요. 발신번호 설정은 /messaging 에서 관리합니다.');
      return;
    }
