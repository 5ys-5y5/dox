...
| `NON-TEXT-BOX-DRAG-RESTORE-01` | 텍스트 입력창 드래그 선택 보호는 `텍스트` 탭에만 한정한다. `속성`, `크기 및 위치` 등 텍스트 외 탭에서 상자 위 드래그 이동은 기존처럼 허용한다. | `docs/diff/2026-05-12_NON_TEXT_BOX_DRAG_RESTORE-01_*` | 완료 |
...
- 2026-05-12: `NON-TEXT-BOX-DRAG-RESTORE-01` 구현. 상자 이동 시작 차단 조건을 `selectionPanelTab !== 'position'`에서 `selectionPanelTab === 'text'`로 좁혔다. 또한 비-위치 탭 전체에서 상자 위 드래그를 마퀴 선택으로 가져가던 조건과 `contenteditable`/입력창 target 가드도 `텍스트` 탭으로 한정해, 텍스트 외 탭에서는 상자 드래그 이동 경로가 실행되도록 복구했다. 좌표가 `.v202-frame-group` 직접 target이 아닌 band 내부 DIV에 걸린 경우도 속성 탭에서 상자 선택/이동으로 들어가도록 비-위치 탭의 좌표 기반 frame 탐색을 복구했다.
- 2026-05-12: chrome-devtools MCP 검증. 대표 URL 새로고침 후 이동 모드에서 `속성` 탭 `band-0-header`를 상자 위 pointer drag로 이동했다. 결과는 left `+28`, top `+22`로 이동했고, 같은 절차를 `크기 및 위치` 탭 `band-3-cell-1`에 적용했을 때 left `+18`, top `+12`로 이동했다. `텍스트` 탭 `band-1-header` 상자 위 drag는 left/top 변화 `0`으로 이동하지 않았다.
