...
const parseStatusOverview = (statusLines: string[]): ParsedStatusOverview => {
  ...
  for (const line of statusLines) {
    ...
    matched = line.match(/^발급자 서명\s+(.+?)\s+전자서명 완료(?:\s+접수자 서명.*)?$/);

    if (matched) {
      overview.issuerSignerName = cleanupPdfValue(matched[1]);
      overview.issuerSignatureStatus = '전자서명 완료';
    }
  }
...
const buildStatusHtml = (statusLines: string[]) => {
  ...
  return `    <table class="template-clone__status-table">
      ...
      <tr>
        ...
        <th>전자서명 상태</th>
        <td><div data-template-value="전자서명 상태">${escapeHtml(overview.issuerSignatureStatus)}</div></td>
      </tr>
      </tbody>
    </table>`;
};
