...
const buildReplicaCandidateQualityReport = (
  layout: TemplateExtractPdfLayoutModel,
  candidate: ReplicaHtmlCandidate
) =>
  TemplateExtractReplicaOfflineQualityService.evaluate({
    sourceText: candidate.sourceText,
    sourceMode: candidate.topology.sourceMode,
    sourcePages: layout.pages,
    topologySummary: candidate.topology.summary,
    replicaHtml: candidate.html,
...
const getReplicaCandidateSelectionScore = (
  qualityReport: TemplateExtractReplicaQualityReport,
  candidate: ReplicaHtmlCandidate
) => {
  const metrics = qualityReport.offlineMetrics;
  const normalized = TemplateExtractReplicaHtmlNormalizerService.normalizeReplicaHtml(candidate.html);

  return (
    (metrics?.overallScore || 0) * 1000 +
    (metrics?.vectorTopologyScore || 0) * 100 +
    (metrics?.textAnchorScore || 0) * 10 +
    normalized.valueMarkerCount * 0.001 +
    candidate.priority * 0.1 -
    qualityReport.summary.hardFailureCount * 100
  );
};

const selectBestReplicaCandidate = (layout: TemplateExtractPdfLayoutModel, candidates: ReplicaHtmlCandidate[]) => {
  const uniqueCandidates = new Map<string, ReplicaHtmlCandidate>();
  ...
  for (const candidate of candidates) {
    ...
    const existing = uniqueCandidates.get(normalizedHtml);

    if (!existing || candidate.priority > existing.priority) {
      uniqueCandidates.set(normalizedHtml, {
        ...candidate,
        html: normalizedHtml,
      });
    }
  ...
  const rankedCandidates = Array.from(uniqueCandidates.values())
    .map((candidate) => {
      const qualityReport = buildReplicaCandidateQualityReport(layout, candidate);

      return {
        candidate,
        qualityReport,
        selectionScore: getReplicaCandidateSelectionScore(qualityReport, candidate),
      };
    })
...
const extractPdfSourceV21 = async (fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> => {
  ...
  if (layoutDetection.sourceMode === 'digital') {
    ...
    if (genericHtml) {
      candidateResults.push({
        ...
        sourceText: layout.rawText,
        topology,
        priority: layoutDetection.documentFamily === 'work_order' ? 20 : 40,
      });
    }
    ...
    const selectedCandidate = selectBestReplicaCandidate(layout, candidateResults);

    if (selectedCandidate) {
      const pipelineTrace = buildPipelineTrace('21', layoutDetection, selectedCandidate.candidate.topology, selectedCandidate.candidate.cloneBuilder);
      return buildResolvedHtmlSource(
        fileName,
        sourceTitle,
        selectedCandidate.candidate.html,
        pipelineTrace,
        selectedCandidate.qualityReport
      );
    }
  ...
  const ruleDetection = TemplateExtractPdfFamilyService.detectFromRuleModel(sourceTitle, fileName, ruleModel);
  const topology = TemplateExtractPdfTopologyService.buildFromScannedRuleModel(ruleModel);
  const layoutTopology = TemplateExtractPdfTopologyService.buildFromDigitalLayout(layout);
  const scannedSourceText = ruleModel.rawText.length >= layout.rawText.length ? ruleModel.rawText : layout.rawText;
  const candidateResults: ReplicaHtmlCandidate[] = [];
  const genericHtml = TemplateExtractPdfLayoutService.buildGenericCloneHtml(sourceTitle, layout);
  ...
  if (genericHtml) {
    candidateResults.push({
      html: withReplicaPageMetrics(layout, genericHtml),
      cloneBuilder: 'generic_layout_scanned',
      sourceText: layout.rawText || scannedSourceText,
      topology: layoutTopology,
      priority: ruleDetection.documentFamily === 'certificate' ? 50 : 25,
    });
  }

  if (ruleDetection.documentFamily === 'work_order') {
    const topologyHtml = TemplateExtractWorkOrderTopologyBuilderService.buildCloneHtml(sourceTitle, topology, '20');
    const fallbackResult = TemplateExtractWorkOrderBuilderService.buildFromRuleModel(
      sourceTitle,
      fileName,
      ruleModel,
      topology,
    ...
    if (fallbackResult.html) {
      candidateResults.push({
        html: withReplicaPageMetrics(layout, fallbackResult.html),
        cloneBuilder: fallbackResult.cloneBuilder,
        sourceText: scannedSourceText,
        topology,
        priority: 20,
      });
    ...
    if (recoveredHtml) {
      candidateResults.push({
        ...
        cloneBuilder:
          ruleDetection.documentFamily === 'certificate' ? 'certificate_family_scanned' : 'generic_rule',
        sourceText: scannedSourceText,
        topology,
        priority: ruleDetection.documentFamily === 'certificate' ? 10 : 15,
      });
    }
  }
  ...
  if (selectedCandidate) {
    const pipelineTrace = buildPipelineTrace(
      '21',
      ruleDetection,
      selectedCandidate.candidate.topology,
