...
- `mcp__chrome_devtools__`
  - 결과: 성공
  - 비고: sandbox 에서 `localhost:3001` 서버를 띄울 수 없어 `file:///tmp/selected-edge-harness/index.html` 하니스에서 실제 `TemplateEditWorkspace` 를 구동했다
  - 실측 1: `band-3-cell-2:left` 1회 클릭 후 drag 시 `band-3-cell-2` 와 `band-4-cell-2` 가 각각 `24px` 이동했고 `band-10-cell-2` 는 `0px` 변화였다
  - 실측 2: 같은 edge 2회 클릭 후 drag 시 `band-3-cell-2` 만 `24px` 이동했고 `band-10-cell-2` 는 `0px` 변화였다
  - 실측 3: `band-3-cell-1:right` drag 시 `band-3` 내부 경계만 `24px` 이동했고 `band-10-cell-2` 는 `0px` 변화였다
  - 실측 4: 위 세 케이스 모두 drag 종료 후 selection mode 는 기대한 상태(`connected` 또는 `isolated`)를 유지했다

### 환경 제약

1. sandbox 제약 때문에 로컬 `next dev` 를 직접 붙여 브라우저 자동 검증하지 못했다.
2. 대신 `chrome-devtools` 와 `file://` 기반 하니스로 실제 DOM/pointer lifecycle 을 검증했다.
3. 이번 턴 자동 검증 범위는 문서, 타입체크, 번들, synthetic runtime, `chrome-devtools` 실동작 기록, `supabase` MCP 기록까지다.
