...
1. `snapshot` 계산
2. `resolveActivation(... phase='pointerdown-preview')`
3. `predictedActivationResult` 를 `edgePressState` 에 저장
4. 저장 대상은 `currentSelection` 이 아니라 `predictedActivationResult`

### 10.2 drag-start

1. `predictedActivationResult.effectiveEdgeIds` 를 `Edge Mutation Handle Service` 로 전달
...
1. drag 가 없었으면 `predictedActivationResult.nextSelectionState` 를 commit
2. drag 가 있었으면 실제 `effectiveSelectionState` 를 commit
...
- `CHK-RESTART-SEL-EDGE-001`
  ...
  - 완료 기준: `predictedActivationResult` 기반 flow 가 문서화되어 있다.
...
2. attach 된 페이지는 `about:blank` 1개뿐이었다
3. sandbox 에서 `next dev` 가 `listen EPERM` 으로 막혀 실페이지 자동 검증은 수행되지 못했다
