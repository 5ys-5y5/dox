...
3. `/Users/gy/Documents/dev/docs/docs/[priority1]highperform_pdf_to_html_converter_type3(edit, checkbox)(mbhj).py`
   - 역할: editable text, checkbox reconstruction, OCR rescue 참고
   - 핵심: `ChoiceMark`, `InlineFragment`, `RichLine`, `Cell`, `Table`, `TextBlock`, `PageModel`
   - 후속 구현 반영점: `contenteditable` 기반 텍스트 편집 가능성, checkbox/choice mark 복원, rich line 추출
   - 금지: type3의 `render_document()`, `render_page()`, `render_table()` 출력을 시각 레이아웃 기준으로 사용하지 않는다.
   - 이유: type3 전체 렌더러는 문서 구조를 band/frame/grid로 재조립하면서 원본 문서를 파괴할 수 있으므로, 편집 가능성과 체크박스 복원 로직만 제한적으로 참고한다.
...
5. `/Users/gy/Documents/dev/docs/scripts/template-extract-raster-first-replica.py`
   - 목적: type1의 crop 기반 visual surface를 기준으로 하고, type3은 editable text/checkbox overlay 생성에만 사용해 서비스 CLI 형태로 이식
...
type3 전체 렌더러는 문서 레이아웃을 파괴하는 출력이 확인되어 v2.01 visual source로 사용하지 않는다. `scripts/template-extract-raster-first-replica.py` 는 type1의 crop 기반 table/block visual surface를 기준으로 HTML을 만들고, type3은 invisible/focus-visible editable text overlay와 checkbox toggle overlay를 만드는 보조 참조로만 사용한다.
...
9. type3 `render_document()` 미사용
10. type1 visual surface와 type3 edit/checkbox overlay 분리
...
`pdf-raster-first-v2.01` clone id를 감지하고 `.editable-text` 및 `.v201-edit-text` 인접 라벨/값 후보에 `data-template-value` 를 추가한다. placeholder 치환은 하지 않아 시각 레이아웃을 보존한다. table/grid cell 전용 정밀 projection 은 후속 보강 대상이다.
...
### `REFACTOR-08` 선택 항목 텍스트 편집 동기화

목표:

시각 유사도 통과 상태를 유지하면서, 선택한 후보 항목의 텍스트 수정이 미리보기 HTML과 승인 템플릿에 반영되게 한다.

완료 조건:

1. `pdf-raster-first-v2.01` projection 이 `.v201-edit-text` 를 value binding 대상으로 인식
2. v2.01 edit overlay 수정값이 focus 해제 후에도 보이는 상태로 유지
3. 미리보기에서 `data-template-value` 를 클릭/포커스하면 오른쪽 검토 항목 선택 상태와 연결
4. 오른쪽 `선택 항목 수정값` 입력 시 왼쪽 선택 텍스트와 승인 HTML에 즉시 반영
5. 미리보기에서 직접 편집한 값도 검토 JSON의 `defaultValue` 와 승인 요청의 `generatedDraftHtml` 에 반영

관련 diff:

1. `2026-04-22_13-22-20_REFACTOR-08_template-extract-raster-first-replica.before.py`
2. `2026-04-22_13-22-20_REFACTOR-08_templateExtractDomProjectionService.before.ts`
3. `2026-04-22_13-22-20_REFACTOR-08_extract-page.before.tsx`
4. `2026-04-22_13-22-20_REFACTOR-08_templateExtractDtos.before.ts`
5. `2026-04-22_13-22-20_REFACTOR-08_templateExtractService.before.ts`
6. `2026-04-22_13-22-20_REFACTOR-08_templateExtractValueBindingService.before.ts`
7. `2026-04-22_13-22-20_REFACTOR-08_refactor.before.md`
...
1. type3 전체 렌더러 사용을 제거하고 type1 visual surface + type3 edit/checkbox overlay 구조로 변경했다.
2. `PYTHONPYCACHEPREFIX=/tmp/template-extract-pycache python3 -m py_compile scripts/template-extract-raster-first-replica.py` 성공
3. `.venv-template-extract-v2/bin/python scripts/template-extract-raster-first-replica.py --help` 성공
4. type3은 `convert_page()` 결과의 `RichLine`/choice 정보만 overlay와 render model 보조 데이터로 사용한다.
5. type3 `render_document()` 출력은 v2.01 visual HTML에 사용하지 않는다.
6. 샘플 PDF 변환 결과 `referenceConverter` 는 `visual=type1; edit=type3` 로 기록된다.
7. 샘플 PDF 변환 결과 `scan-surface`, embedded PNG crop, `v201-edit-overlay`, `v201-choice-box` 가 포함된다.
8. 샘플 PDF 변환 결과 type3의 `.band`, `.text-frame` 기반 visual output 은 포함되지 않는다.
9. `npm run template-extract:measure -- --file "docs/작업지시서_사일동 주상복합.pdf" --version v2.01 --minimumPassScore 0.95` 는 실행 성공, 점수 `59.85%`, `passed=false`.
10. 현재 터미널 측정은 crop image surface를 직접 렌더하지 않는 `positioned-v1` render model 기반이므로, crop 기반 visual HTML의 실제 브라우저 유사도와 다를 수 있다.
...
| `REFACTOR-03` | 부분 완료 | Python wrapper 및 TypeScript adapter 추가, type1 visual surface + type3 edit/checkbox overlay로 정정 |
...
| `REFACTOR-08` | 완료 | 선택 항목 텍스트 편집을 v2.01 overlay, projection, approve payload 에 연결 |
...
9. `src/services/templateExtractService.ts`
10. `src/services/templateExtractValueBindingService.ts`
...
9. `docs/diff/2026-04-22_13-22-20_REFACTOR-08_template-extract-raster-first-replica.before.py`
10. `docs/diff/2026-04-22_13-22-20_REFACTOR-08_templateExtractDomProjectionService.before.ts`
11. `docs/diff/2026-04-22_13-22-20_REFACTOR-08_extract-page.before.tsx`
12. `docs/diff/2026-04-22_13-22-20_REFACTOR-08_templateExtractDtos.before.ts`
13. `docs/diff/2026-04-22_13-22-20_REFACTOR-08_templateExtractService.before.ts`
14. `docs/diff/2026-04-22_13-22-20_REFACTOR-08_templateExtractValueBindingService.before.ts`
15. `docs/diff/2026-04-22_13-22-20_REFACTOR-08_refactor.before.md`
...
9. `PYTHONPYCACHEPREFIX=/tmp/template-extract-pycache python3 -m py_compile scripts/template-extract-raster-first-replica.py` 성공
10. `npx esbuild src/app/templates/extract/page.tsx --bundle --platform=browser --format=esm --outfile=/tmp/template-extract-page-check.js` 성공
11. `npx esbuild src/services/templateExtractDomProjectionService.ts --bundle --platform=node --format=cjs --outfile=/tmp/template-extract-dom-projection-check.cjs` 성공
12. `npx esbuild src/services/templateExtractService.ts --bundle --platform=node --format=cjs --outfile=/tmp/template-extract-service-check.cjs` 성공
13. `.venv-template-extract-v2/bin/python scripts/template-extract-raster-first-replica.py --input-pdf 'docs/작업지시서_사일동 주상복합.pdf' --engine-version 32` 성공
14. v2.01 sample projection 확인: `v201-edit-text=72`, `data-template-edit-kind=79`, `data-template-value=10`, `candidateCount=10`
15. `npm run check:no-shadow-app` 성공
16. `npm run lint` 는 `check:no-shadow-app` 통과 후 ESLint 9 flat config 부재로 실패
17. `npx tsc --noEmit --pretty false --skipLibCheck --jsx react-jsx --moduleResolution bundler --module esnext --target es2022 --lib dom,dom.iterable,es2022 --allowSyntheticDefaultImports --esModuleInterop src/app/templates/extract/page.tsx src/lib/templateExtractDtos.ts src/services/templateExtractDomProjectionService.ts src/services/templateExtractService.ts src/services/templateExtractValueBindingService.ts` 성공
