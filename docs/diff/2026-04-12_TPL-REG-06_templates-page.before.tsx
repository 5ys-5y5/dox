        ...
        <div className="space-y-6">
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent className="space-y-3 text-sm text-slate-700">
              {createdTemplate || recentTemplates[0] ? (
                <>
                  <div className="flex items-center gap-2">
                    <Badge variant="green">{(createdTemplate || recentTemplates[0])!.status}</Badge>
                    <span className="font-medium text-slate-900">
                      {(createdTemplate || recentTemplates[0])!.templateName}
                    </span>
                  </div>
                  <p>템플릿 ID: {(createdTemplate || recentTemplates[0])!.id}</p>
                  <p>레이아웃 정책: {(createdTemplate || recentTemplates[0])!.layoutResizeMode}</p>
                  <p>원본 문서: {(createdTemplate || recentTemplates[0])!.sourceDocumentName || '-'}</p>
                </>
              ) : (
              ...
              {recentTemplates.length > 0 ? (
                recentTemplates.map((template) => (
                  <button
                    ...
                    className="flex w-full flex-col items-start rounded-lg border border-slate-200 p-3 text-left transition hover:border-slate-300 hover:bg-slate-50"
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <p className="mt-2 text-xs text-slate-500">{template.id}</p>
