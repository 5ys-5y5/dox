...
const main = async () => {
  ...
  const version = TemplateExtractVersionService.normalizeVersion(args.version || '19');
  ...
  if (!filePath) {
    throw new Error('사용법: npm run template-extract:measure -- --file <pdf-path> [--version 28] [--minimumPassScore 0.95]');
  }
  ...
  try {
    await TemplateExtractMeasurementLogService.appendEvent({
      ...
      detail: `${fileName} / engineVersion=${version}`,
      payload: { filePath, fileName, version },
    });
