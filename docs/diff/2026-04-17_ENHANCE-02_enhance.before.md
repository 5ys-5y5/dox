...
19. `src/services/templateExtractPdfReplicaComposerService.ts`
   - page-faithful HTML/CSS/SVG composer
20. `src/services/templateExtractReplicaQualityService.ts`
...
원본 PDF 렌더 결과와 HTML 렌더 결과를 비교해 `99.9% 시각 유사성`을 수치화하고, 승격/복귀를 제어한다.
...
1. original PDF raster render
2. replica HTML screenshot render
3. page diff, pixel mismatch, bbox coverage 계산
...
2. `Digital PDF Scene Service` 가 text/vector/image scene 추출
3. `Replica Composer Service` 가 `pdf-replica-v21` 생성
4. `Value Binding Service` 가 placeholder 주입
5. `Replica Quality Service` 가 시각 diff 평가
6. pass 시 `v21` 채택, fail 시 `v20` 복귀
...
2. `Scanned PDF Scene Service` 가 rule/OCR/image fragment scene 생성
3. `Replica Composer Service` 가 `pdf-replica-v21` 생성
4. `Value Binding Service` 가 low-confidence block 을 피해 placeholder 주입
5. `Replica Quality Service` 가 시각 diff 평가
6. pass 시 `v21` 채택, fail 시 `v20` 복귀
...
3. low-confidence OCR 영역을 image fragment 로 전환 가능하게 함
...
2. visual similarity gate 도입
3. pass/fail 시 fallback 정책 명시
...
1. 샘플 4건에 대해 mismatch ratio 가 계산된다.
2. threshold 미달 시 `v20` fallback 과 reason 이 trace 에 남는다.
...
4. `cloneBuilder`
5. `quality pass/fail`
6. `mismatchPixelRatio`
7. `value marker count`
8. DOM layer 구성 여부
