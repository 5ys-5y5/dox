              ...
              if (isSignatureLabel(change.labelKey)) {
                if (!signatureAuthorization?.ok) {
                  return {
                    ...
                    afterValue: change.action === 'delete' ? null : change.value,
                    itemStatus: 'blocked' as const,
                    warningText: buildWarning(detail.document.title, change.labelKey, signatureAuthorization?.reason || ''),
                  };
                }
                ...
                if (signatureAuthorization.authentication.documentId !== documentId) {
                  return {
                    ...
                    warningText: buildWarning(
                      ...
                      change.labelKey,
                      `선택한 본인확인 기록은 다른 문서(${signatureAuthorization.authentication.documentTitle || signatureAuthorization.authentication.documentId || '알 수 없는 문서'})용입니다.`
                    ),
                  };
                }
