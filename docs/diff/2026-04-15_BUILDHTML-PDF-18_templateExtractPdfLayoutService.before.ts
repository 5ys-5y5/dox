...
const buildCloneStyleHtml = (version: '13' | '14' | '15' | '16' | '17') => `    <style>
...
${buildCloneStyleHtml(version)}
    <h1>작업지시서</h1>
${version === '17' ? buildUnifiedWorkOrderTableHtml(structured) : `${buildMetadataHtml(structured.metadataFields)}
...
export const TemplateExtractPdfLayoutService = {
  // BUILDHTML-PDF-16
  // text layer가 있는 작업지시서형 PDF는 v5의 form clone 방향을 유지하되,
  // raw heading 표기를 보존하고 wrapped heading 을 더 긴 section label 우선으로 병합합니다.
  buildCloneHtml(
    sourceTitle: string,
    rawText: string,
    layout?: TemplateExtractPdfLayoutModel | null,
