...
const buildMemberDocumentHref = (documentId: string, phoneNumber?: string | null) => {
  const normalizedDocumentId = documentId.trim();
  const normalizedPhoneNumber = normalizePhoneNumber(phoneNumber);
  const baseHref = `/member-access/document/${encodeURIComponent(normalizedDocumentId)}`;

  return normalizedPhoneNumber ? `${baseHref}?phoneNumber=${encodeURIComponent(normalizedPhoneNumber)}` : baseHref;
};
  ...
  const buildDocumentRows = React.useCallback(
    (documents: AccessibleDocument[]): MejaiScrollTableRow[] =>
      documents.map((document) => {
        const documentHref = buildMemberDocumentHref(document.documentId, session?.member.phoneNumber);
    ...
    [router, session?.member.phoneNumber]
  );
