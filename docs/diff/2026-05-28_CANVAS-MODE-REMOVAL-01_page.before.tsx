...
export default function CanvasOwnerPage() {
  ...
  const workspaceRuntime = selectedManagedPage.workspaceRuntime;
  ...
  const canEditCurrentWorkspace = workspaceRuntime !== 'read';
  ...
  React.useEffect(() => {
    ...
    if (rawPageId === selectedManagedPage.id && !rawMode) {
      return;
    }
  ...
  }, [selectedDocumentId, selectedManagedPage.id, selectedTemplateId, workspaceRuntime]);
  ...
  React.useEffect(() => {
    if (workspaceRuntime === 'template' || !selectedDocumentId) {
  ...
  }, [workspaceRuntime, selectedDocumentId]);
  ...
  const selectedDocumentInitialDraft = React.useMemo<TemplateEditWorkspaceInitialDraft | null>(() => {
    ...
    return {
      draftKey: `${selectedManagedPage.id}:${workspaceRuntime}:${selectedDocumentDetail.document.id}:${selectedDocumentDetail.latestVersion?.id || 'no-version'}:${buildDocumentHtmlContentKey(draftHtml)}:${draftReloadNonce}`,
  ...
  const effectiveCanvasToolbarVisibility = React.useMemo(
    () => buildCanvasToolbarVisibility(previewSettings, workspaceRuntime),
    [workspaceRuntime, previewSettings]
  );
  ...
  const routeEquivalentPreviewProps: CanvasRoutePreviewProps | null = (() => {
    ...
    if (selectedManagedPage.id === 'project') {
      return {
        ...
        saveDisabled: !canEditCurrentWorkspace || workspaceRuntime !== 'document',
        documentAttachmentApiPath:
          previewDocumentId && canEditCurrentWorkspace && workspaceRuntime === 'document'
    ...
    if (selectedManagedPage.id === 'request-links') {
      return {
        hideHeader: false,
        hidePersistencePanel: true,
        editableValueKeys: workspaceRuntime === 'document' ? effectiveEditableValueKeys : null,
        showWorkspaceMessages: true,
        suppressInitialDraftLoadedMessage: true,
        ...
        saveButtonLabel: '문서 저장',
        templateNameReadOnly: true,
        saveDisabled: !canEditCurrentWorkspace || workspaceRuntime !== 'document' || loadingDocumentDetail,
        documentAttachmentApiPath: '',
      };
    }
    ...
    return {
      ...
      nameFieldLabel: '문서 이름:',
      saveButtonLabel: workspaceRuntime === 'document' && canEditCurrentWorkspace ? '문서 저장' : '열람 전용',
      templateNameReadOnly: true,
      saveDisabled: !canEditCurrentWorkspace || workspaceRuntime !== 'document',
      documentAttachmentApiPath:
        canEditCurrentWorkspace && workspaceRuntime === 'document' && previewDocumentId
  ...
  const canvasOwnerPreviewBaseProps: CanvasRoutePreviewProps = {
    ...
    saveDisabled:
      !canEditCurrentWorkspace ||
      (workspaceRuntime === 'document' && loadingDocumentDetail),
  ...
  const previewTodoPanelEnabled = workspaceRuntime === 'document' && Boolean(selectedDocumentInitialDraft);
  ...
  const settingOutputViewBadges: Partial<Record<CanvasOwnerSettingKey, string[]>> = {
    hideHeader: ['공통'],
    hidePersistencePanel: ['템플릿', '문서'],
    templateListDisplay: ['템플릿'],
    showTopNotice: ['공통'],
    showWorkspaceMessages: ['공통'],
    showAdditionalControlPanels: ['템플릿'],
    showCanvasTitle: ['공통'],
    showCanvasNameField: ['템플릿'],
    showCanvasSaveButton: ['템플릿', '문서'],
    showCanvasTodoButton: ['문서'],
    showCanvasPreviewToggle: ['템플릿'],
    showCanvasInteractionModeControls: ['템플릿'],
    showCanvasHistoryControls: ['템플릿', '문서'],
    showCanvasZoomControls: ['공통'],
    showCanvasFullscreenControl: ['공통'],
    defaultCanvasFullscreen: ['공통'],
    autoCanvasHeight: ['공통'],
    autoCanvasWidth: ['공통'],
    specifiedCanvasHeight: ['공통'],
    specifiedCanvasWidth: ['공통'],
    showCanvasEditSettingsToggle: ['템플릿'],
    showCanvasSelectionPanelTabs: ['템플릿'],
    showPersistenceTemplateList: ['템플릿'],
    showPersistenceTemplateNameField: ['템플릿'],
    showPersistenceLayoutResizeModeField: ['템플릿'],
    showPersistenceSourceDocumentNameField: ['템플릿'],
    showPersistenceSaveButton: ['템플릿'],
    suppressInitialDraftLoadedMessage: ['문서', '읽기'],
    stabilizeInitialLayout: ['미리보기'],
    enableRuntimeInitialAutoSize: ['미리보기'],
    preventInitialValueClearShrink: ['미리보기'],
    preventRuntimeAutoSizeShrink: ['미리보기'],
    blockPeerClusterHeightTargets: ['미리보기'],
    blockPeerClusterWidthTargets: ['미리보기'],
    selectionInactiveOverlayOpacity: ['상자 선택'],
    readModeInteractionMode: ['읽기'],
    canvasViewMode: ['템플릿'],
    templateNameReadOnly: ['템플릿', '문서'],
    saveDisabled: ['템플릿', '문서'],
    enableDocumentAttachmentApiPath: ['문서'],
    limitEditableValueKeys: ['문서'],
    enableOnTemplateSaved: ['템플릿'],
    headerTitle: ['공통'],
    headerDescription: ['공통'],
    nameFieldLabel: ['템플릿', '문서'],
    saveButtonLabel: ['템플릿', '문서'],
  };
  const getSettingOutputViewBadges = (settingKey: CanvasOwnerSettingKey | null) =>
    settingKey ? settingOutputViewBadges[settingKey] || ['공통'] : [];
  const renderSettingBadges = (settingKey: CanvasOwnerSettingKey) => (
    <span className="inline-flex shrink-0 flex-wrap items-center justify-end gap-1">
      ...
      {getSettingOutputViewBadges(settingKey).map((badge) => (
        <span
          key={`${settingKey}:${badge}`}
          className="rounded border border-sky-200 bg-sky-50 px-1 py-px text-[9px] font-semibold leading-3 text-sky-700"
        >
          {badge}
        </span>
      ))}
  ...
  const effectiveTemplateWorkspacePropRows = [
    ...
    },
    {
      ...
      value: workspaceRuntime === 'template' ? selectedTemplateId || '-' : '-',
    ...
    },
    {
      ...
      value: workspaceRuntime === 'template' ? previewTemplateListDisplay : 'not passed',
    ...
    },
    {
      ...
      value: canEditCurrentWorkspace && workspaceRuntime === 'document' ? 'enabled' : 'disabled',
  ...
  const renderPageSettingsImportSection = () => (
    <div className="space-y-3">
      ...
      <div className="space-y-1.5" {...canvasOwnerEnv('canvasViewMode')}>
        <div className="flex items-center justify-between gap-2">
          <span className="text-[11px] font-semibold text-slate-800">상자 편집 캔버스 출력 뷰</span>
          {renderSettingBadges('canvasViewMode')}
        </div>
  ...
  const renderCanvasSizeSettings = () => (
    <div className="space-y-1.5">
      ...
      <div className="grid gap-2 lg:grid-cols-2">
        <div
          className={`space-y-2 rounded border px-2 py-1.5 ${getModeManagedClassName('autoCanvasHeight')}`}
        ...
        <div
          className={`space-y-2 rounded border px-2 py-1.5 ${getModeManagedClassName('autoCanvasWidth')}`}
  ...
  const renderCanvasSelectionOverlaySettings = () => {
    ...
    return (
      <div className="space-y-1.5">
        ...
        />
        <div
          className={`space-y-2 rounded border px-2 py-1.5 ${getModeManagedClassName('selectionInactiveOverlayOpacity')}`}
              ...
              {section.rows.map((row) => {
                ...
                return (
                  <SettingToggleRow
                    ...
                    outputViewBadges={getSettingOutputViewBadges(settingKey)}
                    checked={row.checked}
            ...
            <Card className="border-slate-200" {...canvasOwnerItem('document-picker-panel', '문서 선택 상태 패널')}>
              ...
              {workspaceRuntime === 'template' ? (
                <CanvasOwnedWorkspace
                  ...
                  workspaceMode={workspaceRuntime}
                ...
                <CanvasOwnedWorkspace
                  ...
                  onSaveDraftHtml={
                    canEditCurrentWorkspace && workspaceRuntime === 'document'
