...
const buildPositionedLineDescriptor = (page: TemplateExtractPdfPage, line: TemplateExtractPdfLine, lineIndex: number) => {
...
const buildRuleGuidedReplicaRuleSegments = (page: TemplateExtractPdfPage, rulePage: TemplateExtractPdfRulePage) => {
  ...
  if (explicitSegmentCount < Math.max(8, rowRules.length + columnRules.length - 4)) {
    ...
    for (const columnRule of [columnRules[0], columnRules[columnRules.length - 1]]) {
      appendReplicaRuleSegment(segments, seen, {
        orientation: 'v',
        left: columnRule,
        top: outerTop,
...
const findRuleBandByCenter = (axes: number[], center: number) => {
  for (let index = 0; index < axes.length - 1; index += 1) {
    if (center >= axes[index] && center <= axes[index + 1]) {
      return {
        start: axes[index],
        end: axes[index + 1],
      };
...
const buildRuleSnappedGeometrySegments = (
  page: TemplateExtractPdfPage,
  geometryPage: TemplateExtractPdfPageGeometry,
  rulePage: TemplateExtractPdfRulePage
) => {
  const rowRules = dedupeReplicaRuleAxis(rulePage.rowRules, page.height);
  const columnRules = dedupeReplicaRuleAxis(rulePage.columnRules, page.width);
  ...
  for (const row of geometryPage.rows) {
    ...
    const rowBand = findRuleBandByCenter(rowRules, row.top + row.height / 2);

    if (!rowBand || rowBand.end - rowBand.start <= 2) {
      continue;
    }
    ...
    for (const cell of row.cells) {
      ...
      for (let index = 0; index < columnRules.length - 1; index += 1) {
        const bandLeft = columnRules[index];
        const bandRight = columnRules[index + 1];
        const overlap = Math.min(cell.right, bandRight) - Math.max(cell.x, bandLeft);
        const bandWidth = bandRight - bandLeft;

        if (overlap >= Math.max(6, bandWidth * 0.25)) {
          matchingColumnIndexes.push(index);
        }
      }

      if (!matchingColumnIndexes.length) {
        continue;
      }

      const snappedLeft = columnRules[matchingColumnIndexes[0]];
      const snappedRight = columnRules[matchingColumnIndexes[matchingColumnIndexes.length - 1] + 1];
      const snappedHeight = Number((rowBand.end - rowBand.start).toFixed(2));
...
const buildReplicaRuleSegments = (
  ...
  const ruleSnappedGeometrySegments =
    framePolicy === 'rule_geometry_only' && geometryPage && rulePage
      ? buildRuleSnappedGeometrySegments(page, geometryPage, rulePage)
      : [];

  if (framePolicy === 'rule_geometry_only' && ruleSnappedGeometrySegments.length > 0) {
    return {
      segments: mergeReplicaRuleSegments([...ruleGuidedSegments, ...ruleSnappedGeometrySegments]),
...
const buildPageFrameDiagnostics = (
  ...
  return {
    ...
    maskVectorEnabled: framePolicy === 'mask_vector_priority',
    maskVectorPageCount: maskPageCount,
    maskRuleMinLength: framePolicy === 'mask_vector_priority' ? maskRuleMinLength : null,
  };
};
...
const buildPositionedPageHtml = (
  ...
  const lineDescriptors = page.lines
    .slice()
    .sort((left, right) => {
    ...
    .map((line, index) => buildPositionedLineDescriptor(page, line, index))
