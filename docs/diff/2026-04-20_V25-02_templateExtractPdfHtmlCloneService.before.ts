...
const resolvePdfFontFamily = (fontName: string | undefined) => {
  ...
  return `"${escapeAttribute(normalized)}", sans-serif`;
};

const resolvePdfFontWeight = (fontName: string | undefined) => (/bold|black|heavy/i.test(String(fontName || '')) ? '700' : '400');
...
const buildRuleTextSvgTextElement = (
  ...
  const pdfFontFamily = options.usePdfFontMetrics ? resolvePdfFontFamily(line.fontName) : null;
  const pdfFontWeight = options.usePdfFontMetrics ? resolvePdfFontWeight(line.fontName) : null;

  const clusteredNodes = mapClustersToTextNodes(buildTextClustersForLine(maskPage, page, line), rawText);
    ...
    return clusteredNodes
      .map((node, index) => {
        ...
        const fontSize = Number(
          Math.max(8.25, Math.min(pdfFontSize || height * 1.02, Math.max(height * 1.1, 36))).toFixed(2)
        );
        const baseline = Number((node.top + height - Math.max(1.2, height * 0.12)).toFixed(2));
        const fontAttributes = [
          pdfFontFamily ? `font-family="${pdfFontFamily}"` : null,
  ...
  const fontSize = Number(Math.max(8.25, Math.min(pdfFontSize || height * 0.98, 36)).toFixed(2));
  const baseline = Number((top + height - Math.max(1.4, height * 0.16)).toFixed(2));
  const fontAttributes = [
    pdfFontFamily ? `font-family="${pdfFontFamily}"` : null,
    pdfFontWeight ? `font-weight="${pdfFontWeight}"` : null,
  ]
  ...
  return `        <text class="${classes.join(' ')}" ${dataAttributes.join(
    ' '
  )} ${fontAttributes} x="${left}" y="${baseline}" font-size="${fontSize}" textLength="${width}" lengthAdjust="spacingAndGlyphs" xml:space="preserve">${escapeSvgText(
    rawText
  )}</text>`;
};
  ...
  buildMaskVectorCloneHtml(
    ...
    const pagesHtml = pages
      .map((maskPage) => {
        ...
        return layoutPage ? buildRuleTextSvgPageHtml(maskPage, layoutPage, options) : buildMaskVectorPageHtml(maskPage);
      })
      .join('\n');
