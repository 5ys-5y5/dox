...
export default function BulkOpsPage() {
  ...
  const handleCreateSignatureAuthentication = async () => {
    ...
    try {
      ...
      setSelectedAuthenticationId(authResult.data.authentication.id || '');
      setMessage('서명용 본인확인 요청을 생성했습니다. 이어서 검증 완료 처리를 눌러 verified 상태를 만드세요.');
    } catch (error) {
  ...
  const handleVerifySignatureAuthentication = async () => {
    ...
    if (!signatureAuthRequestId || !selectedAuthenticationId) {
    ...
    try {
      ...
      setSelectedAuthenticationId(verificationResult.data.authentication?.id || selectedAuthenticationId);
      setMessage('본인확인 검증이 완료되었습니다. 이제 미리보기를 다시 계산하면 서명 항목 반영 가능 여부를 확인할 수 있습니다.');
    } catch (error) {
            ...
            {hasSignatureLabels ? (
              <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                {selectedDocumentIds.length !== 1 ? (
                  ...
                  <>
                    <div className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-[1fr_auto_auto]">
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-800">서명자 이름</label>
                        <Input
                    ...
                    {signatureAuthStatus ? (
                      <p className="text-xs text-slate-500">
                        현재 서명용 인증 상태: {signatureAuthStatus}
