...
import { ExportRendererService } from './exportRendererService';
...
const getExportDownloadInfo = (row: ExportJobRow) => {
  const downloadSupported = row.target_format === 'pdf' || row.target_format === 'docx';
  const downloadReady = downloadSupported && row.status === 'completed' && Boolean(row.artifact_id);

  return {
    downloadSupported,
    downloadReady,
    downloadUrl: downloadReady ? `/api/exports/${row.id}/download` : null,
  };
};
...
const buildExportFileName = (title: string, versionNumber: number, format: ExportTargetFormat) => {
  const safeTitle = title.trim().replace(/[\\/:*?"<>|]+/g, '-').replace(/\s+/g, '_') || 'document';
  return `${safeTitle}-v${versionNumber}.${format}`;
};
...
export const ExportService = {
  async createExportJob(params: ExportJobCreateInput): Promise<ExportJobCreateResult> {
      ...
      .insert([
        {
          ...
          // documents.document_artifacts.status is owned by the documents schema and
          // only accepts queued/processing/ready/failed. Export job completion is tracked
          // separately in exports.export_job_registry.status.
          status: 'ready',
          metadata: {
    ...
    if (artifactError || !artifact) {
      await markExportJobFailed(
        client,
        exportJob.id,
        `artifact insert failed: ${artifactError?.message || 'unknown artifact insert error'}`
      );
    ...
    if (updateJobError || !updatedJobData) {
      await markExportJobFailed(
        client,
        exportJob.id,
        `export completion update failed: ${updateJobError?.message || 'unknown export completion update error'}`
      );
  ...
  async getExportDownloadPayload(exportJobId: string) {
    const normalizedExportJobId = exportJobId.trim();

    if (!normalizedExportJobId) {
      throw new Error('다운로드 실패: exportJobId가 필요합니다.');
    }
    ...
    if (exportJobError || !exportJob) {
      throw new Error(`다운로드 실패: ${exportJobError?.message || 'export job을 찾을 수 없습니다.'}`);
    }

    if (exportJob.target_format !== 'pdf' && exportJob.target_format !== 'docx') {
      throw new Error('다운로드 실패: 현재 구현에서는 PDF와 DOCX만 실제 다운로드를 지원합니다.');
    }

    if (exportJob.status !== 'completed' || !exportJob.artifact_id) {
      throw new Error('다운로드 실패: 아직 실제 다운로드 가능한 PDF 상태가 아닙니다.');
    }

    const { document, version } = await getDocumentAndVersion(client, exportJob.document_id, exportJob.version_id);
    const { data: artifactData, error: artifactError } = await documentsClient
      .from('document_artifacts')
      .select('*')
      .eq('id', exportJob.artifact_id)
    ...
    if (artifactError || !artifact) {
      throw new Error(`다운로드 실패: artifact 조회 중 오류가 발생했습니다. (${artifactError?.message})`);
    }
    ...
    const renderInput = {
      ...
      htmlCanonical: version.html_canonical,
    };
    const bytes =
      exportJob.target_format === 'pdf'
        ? ExportRendererService.renderPdf(renderInput)
        : ExportRendererService.renderDocx(renderInput);
    const mimeType =
      exportJob.target_format === 'pdf'
        ? 'application/pdf'
        : 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
    const actualBinaryMode =
      exportJob.target_format === 'pdf' ? 'on-demand-pdf-v1' : 'on-demand-docx-v1';
    ...
    await documentsClient
      ...
      .update({
        ...
        metadata: nextArtifactMetadata,
      })
      .eq('id', artifact.id);

    return {
      fileName: buildExportFileName(document.title, version.version_number, exportJob.target_format),
      mimeType,
      bytes,
    };
  },
