...
export type VerifiedAuthenticationSummary = {
  id: string;
  requestId: string;
...
const documentsSchema = (client = getSupabase()) => client.schema('documents');
...
const resolveDocumentTitles = async (client: ReturnType<typeof supabase>, documentIds: string[]) => {
  ...
  if (error) {
    throw new Error(`본인확인 요약 조회 실패: 문서 제목 조회 중 오류가 발생했습니다. (${error.message})`);
  }

  return new Map(((data || []) as DocumentRecordRow[]).map((row) => [row.id, row.title]));
};

const buildVerifiedAuthenticationSummaries = async (
  client: ReturnType<typeof supabase>,
  authentications: SignAuthenticationRow[]
): Promise<VerifiedAuthenticationSummary[]> => {
  ...
  if (requestRowsError) {
    throw new Error(`본인확인 요약 조회 실패: sign_requests 조회 중 오류가 발생했습니다. (${requestRowsError.message})`);
  }

  const requestRows = (requestRowsData || []) as SignRequestRow[];
  const requestMap = new Map(requestRows.map((row) => [row.id, row]));
  const documentTitleMap = await resolveDocumentTitles(
    client,
    requestRows.map((row) => row.document_id || '').filter(Boolean)
  );

  return authentications.map((row) => {
    const requestRow = requestMap.get(row.request_id);
    const documentId = requestRow?.document_id || null;
    const signerName =
      typeof requestRow?.signer_info?.name === 'string' ? requestRow.signer_info.name.trim() : null;
    const providerDefinition = getAuthProviderDefinition(row.provider_group, row.provider);
    ...
    return {
      ...
      providerProduct: row.provider_product,
      providerLabel: providerDefinition?.label || `${row.provider_group}/${row.provider}`,
      verifiedAt: row.verified_at || row.completed_at || row.requested_at,
...
export const SignAuthService = {
  async listVerifiedAuthentications(params?: { limit?: number }) {
    const client = supabase();
    const limit = Math.min(Math.max(params?.limit || 12, 1), 50);
    const { data, error } = await signingSchema(client)
      .from('sign_authentications')
      .select(
        'id, request_id, provider_group, provider, provider_product, auth_status, requested_at, completed_at, verified_at, expires_at'
      )
    ...
    if (error) {
      throw new Error(`본인확인 목록 조회 실패: ${error.message}`);
    }

    return buildVerifiedAuthenticationSummaries(client, (data || []) as SignAuthenticationRow[]);
  },

  async getVerifiedAuthenticationContext(authenticationId: string) {
    const normalizedId = authenticationId.trim();

    if (!normalizedId) {
      throw new Error('본인확인 조회 실패: authenticationId가 필요합니다.');
    }
    ...
    if (error || !data) {
      throw new Error(`본인확인 조회 실패: ${error?.message || 'verified 인증 레코드를 찾을 수 없습니다.'}`);
    }

    const summaries = await buildVerifiedAuthenticationSummaries(client, [data as SignAuthenticationRow]);
