...
const buildRuleTextHtmlCharacterSpans = (
  ...
  return line.characters
    .map((character, characterIndex) => {
      ...
      const style = [
        `left:${Number(character.x.toFixed(2))}px`,
        `top:${toReplicaCharacterTop(page, character)}px`,
  ...
  return runs
    .map((run) => {
      ...
      const style = [
        ...
        `line-height:${Math.max(run.height, run.fontSize).toFixed(2)}px`,
        typeof spacingValue === 'number' && Number.isFinite(spacingValue) && Math.abs(spacingValue) > 0.001
          ? `letter-spacing:${Number(spacingValue.toFixed(4))}px`
          : null,
      ]
        .filter(Boolean)
        .join(';');
...
const buildRuleTextHtmlAbsolutePageHtml = (
  ...
  const ruleHtml = buildMaskRuleSegments(maskPage)
    .map((segment) => {
      const style = [
        `left:${Number((segment.x * scaleX).toFixed(2))}px`,
        `top:${Number((segment.y * scaleY).toFixed(2))}px`,
        `width:${Number((segment.width * scaleX).toFixed(2))}px`,
        `height:${Number((segment.height * scaleY).toFixed(2))}px`,
      ].join(';');
