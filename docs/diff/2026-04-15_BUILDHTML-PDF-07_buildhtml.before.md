...
4. PDF 텍스트 추출 방식의 구조 손실
- 현재 PDF는 `PDFKit page.string` 기반이라 위치 정보가 사라진다.
- 위치 정보가 사라진 상태에서 원문 배치를 HTML로 복제하는 것은 원리적으로 불가능하다.
...
### BUILDHTML-05
목표:
- 특정 문서형 재조립이 아니라, 일반 PDF에도 적용 가능한 좌표 기반 absolute HTML clone 을 도입한다.

완료 기준:
- PDF 결과가 메타/본문 재조립 표가 아니라 페이지 줄 배치를 유지한 absolute HTML clone 으로 나온다.
- work-order 류가 아닌 다른 PDF도 최소한 “원문 줄 위치를 따라간 clone” 으로 표시된다.
...
따라서 후속 구현은 후보 개수 조정이나 label 튜닝보다, 반드시 `BUILDHTML-02~04`를 우선해야 한다.
