...
const buildTopologySummaryToken = (summary: TemplateExtractPdfPipelineTrace['topologySummary']) =>
  [
    `p${summary.pageCount}`,
    `rb${summary.rowBandCount}`,
    `ce${summary.columnEdgeCount}`,
    `hs${summary.horizontalSegmentCount}`,
    `vs${summary.verticalSegmentCount}`,
    `tb${summary.textBlockCount}`,
    `cc${summary.cellCandidateCount}`,
  ].join('-');
...
const buildPipelineTrace = (
  engineVersion: Extract<TemplateExtractEngineVersion, '19' | '20'>,
  detection: TemplateExtractPdfFamilyDetectionResult,
  ...
  cloneBuilder: inferCloneBuilder(detection),
});
...
const attachPipelineTraceToHtml = (html: string, pipelineTrace: TemplateExtractPdfPipelineTrace) => {
  const attrs = [
    ...
    `data-template-family-reasons="${escapeAttribute(pipelineTrace.familyDetectionReasons.join('|'))}"`,
    `data-template-topology-summary="${buildTopologySummaryToken(pipelineTrace.topologySummary)}"`,
    `data-template-clone-builder="${pipelineTrace.cloneBuilder}"`,
  ].join(' ');

  if (!html.includes('data-template-extract-draft="true"')) {
    return html;
  }

  return html.replace('<section data-template-extract-draft="true"', `<section data-template-extract-draft="true" ${attrs}`);
};
...
const buildResolvedHtmlSource = (
  ...
  sourceTitle,
  sourceKind: 'html',
  sourceContent: attachPipelineTraceToHtml(html, pipelineTrace),
...
const extractPdfSourceV20 = async (fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> => {
  ...
  if (layoutDetection.sourceMode === 'digital') {
    ...
    const clonedHtml =
      layoutDetection.documentFamily === 'work_order'
        ? TemplateExtractWorkOrderBuilderService.buildFromDigitalLayout(sourceTitle, layout, '20')
        : TemplateExtractPdfLayoutService.buildGenericCloneHtml(sourceTitle, layout);

    if (clonedHtml) {
      return buildResolvedHtmlSource(fileName, sourceTitle, clonedHtml, buildPipelineTrace('20', layoutDetection, topology));
    }
    ...
    if (layout.rawText.trim()) {
      return buildResolvedTextSource(
        ...
        buildPipelineTrace('20', layoutDetection, topology)
      );
    }
  }
  ...
  const recoveredHtml =
    ruleDetection.documentFamily === 'work_order'
      ? TemplateExtractWorkOrderBuilderService.buildFromRuleModel(sourceTitle, fileName, ruleModel, '20')
      : TemplateExtractPdfTextRecoveryService.buildHtmlFromRuleModel(sourceTitle, fileName, ruleModel, '20');

  if (recoveredHtml) {
    return buildResolvedHtmlSource(fileName, sourceTitle, recoveredHtml, buildPipelineTrace('20', ruleDetection, topology));
  }
  ...
  if (layout.rawText.trim()) {
    return buildResolvedTextSource(
      ...
      sourceTitle,
      normalizePdfFallbackText(layout.rawText),
      buildPipelineTrace('20', ruleDetection, topology)
    );
  }
...
export const TemplateExtractPdfService = {
  ...
  async extractPdfSource(
    fileName: string,
    bytes: Uint8Array,
    version: Extract<TemplateExtractEngineVersion, '19' | '20'> = '19'
