...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  try {
    ...
    const hintMessage =
      ...
      const manualReason = formatUnknownError(manualError);

      throw new Error(
        `시각 유사도 측정 실패: foreignObject 캡처 후 canvas 를 읽을 수 없어 manual canvas 로 재시도했지만 같은 문제가 발생했습니다. foreignObject=${foreignObjectReason}; manual=${manualReason}.${hintMessage}`
      );
    }
  }
