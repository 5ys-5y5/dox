...
- `/project`에서는 `/canvas`의 현장 관리 페이지별 캔버스 설정 적용 여부를 ON/OFF로 전환할 수 있어야 한다.
...
| `src/app/documents/_owner/DocumentsOwnerWorkspace.tsx` | `current-work-panel` 단독 렌더 모드와 저장된 canvas owner 설정 적용 여부 연결 |
| `src/app/documents/_owner/index.ts` | 새 타입 export가 필요할 때만 수정 |
| `src/app/project/page.tsx` | owner 컴포넌트 배치 변경, 기존 하단 캔버스 자리 교체, 페이지별 설정 ON/OFF UI 추가 |
...
### 5. `/project` 페이지별 설정 ON/OFF

`/project`에는 `/canvas`에서 저장한 현장 관리 페이지별 캔버스 설정을 적용할지 정하는 ON/OFF 설정을 둔다.

- 설정 라벨: `페이지별 캔버스 설정`
- 저장 키: `project.canvasOwnerSettingsEnabled.v1`
- 기본값: `ON`
- `ON`: `DocumentsOwnerWorkspace` 내부의 `CanvasOwnedWorkspace`가 `/canvas`의 `project/read` 페이지 설정을 적용한다.
- `OFF`: 저장된 page/mode/role 설정을 적용하지 않고 `DocumentsOwnerWorkspace`가 전달한 기본 props만 사용한다.
- 이 설정은 `/project` 화면의 표시 선택이며 `/canvas`에 저장된 설정값 자체를 삭제하거나 수정하지 않는다.
...
- [x] `DocumentsOwnerWorkspaceProps.applyStoredCanvasOwnerSettings`를 추가했다.
...
- [x] `/project`에 `페이지별 캔버스 설정` ON/OFF UI를 추가했다.
- [x] `페이지별 캔버스 설정` OFF 시 `DocumentsOwnerWorkspace` 내부 캔버스에 저장된 canvas owner 설정을 적용하지 않는다.
...
- 2026-05-24: `/project`에 `페이지별 캔버스 설정` ON/OFF를 추가하고, `DocumentsOwnerWorkspace`가 내부 `CanvasOwnedWorkspace`에 `applyStoredCanvasOwnerSettings`를 전달하도록 했다.
