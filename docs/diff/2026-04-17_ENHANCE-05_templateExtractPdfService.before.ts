...
const extractPdfSourceV21 = async (fileName: string, bytes: Uint8Array): Promise<TemplateExtractResolvedSource> => {
  const layout = await extractPdfLayoutModel(fileName, bytes);
  const sourceTitle = fileName.replace(/\.pdf$/i, '').trim() || '업로드 PDF';
  const fallbackReason = 'v21_scene_replica_not_implemented';
  const layoutDetection = TemplateExtractPdfFamilyService.detectFromLayout(sourceTitle, fileName, layout);
  ...
  if (layoutDetection.sourceMode === 'digital') {
    ...
    if (clonedHtml) {
      const qualityReport = buildOfflineQualityReport(layout.rawText, layout, topology, clonedHtml, fallbackReason);
      return buildResolvedHtmlSource(fileName, sourceTitle, clonedHtml, pipelineTrace, qualityReport);
    }
  ...
  if (recoveredHtml) {
    const qualityReport = buildOfflineQualityReport(ruleModel.rawText || layout.rawText, layout, topology, recoveredHtml, fallbackReason);
    return buildResolvedHtmlSource(fileName, sourceTitle, recoveredHtml, pipelineTrace, qualityReport);
  }

  if (layout.rawText.trim()) {
    const qualityReport = buildOfflineQualityReport(ruleModel.rawText || layout.rawText, layout, topology, '', fallbackReason);
    return buildResolvedTextSource(
      fileName,
      sourceTitle,
      normalizePdfFallbackText(layout.rawText),
