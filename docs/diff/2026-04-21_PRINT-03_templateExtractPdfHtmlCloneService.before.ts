...
const buildReplicaRulesHtml = (
  ...
  buildReplicaRuleSegments(page, geometryPage, rulePage, maskPage, maskRuleMinLength)
    .map((segment) =>
      segment.orientation === 'h'
...
const buildReplicaFrameRenderSegments = (
  page: TemplateExtractPdfPage,
  geometryPage: TemplateExtractPdfPageGeometry | null,
  rulePage: TemplateExtractPdfRulePage | null,
  maskPage: TemplateExtractPdfMaskVectorPage | null,
  ...
  buildReplicaRuleSegments(page, geometryPage, rulePage, maskPage, maskRuleMinLength).map((segment) =>
    segment.orientation === 'h'
      ? {
          orientation: 'h',
          left: segment.left,
...
const buildPositionedPageHtml = (
  ...
  const rulesHtml = buildReplicaRulesHtml(page, geometryPage, rulePage, maskPage, maskRuleMinLength, precisionScale);
  const frameSegments = buildReplicaFrameRenderSegments(page, geometryPage, rulePage, maskPage, maskRuleMinLength);
...
const buildPositionedSvgPageHtml = (
  ...
  const textHtml = page.lines
    ...
    .filter(Boolean)
    .join('\n');
  const ruleHtml = buildReplicaRuleSegments(page, geometryPage, rulePage, maskPage)
    .map((segment) =>
      segment.orientation === 'h'
...
${pagesHtml}
  ...
  buildPositionedCloneHtml(
    ...
    return `<section data-template-extract-draft="true" data-template-clone="${cloneId}">
