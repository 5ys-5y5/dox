...
export default function TemplateExtractPage() {
  ...
  const handleMeasureVisualSimilarity = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/templates/extract/measure', {
        method: 'POST',
        body: formData,
      });
      const result = await response.json();
      ...
      if (!result.success) {
        throw new Error(result.message || 'PDF 렌더 이미지를 생성하지 못했습니다.');
      }

      const pdfPageDataUrls = Array.isArray(result.data?.pageImages) ? result.data.pageImages : [];

      if (pdfPageDataUrls.length === 0) {
        throw new Error('PDF 페이지 이미지를 생성하지 못했습니다.');
      }
  ...
  }, [draftDetail, handleMeasureVisualSimilarity, selectedFile, visualSimilarityMeasuring]);
          ...
          <Card className="border-slate-200">
            <CardHeader className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {visualSimilarityMeasuring ? '시각 유사도 측정 중' : '시각 유사도 측정'}
                </Button>
                  ...
                  {qualityReport ? (
                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                      ...
                      <p className="text-[11px] text-amber-700">
                        현재 값은 PDF와 HTML을 같은 렌더 조건으로 비교한 `1px 이내 픽셀 중첩률`이 아닙니다. 따라서 시각 유사도 점수로 사용하면 안 됩니다.
                      </p>
                      {!visualSimilarityReport ? <p>visualSimilarity: 미측정</p> : null}
