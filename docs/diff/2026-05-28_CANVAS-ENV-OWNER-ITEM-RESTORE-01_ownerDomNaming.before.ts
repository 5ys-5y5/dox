...
const OWNER_SETTING_DEFINITION_ATTR = 'data-canvas-owner-setting-definition';
...
const findNearestOwnerElement = (
  ...
  while (parent && parent !== root.parentElement) {
    if (
      parent.hasAttribute(itemAttribute) &&
      parent.getAttribute(autoNamedAttribute) !== 'true' &&
      !parent.hasAttribute(OWNER_SETTING_DEFINITION_ATTR)
    ) {
      return parent;
    }
