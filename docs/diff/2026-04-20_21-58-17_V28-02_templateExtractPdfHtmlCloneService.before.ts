...
const buildReplicaFrameRenderSegments = (
  ...
  buildReplicaRuleSegments(page, geometryPage, rulePage).map((segment) =>
    segment.orientation === 'h'
      ? {
          orientation: 'h',
          left: segment.left,
...
const buildPositionedPageHtml = (
  ...
  const rulesHtml = buildReplicaRulesHtml(page, geometryPage, rulePage, precisionScale);
  const frameSegments = buildReplicaFrameRenderSegments(page, geometryPage, rulePage);
