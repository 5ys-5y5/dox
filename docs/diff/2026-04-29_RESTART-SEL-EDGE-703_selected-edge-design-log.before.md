...
3. isolated mode 이면 `targetEdgeIds` 는 강제로 clicked edge 1개만 반환한다.
...
8. 실제 localhost giant shared band 를 row-range shell 로 분해하는 `ensurePreviewFrameBandNormalization()` 을 추가했다.
9. 정규화는 `buildLiveEdgeTopologySnapshot()` 이전, 그리고 editor state 적용 effect 이전에 실행된다.
10. 분해된 shell 은 `data-v106-normalized-band="true"` 로 마킹되어 이후 topology / resize 계산이 정규화된 shell 기준으로만 수행된다.
...
- `mcp__chrome_devtools__`
  ...
  - 실측 2: 새 코드 반영 후 root 하위 `.v102-frame-band` 는 20개, `data-v106-normalized-band="true"` shell 은 17개였다
  - 실측 3: `band-3-cell-2:left` 1회 클릭 시 `connected`, edge 수 `2`, anchor `band-3-cell-2:left`
  - 실측 4: 같은 edge 2회 클릭 시 `isolated`, edge 수 `1`, anchor `band-3-cell-2:left`
  - 실측 5: devtools 는 직접 drag primitive 를 제공하지 않아 synthetic pointer / React capture handler 경로로 drag automation 을 시도했다
  - 실측 6: click selection 전이와 giant shell 정규화는 확인했지만, 최종 drag 동작은 human 수동 drag 로 한 번 더 확인이 필요하다
...
3. 이번 턴 자동 검증 범위는 문서, 타입체크, 컴포넌트 번들, localhost giant shell 정규화 확인, click selection 전이 확인, `supabase` MCP 실패 사유 기록까지다.
