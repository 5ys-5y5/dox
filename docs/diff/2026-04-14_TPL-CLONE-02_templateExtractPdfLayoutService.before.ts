...
const buildStatusHtml = (statusLines: string[]) => {
  ...
  return `    <div class="template-clone__status-strip">
${statusLines.map((line) => `      <p>${escapeHtml(line)}</p>`).join('\n')}
    </div>`;
};
...
const buildSectionValueHtml = (section: PdfSectionBlock) => {
  if (section.asList) {
    return `<div class="template-clone__section-value" data-template-value="${escapeHtml(section.label)}">
        <ul>
${section.valueLines.map((line) => `          <li>${escapeHtml(line)}</li>`).join('\n')}
        </ul>
      </div>`;
  }

  return `<div class="template-clone__section-value" data-template-value="${escapeHtml(section.label)}">
${section.valueLines.map((line) => `        <p>${escapeHtml(line)}</p>`).join('\n')}
      </div>`;
};
...
const buildSectionsHtml = (sections: PdfSectionBlock[]) =>
  sections
    .map(
      (section) => `    <section class="template-clone__section">
      <header class="template-clone__section-header">
        <span class="template-clone__section-number">${escapeHtml(section.number)}.</span>
        <h2>${escapeHtml(section.label)}</h2>
      </header>
${buildSectionValueHtml(section)}
    </section>`
    )
    .join('\n');
...
const buildFooterHtml = (footerNotes: string[]) => {
  ...
  return `    <footer class="template-clone__footer">
