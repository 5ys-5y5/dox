...
type SelectionStylePatch = Partial<Omit<SelectionStyleDraft, 'width' | 'height'>> & {
  width?: number;
  height?: number;
};
...
type ResizeState = {
  ...
  edgeResizeIntent?: TemplateEdgeResizeIntentDto;
...
type EdgePressState = {
  ...
  predictedActivationResult: TemplateSelectedEdgeActivationResultDto;
  withShift: boolean;
};
...
export default function TemplateEditWorkspace({ initialTemplateId = '' }: TemplateEditWorkspaceProps) {
  ...
  const commitSelectionState = React.useCallback(
    (nextSelectedFrameGroupIds: string[], nextEdgeSelectionState: TemplateEdgeSelectionStateDto) => {
      const normalizedEdgeSelection = previewRef.current
        ? reconcileLiveEdgeSelection(previewRef.current, nextEdgeSelectionState)
        : nextEdgeSelectionState;

      applyRuntimeSelectionUi(nextSelectedFrameGroupIds, normalizedEdgeSelection);
      setSelectedFrameGroupIds(nextSelectedFrameGroupIds);
      setEdgeSelectionState(normalizedEdgeSelection);
    },
    [applyRuntimeSelectionUi, reconcileLiveEdgeSelection]
  );
  ...
  const collectEdgeResizeTargets = React.useCallback(
    (root: HTMLElement, snapshot: TemplateEdgeTopologySnapshotDto, resizeIntent: TemplateEdgeResizeIntentDto) => {
      const targets: EdgeResizeTarget[] = [];

      resizeIntent.targetEdgeIds.forEach((edgeId) => {
        ...
        const target = {
          ...
          node,
          side: edge.side,
          mode: resizeIntent.activatedMode,
          widthInstructions:
            edge.side === 'left' || edge.side === 'right'
              ? [buildSelfWidthResizeInstruction(context, edge.side) as FrameWidthResizeInstruction].filter(Boolean)
              : undefined,
        };
  ...
  const clearFrameSelection = React.useCallback(() => {
    ...
    commitSelectionState([], TemplateEdgeSelectionService.createEmptyState());
  }, [commitSelectionState, stopPointerInteraction]);
  ...
  const handlePreviewPointerDown = React.useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (explicitEdgeSide && pageInner) {
        ...
        const predictedActivationResult = TemplateEdgeSelectionService.resolveActivation({
          snapshot,
          currentSelection,
          clickedEdgeId,
  ...
  const handlePreviewPointerMove = React.useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!resizeState && edgePressState && event.pointerId === edgePressState.pointerId) {
      ...
      const resizeIntent = TemplateEdgeResizeIntentService.resolveResizeIntent({
        snapshot: edgePressState.snapshot,
        activationResult: edgePressState.predictedActivationResult,
        clickedEdgeId: edgePressState.clickedEdgeId,
        side: edgePressState.side,
        pointerDeltaPx: edgePressState.side === 'left' || edgePressState.side === 'right' ? rawDeltaX : rawDeltaY,
      });

      if (resizeIntent.blockedReason !== 'none' || resizeIntent.targetEdgeIds.length === 0) {
        return;
      }
      ...
      applyRuntimeSelectionUi([], resizeIntent.effectiveSelectionState);
      resizeStateRef.current = {
    ...
    if (resizeState && event.pointerId === resizeState.pointerId) {
      ...
      if (resizeState.edgeResizeTargets && resizeState.edgeResizeTargets.length > 0) {
        ...
        resizeState.edgeResizeTargets.forEach((target) => {
          if (
            resizeState.edgeResizeIntent &&
            !resizeState.edgeResizeIntent.targetEdgeIds.includes(target.edgeId)
          ) {
            return;
          }
  ...
  }, [applyRuntimeSelectionUi, getFrameNodes, collectEdgeResizeTargets]);
  ...
  const handlePreviewPointerUp = React.useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (edgePressState?.pointerId === event.pointerId) {
        ...
        const nextEdgeSelection = TemplateEdgeSelectionService.reconcileSelectionState({
          snapshot,
          currentSelection: edgePressState.predictedActivationResult.nextSelectionState,
        });
        const owner = activePointerOwnerRef.current;
        ...
        activePointerOwnerRef.current = null;
        edgePressStateRef.current = null;
        commitSelectionState([], nextEdgeSelection);
        return;
      }
    ...
    [buildLiveEdgeTopologySnapshot, commitSelectionState, stopPointerInteraction]
  );
