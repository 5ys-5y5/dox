...
const buildRuleGuidedReplicaRuleSegments = (page: TemplateExtractPdfPage, rulePage: TemplateExtractPdfRulePage) => {
  ...
  const boundedSegments = segments.filter((segment) =>
    segment.orientation === 'h'
      ? segment.top >= outerTop - 2 &&
        segment.top <= outerBottom + 2 &&
        segment.left + segment.width >= outerLeft &&
        segment.left <= outerRight
      : segment.left >= outerLeft - 2 &&
        segment.left <= outerRight + 2 &&
        segment.top + segment.height >= outerTop &&
        segment.top <= outerBottom
  );
  segments.length = 0;
  seen.clear();

  for (const segment of boundedSegments) {
    appendReplicaRuleSegment(segments, seen, segment);
  }
