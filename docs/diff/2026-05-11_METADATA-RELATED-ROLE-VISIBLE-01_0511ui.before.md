...
| `SELECTION-TONEDOWN-01` | 선택 톤다운 방식 변경 전 원본을 `docs/diff`에 백업한다. | `docs/diff/2026-05-11_SELECTION-TONEDOWN-01_*.before.*` | 완료 |
| `SELECTION-TONEDOWN-02` | 위치/속성 탭의 미선택 10% opacity를 폐기하고, 90% 흰색 편집기 overlay로 비대상 항목을 덮는다. | `TemplateEditWorkspace.before.tsx` | 완료 |
| `SELECTION-TONEDOWN-03` | 선택/연관 항목은 overlay보다 높은 z-index로 올려 항상 비대상 위에 출력한다. | `TemplateEditWorkspace.before.tsx` | 완료 |
| `SELECTION-TONEDOWN-04` | 정적 검증, chrome-devtools 위치/속성 탭 검증, Supabase MCP 제한 사항을 테스트 기록에 남긴다. | 테스트 기록 | 완료 |
| `METADATA-SELECTION-RELATION-01` | 속성 탭 관계/선택 표시 수정 전 원본을 `docs/diff`에 백업한다. | `docs/diff/2026-05-11_METADATA-SELECTION-RELATION-01_*.before.*` | 완료 |
| `METADATA-SELECTION-RELATION-02` | 속성 탭 관계 connector/dot/outline은 항목 선택 여부와 무관하게 미선택 상태와 같은 수로 유지한다. | `TemplateEditWorkspace.before.tsx` | 대체됨: `METADATA-SELECTION-SCOPED-02` 기준으로 폐기 |
| `METADATA-SELECTION-RELATION-03` | 속성 탭 선택 항목은 크기 및 위치 탭처럼 선택 번호와 primary 선택 윤곽선/그림자를 출력한다. | `TemplateEditWorkspace.before.tsx` | 완료 |
| `METADATA-SELECTION-RELATION-04` | 정적 검증, chrome-devtools 속성 탭 미선택/선택 비교 검증, Supabase MCP 제한 사항을 테스트 기록에 남긴다. | 테스트 기록 | 완료 |
| `METADATA-SELECTION-SCOPED-01` | 속성 탭 선택 관계 범위 재수정 전 원본을 `docs/diff`에 백업한다. | `docs/diff/2026-05-11_METADATA-SELECTION-SCOPED-01_*.before.*` | 완료 |
| `METADATA-SELECTION-SCOPED-02` | 속성 탭 미선택 상태는 전체 관계를 표시하고, 선택 상태는 선택 항목이 포함된 관계만 표시한다. | `TemplateEditWorkspace.before.tsx` | 완료 |
| `METADATA-SELECTION-SCOPED-03` | 선택 value는 해당 value와 부모 key만, 선택 key는 해당 key와 소유 value 관계만 남긴다. | `TemplateEditWorkspace.before.tsx` | 대체됨: `METADATA-VALUE-FAMILY-02` 기준으로 value 형제 관계 포함 |
| `METADATA-SELECTION-SCOPED-04` | 정적 검증, chrome-devtools 선택 범위 검증, Supabase MCP 제한 사항을 테스트 기록에 남긴다. | 테스트 기록 | 완료 |
| `METADATA-VALUE-FAMILY-01` | 속성 탭 value 관계 범위 및 관계 레이어 수정 전 원본을 `docs/diff`에 백업한다. | `docs/diff/2026-05-11_METADATA-VALUE-FAMILY-01_*.before.*` | 완료 |
| `METADATA-VALUE-FAMILY-02` | value 선택 시 부모 key와 해당 부모 key가 소유한 모든 value 관계를 표시한다. | `TemplateEditWorkspace.before.tsx` | 완료 |
| `METADATA-VALUE-FAMILY-03` | 관계 line/dot은 tone-down overlay와 선택/관계 상자 위에 보이도록 관계 overlay z-index를 높인다. | `TemplateEditWorkspace.before.tsx` | 완료 |
| `METADATA-VALUE-FAMILY-04` | 정적 검증, chrome-devtools value 관계 범위 및 z-index 검증, Supabase MCP 제한 사항을 테스트 기록에 남긴다. | 테스트 기록 | 완료 |
...
선택 톤다운 overlay 추가 변경:

- 2026-05-11: `SELECTION-TONEDOWN-01` 백업 완료.
  - `docs/diff/2026-05-11_SELECTION-TONEDOWN-01_TemplateEditWorkspace.before.tsx`
  - `docs/diff/2026-05-11_SELECTION-TONEDOWN-01_0511ui.before.md`
- 2026-05-11: 위치/속성 탭에서 비대상 항목을 `opacity: .1`로 직접 낮추는 방식을 폐기했다. 비대상 상자는 원래 opacity 1을 유지하고, `.page-inner`의 편집기 전용 `data-v106-selection-tonedown-overlay="true"` 흰색 overlay가 `rgba(255, 255, 255, .9)`로 덮는다.
- 2026-05-11: 선택/연관 항목은 `z-index: 31`, 위치 그룹 proxy 선택 overlay는 `z-index: 32`, 흰색 tone-down overlay는 `z-index: 30`으로 배치한다. 이 기준은 겹침 여부와 무관하게 비대상 항목이 같은 기준으로 낮아지고 선택/연관 항목이 항상 위에 출력되도록 하기 위한 것이다.
- 2026-05-11: 추가 검증으로 `npx esbuild src/app/templates/edit/page.tsx --bundle --platform=browser --format=esm --jsx=automatic --log-level=warning --outfile=/tmp/template-edit-page-check.js`, `npm run check:no-shadow-app`, `git diff --check -- src/components/template/TemplateEditWorkspace.tsx docs/0511ui.md`를 실행했고 모두 통과했다.
- 2026-05-11: chrome-devtools MCP에서 `http://localhost:3001/templates/edit?templateId=d3a38b9c-2603-4bc4-88e6-6b15fcfd0c40`를 reload한 뒤 접근성 스냅샷의 `B90467CA-12` 텍스트 상자를 화면 클릭 방식으로 눌러 위치 탭을 확인했다.
  - 위치 탭 그룹 proxy 상태: `positionFilter=true`, `activeCount=51`, `inactiveCount=3`, `tonedownOverlayCount=3`, `overlayModes=["position"]`, overlay background `rgba(255, 255, 255, 0.9)`, inactive opacity `1`, active z-index `31`, proxy z-index `32`, `opacityPointOneCount=0`.
- 2026-05-11: chrome-devtools MCP에서 `속성` 탭으로 전환해 같은 기준을 확인했다.
  - 속성 탭 상태: `metadataFilter=true`, `positionFilter=null`, `activeCount=52`, `inactiveCount=2`, `tonedownOverlayCount=2`, `overlayModes=["metadata"]`, overlay background `rgba(255, 255, 255, 0.9)`, inactive opacity `1`, active z-index `31`, `positionOverlayCount=0`, `opacityPointOneCount=0`.
- 2026-05-11: chrome-devtools MCP 콘솔 확인 결과 error/warn 메시지는 없었다.
- 2026-05-11: `supabase` MCP는 현재 세션에 노출되지 않았다. 이번 변경은 DB 스키마/데이터 변경이 없으므로 사용자 실행 SQL도 없다.

속성 탭 관계/선택 표시 추가 변경:

- 2026-05-11: `METADATA-SELECTION-RELATION-01` 백업 완료.
  - `docs/diff/2026-05-11_METADATA-SELECTION-RELATION-01_TemplateEditWorkspace.before.tsx`
  - `docs/diff/2026-05-11_METADATA-SELECTION-RELATION-01_0511ui.before.md`
- 2026-05-11: 속성 탭의 관계 connector 렌더링 조건에서 “선택된 관계만 connector 유지” 제한을 제거했다. 이 구현은 사용자가 확정한 최종 기준과 달라 `METADATA-SELECTION-SCOPED-02`로 대체했다.
- 2026-05-11: 속성 탭에서 선택된 항목의 outline과 번호 배지를 숨기던 CSS suppression을 제거하고, metadata visual mode에서도 `data-template-selected="true"` 항목이 크기 및 위치 탭과 같은 선택 번호와 primary 선택 윤곽선/그림자를 갖도록 override했다.
- 2026-05-11: 관계 connector line/dot은 tone-down overlay와 같은 z-index plane에 후속 append되어 overlay 아래로 사라지지 않게 하고, 선택 항목은 z-index `31`로 그 위에 둔다.
- 2026-05-11: 추가 검증으로 `npx esbuild src/app/templates/edit/page.tsx --bundle --platform=browser --format=esm --jsx=automatic --log-level=warning --outfile=/tmp/template-edit-page-check.js`, `npm run check:no-shadow-app`, `git diff --check -- src/components/template/TemplateEditWorkspace.tsx docs/0511ui.md`를 실행했고 모두 통과했다.
- 2026-05-11: chrome-devtools MCP에서 `속성` 탭으로 전환한 뒤 미선택 상태를 확인했다. `selectedCount=0`, `connectorLines=27`, `connectorDots=52`, `relationOutlines=52`, `metadataFilter=null`, `tonedownOverlayCount=0`.
- 2026-05-11: chrome-devtools MCP 접근성 스냅샷의 `B90467CA-12` 텍스트 상자를 화면 클릭 방식으로 선택해 확인했다. `div` selector를 직접 타겟하지 않았다.
  - 선택 상태: `selectedCount=1`, `connectorLines=27`, `connectorDots=52`, `relationOutlines=52`, `metadataFilter=true`, `activeCount=3`, `inactiveCount=51`, `tonedownOverlayCount=51`, `opacityPointOneCount=0`.
  - 선택 항목 `band-5-cell-2`: `outline=2px solid rgba(13, 148, 136, 0.98)`, `beforeContent="1"`, `beforeDisplay=flex`, `data-template-selection-order=1`, `data-template-primary-selected=true`, `relationOutline=bottom right`, `relationSelection=linked-value`.
- 2026-05-11: chrome-devtools MCP 콘솔 확인 결과 error/warn 메시지는 없었다.
- 2026-05-11: `supabase` MCP는 현재 세션에 노출되지 않았다. 이번 변경은 DB 스키마/데이터 변경이 없으므로 사용자 실행 SQL도 없다.

속성 탭 선택 관계 범위 재수정:

- 2026-05-11: `METADATA-SELECTION-SCOPED-01` 백업 완료.
  - `docs/diff/2026-05-11_METADATA-SELECTION-SCOPED-01_TemplateEditWorkspace.before.tsx`
  - `docs/diff/2026-05-11_METADATA-SELECTION-SCOPED-01_0511ui.before.md`
- 2026-05-11: 사용자 재확인 기준에 따라 속성 탭 미선택 상태는 전체 관계를 표시하고, 선택 상태는 선택 항목이 포함된 관계만 표시하도록 바꿨다.
- 2026-05-11: 선택 항목이 key이면 해당 key와 연결된 value 관계만 남긴다. 선택 항목이 value이면 해당 value와 부모 key 관계만 남기며, 같은 key의 형제 value 관계는 표시하지 않는다. 이 value 기준은 후속 요청에 따라 `METADATA-VALUE-FAMILY-02`로 대체했다.
- 2026-05-11: 선택 항목의 번호 배지와 primary 선택 윤곽선/그림자는 `METADATA-SELECTION-RELATION-03` 기준을 유지한다.
- 2026-05-11: 추가 검증으로 `npx esbuild src/app/templates/edit/page.tsx --bundle --platform=browser --format=esm --jsx=automatic --log-level=warning --outfile=/tmp/template-edit-page-check.js`, `npm run check:no-shadow-app`, `git diff --check -- src/components/template/TemplateEditWorkspace.tsx docs/0511ui.md`를 실행했고 모두 통과했다.
- 2026-05-11: chrome-devtools MCP에서 `속성` 탭 미선택 상태를 확인했다. `selectedCount=0`, `connectorLines=27`, `connectorDots=52`, `relationOutlines=52`, `relationSelections=53`, `metadataFilter=null`, `tonedownOverlayCount=0`.
- 2026-05-11: chrome-devtools MCP 접근성 스냅샷의 `B90467CA-12` 텍스트 상자를 화면 클릭 방식으로 선택해 확인했다. `div` selector를 직접 타겟하지 않았다.
  - 선택 상태: `selectedCount=1`, `connectorLines=1`, `connectorDots=2`, `relationOutlines=2`, `relationSelections=2`, `metadataFilter=true`, `activeCount=2`, `inactiveCount=52`, `tonedownOverlayCount=52`.
  - 남은 관계: `band-5-cell-1` linked-key, `band-5-cell-2` linked-value.
  - 선택 항목 `band-5-cell-2`: `outline=2px solid rgba(13, 148, 136, 0.98)`, `beforeContent="1"`, `beforeDisplay=flex`, `data-template-selection-order=1`, `data-template-primary-selected=true`.
- 2026-05-11: chrome-devtools MCP 콘솔 확인 결과 error/warn 메시지는 없었다.
- 2026-05-11: `supabase` MCP는 현재 세션에 노출되지 않았다. 이번 변경은 DB 스키마/데이터 변경이 없으므로 사용자 실행 SQL도 없다.

속성 탭 value 관계 범위 및 관계 레이어 추가 변경:

- 2026-05-11: `METADATA-VALUE-FAMILY-01` 백업 완료.
  - `docs/diff/2026-05-11_METADATA-VALUE-FAMILY-01_TemplateEditWorkspace.before.tsx`
  - `docs/diff/2026-05-11_METADATA-VALUE-FAMILY-01_0511ui.before.md`
- 2026-05-11: value 항목 선택 시 선택 value만 남기지 않고, 부모 key가 소유한 모든 value 관계를 active 관계 범위에 포함하도록 바꿨다.
- 2026-05-11: 관계 connector line/dot의 z-index를 `34`로 올렸다. tone-down overlay는 `30`, 관계/선택 상자는 `31`이므로 관계 표시가 overlay와 상자 뒤에 숨지 않고 위에 출력된다.
- 2026-05-11: 추가 검증으로 `npx esbuild src/app/templates/edit/page.tsx --bundle --platform=browser --format=esm --jsx=automatic --log-level=warning --outfile=/tmp/template-edit-page-check.js`, `npm run check:no-shadow-app`, `git diff --check -- src/components/template/TemplateEditWorkspace.tsx docs/0511ui.md`를 실행했고 모두 통과했다.
- 2026-05-11: chrome-devtools MCP 접근성 스냅샷의 `B90467CA-12` 텍스트 상자를 화면 클릭 방식으로 선택해 확인했다. `div` selector를 직접 타겟하지 않았다.
  - 선택 상태: `selectedCount=1`, `connectorLines=2`, `connectorDots=3`, `relationOutlines=3`, `relationSelections=3`, `activeCount=3`, `inactiveCount=51`, `tonedownOverlayCount=51`.
  - 남은 관계: `문서번호:2402-049192` linked-value, `band-5-cell-1` linked-key, `band-5-cell-2` linked-value.
  - 레이어: relation line/dot z-index `34`, tone-down overlay z-index `30`, 선택 항목 z-index `31`.
  - 선택 항목 `band-5-cell-2`: `outline=2px solid rgba(13, 148, 136, 0.98)`, `beforeContent="1"`.
- 2026-05-11: chrome-devtools MCP 콘솔 확인 결과 error/warn 메시지는 없었다.
- 2026-05-11: `supabase` MCP는 현재 세션에 노출되지 않았다. 이번 변경은 DB 스키마/데이터 변경이 없으므로 사용자 실행 SQL도 없다.

속성 탭 직접 선택 외 연관 항목 윤곽선 제거:

- 2026-05-11: `METADATA-RELATED-NOOUTLINE-01` 백업 완료.
  - `docs/diff/2026-05-11_METADATA-RELATED-NOOUTLINE-01_TemplateEditWorkspace.before.tsx`
  - `docs/diff/2026-05-11_METADATA-RELATED-NOOUTLINE-01_0511ui.before.md`
- 2026-05-11: 속성 탭 metadata visual mode에서 `linked-key`/`linked-value` 관계 outline을 가진 항목이 직접 선택된 항목이 아니면 `outline`, `box-shadow`, `background-image`를 모두 제거하도록 했다. 관계 범위와 line/dot 표시는 `METADATA-VALUE-FAMILY-*` 기준을 유지한다.
- 2026-05-11: chrome-devtools MCP에서 `속성` 탭으로 전환한 뒤 접근성 스냅샷의 `B90467CA-12` 텍스트 상자를 화면 클릭 방식으로 선택해 확인했다. `div` selector를 직접 타겟하지 않았다.
  - 선택 상태: `selectedCount=1`, `relationSelections=3`, `relationOutlines=3`, connector line `2`, connector dot `3`.
  - 직접 선택 항목 `band-5-cell-2`: `boxShadow=rgb(3, 105, 161) 0px 0px 0px 2px inset`, `backgroundImage=linear-gradient(rgb(240, 249, 255), rgb(240, 249, 255))`, `beforeContent="1"`.
  - 직접 선택되지 않은 연관 항목 `문서번호:2402-049192`, `band-5-cell-1`: `boxShadow=none`, `backgroundImage=none`, `beforeContent=none`.
  - 관계 line/dot은 올바른 connector 속성 기준으로 line `2`, dot `3`, z-index `34`로 유지된다.
- 2026-05-11: 추가 검증으로 `npx esbuild src/app/templates/edit/page.tsx --bundle --platform=browser --format=esm --jsx=automatic --log-level=warning --outfile=/tmp/template-edit-page-check.js`, `npm run check:no-shadow-app`, `git diff --check -- src/components/template/TemplateEditWorkspace.tsx docs/0511ui.md`를 실행했고 모두 통과했다.
- 2026-05-11: chrome-devtools MCP 콘솔 확인 결과 error/warn 메시지는 없었다.
- 2026-05-11: `supabase` MCP는 현재 세션에 노출되지 않았다. 이번 변경은 DB 스키마/데이터 변경이 없으므로 사용자 실행 SQL도 없다.
