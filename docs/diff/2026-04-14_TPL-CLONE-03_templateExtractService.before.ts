...
const buildCandidatesFromPairs = (pairs: ExtractPair[]) => {
...
const extractPairsFromHtml = (
  sourceHtml: string
): { pairs: ExtractPair[]; candidates: ExtractCandidateSeed[]; generatedDraftHtml: string } => {
  const pairs: ExtractPair[] = [];
  const candidates: ExtractCandidateSeed[] = [];
  const candidateByLabel = new Map<string, ExtractCandidateSeed>();

  const registerPair = (pair: ExtractPair, options?: { allowKnownEmptyValue?: boolean }) => {
    const labelText = pair.labelText.trim();
    const valueText = pair.valueText.trim();
  ...
  const cloneAwareHtml = sourceHtml.replace(
    ...
    (
      ...
      const labelText = decodeHtmlEntities(rawLabel).trim();
      ...
      const candidate = registerPair(pair, { allowKnownEmptyValue: true });
  ...
  return { pairs, candidates, generatedDraftHtml };
};

const buildFallbackDraftHtml = (
  sourceTitle: string | null,
  pairs: ExtractPair[],
  candidates: ExtractCandidateSeed[]
) => {
  const rows = pairs
    .map((pair, index) => {
...
const analyzeSource = (
  ...
  let candidates: ExtractCandidateSeed[] = [];

  if (htmlMode) {
    const htmlAnalysis = extractPairsFromHtml(trimmedContent);
    pairs = htmlAnalysis.pairs;
    candidates = htmlAnalysis.candidates;
    generatedDraftHtml = htmlAnalysis.generatedDraftHtml;

    if (pairs.length === 0) {
      const fallbackAnalysis = buildCandidatesFromPairs(extractPairsFromText(stripHtml(trimmedContent)));
      pairs = fallbackAnalysis.pairs;
      candidates = fallbackAnalysis.candidates;
      generatedDraftHtml = buildFallbackDraftHtml(sourceTitle, pairs, candidates);
    }
  } else {
    const fallbackAnalysis = buildCandidatesFromPairs(extractPairsFromText(trimmedContent));
    pairs = fallbackAnalysis.pairs;
    candidates = fallbackAnalysis.candidates;
    generatedDraftHtml = buildFallbackDraftHtml(sourceTitle, pairs, candidates);
  }
