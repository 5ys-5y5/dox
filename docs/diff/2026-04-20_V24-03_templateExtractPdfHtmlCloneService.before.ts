...
const buildPositionedSvgTextElement = (page: TemplateExtractPdfPage, line: TemplateExtractPdfLine, lineIndex: number) => {
  ...
  const fontSize = Number(Math.max(7.8, Math.min(height * 0.82, 14.5)).toFixed(2));
  ...
  return `        <text class="${classes.join(' ')}" ${dataAttributes.join(
    ' '
  )} x="${left}" y="${baseline}" font-size="${fontSize}" data-template-width="${width}" xml:space="preserve">${escapeSvgText(
    rawText
  )}</text>`;
};
