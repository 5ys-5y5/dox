...
export const useDocumentsOwnerSettings = () => {
  const [state, setState] = React.useState(() => ({
    savedSettingsStore: readDefaultDocumentsOwnerSettingsStore().settingsStore,
    settingsStore: readDefaultDocumentsOwnerSettingsStore().settingsStore,
    hasStoredSettings: false,
    loaded: false,
  }));

  React.useEffect(() => {
    const nextStoredSettings = readDocumentsOwnerSettingsStoreFromStorage();

    setState({
      savedSettingsStore: nextStoredSettings.settingsStore,
      settingsStore: nextStoredSettings.settingsStore,
      hasStoredSettings: nextStoredSettings.hasStoredSettings,
      loaded: true,
    });
  ...
  const updateSurfaceSetting = React.useCallback(
    <K extends DocumentsOwnerSettingKey>(
      ...
      setState((current) => {
        return {
          ...current,
          settingsStore: updateDocumentsOwnerSettingsStoreOverride(current.settingsStore, { surface, key, value }),
          loaded: true,
        };
      });
    },
  ...
  const saveSettings = React.useCallback(() => {
    const nextSettingsStore = saveDocumentsOwnerSettingsStoreToStorage(state.settingsStore);

    setState((current) => ({
      ...current,
      savedSettingsStore: nextSettingsStore,
  ...
  const resetSettings = React.useCallback(() => {
    setState((current) => ({
      ...current,
      settingsStore: current.savedSettingsStore,
      loaded: true,
    }));
  }, []);
  ...
  const resolveSavedSurfaceSettings = React.useCallback(
    (surface: DocumentsOwnerSurface) => resolveDocumentsOwnerSettings(state.savedSettingsStore, surface),
    [state.savedSettingsStore]
  );
  const hasUnsavedSettings = React.useMemo(
    () => JSON.stringify(state.settingsStore) !== JSON.stringify(state.savedSettingsStore),
    [state.savedSettingsStore, state.settingsStore]
  );
