...
const toSettingsDto = (row: SmsSettingsRow | null): SmsSettingsDto => ({
  ...
  defaultRecipientIds: Array.isArray(row?.default_recipient_ids) ? row!.default_recipient_ids : [],
...
export const MessagingService = {
  ...
  async updateSmsSettings(input: SmsSettingsUpdateInput): Promise<SmsSettingsDto> {
    ...
    const normalizedRecipientIds = Array.isArray(input.defaultRecipientIds)
      ? Array.from(
          new Set(
            input.defaultRecipientIds
    ...
    if (Array.isArray(normalizedRecipientIds) && normalizedRecipientIds.length > 0) {
      ...
      const missingRecipientIds = normalizedRecipientIds.filter((value) => !foundRecipientIds.has(value));

      if (missingRecipientIds.length > 0) {
        throw new Error(`문자 발송 설정 저장 실패: 기본 수신번호를 찾을 수 없습니다. (${missingRecipientIds.join(', ')})`);
      }
    }
        ...
        .insert([
          {
            ...
            default_recipient_ids: normalizedRecipientIds ?? [],
    ...
    if (normalizedRecipientIds !== undefined) {
      patch.default_recipient_ids = normalizedRecipientIds;
    }
