...
export const SignService = {
  ...
  async executeSignature(params: {
    ...
    const { data: signRequestData, error: requestError } = await client
      .from('sign_requests')
    ...
    if (isExpired(signRequest.expiration_date)) {
      await client.from('sign_requests').update({ status: 'expired' }).eq('id', params.requestId);
    ...
    if (signRequest.status === 'authenticated') {
      ...
      const { data: verifiedAuthData, error: verifiedAuthError } = await client
    ...
    const { data: existingSignatureData, error: existingSignatureError } = await client
      .from('signatures')
      .select('id')
      .eq('request_id', params.requestId)
      .maybeSingle();
    ...
    const { data: signatureData, error: signError } = await client
      .from('signatures')
      .insert([
        {
