...
4. 따라서 후속 구현은 반드시 아래 순서를 따른다.
5. `올바른 측정`
6. `측정에서 벗어나는 지점 식별`
7. `그 지점만 수정`
8. `동일 측정으로 재검증`
9. 이 루프를 벗어난 수정은 폐기 대상이다.
...
1. `v21` 은 `hybrid` 를 되살리지 않고 `HTML-only candidate arbitration` 으로 보강한다.
2. 디지털 문서는 `legacy`, `generic`, `topology` 후보를 모두 점수화한 뒤 가장 높은 HTML만 채택한다.
3. 스캔 문서는 `generic layout clone`, `rule/certificate clone`, `work-order topology clone` 을 비교해 최선 후보를 선택한다.
4. 모든 후보는 선택 전에 실제 page width/height 를 주입해서 `pageContractScore` 손실을 줄인다.
5. 동점이면 `work_order_topology` 처럼 구조 보존력이 높은 builder 를 우선한다.
...
3. 디지털 문서에서 높은 heuristic score 가 나와도, 그것은 “측정이 충분하다”는 뜻이 아니라 “이제 렌더 비교로 검증할 차례”라는 뜻이다.
