...
func makeFont(size: CGFloat, weight: Double, familyHint: String?) -> NSFont {
  let systemWeight: NSFont.Weight = weight >= 600 ? .bold : .medium

  if let familyHint {
    for family in familyHint.split(separator: ",").map({ $0.replacingOccurrences(of: "\\\"", with: "").trimmingCharacters(in: .whitespacesAndNewlines) }) {
...
func drawString(
  ...
  let resolvedFont = makeFont(size: fontSize, weight: fontWeight, familyHint: fontFamily)
  let attributes: [NSAttributedString.Key: Any] = [
    .font: resolvedFont,
    .foregroundColor: textColor,
    .strokeColor: textColor,
    .strokeWidth: -0.35,
    .kern: CGFloat(letterSpacing ?? 0) * scale,
    .paragraphStyle: makeParagraphStyle(alignment: alignment),
  ]
  NSAttributedString(string: value, attributes: attributes).draw(in: rect)
}

func measureStringWidth(_ value: String, fontSize: CGFloat, fontWeight: Double, fontFamily: String? = nil) -> CGFloat {
  let attributes: [NSAttributedString.Key: Any] = [
    .font: makeFont(size: fontSize, weight: fontWeight, familyHint: fontFamily),
  ]
...
func drawStatusLine(_ item: TextItem, pageHeightPx: CGFloat) {
  ...
  let codeWidth = max(CGFloat(24) * scale, measureStringWidth(code, fontSize: fontSize, fontWeight: item.fontWeight, fontFamily: item.fontFamily) + 2)
  let timeWidth = measureStringWidth(timestamp, fontSize: fontSize, fontWeight: item.fontWeight, fontFamily: item.fontFamily) + 4
  ...
  drawString(code, in: codeRect, fontSize: fontSize, fontWeight: item.fontWeight, fontFamily: item.fontFamily, letterSpacing: item.letterSpacing, alignment: .left)
  drawString(actor, in: actorRect, fontSize: fontSize, fontWeight: item.fontWeight, fontFamily: item.fontFamily, letterSpacing: item.letterSpacing, alignment: .center)
  drawString(timestamp, in: timeRect, fontSize: fontSize, fontWeight: item.fontWeight, fontFamily: item.fontFamily, letterSpacing: item.letterSpacing, alignment: .right)
}
...
func drawStatusOptions(_ item: TextItem, pageHeightPx: CGFloat, in context: CGContext) {
  ...
  for option in options {
    let labelWidth = measureStringWidth(option.label, fontSize: fontSize, fontWeight: item.fontWeight, fontFamily: item.fontFamily)
    ...
    drawString(option.label, in: labelRect, fontSize: fontSize, fontWeight: item.fontWeight, fontFamily: item.fontFamily, letterSpacing: item.letterSpacing, alignment: .left)
...
func renderReplicaPage(_ page: PageModel) -> CGImage? {
  ...
  for item in page.textItems {
    ...
    default:
      ...
      drawString(item.text ?? "", in: rect, fontSize: CGFloat(item.fontSize) * scale, fontWeight: item.fontWeight, fontFamily: item.fontFamily, letterSpacing: item.letterSpacing, alignment: .left)
    }
  }
