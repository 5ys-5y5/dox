...
1. giant multi-row band 는 edge topology 계산 전에 row-range shell 로 분해되어야 한다.
2. 이후 selection / intent / resize 는 정규화된 shell 을 기준으로만 동작해야 한다.
...
1. `snapshot` 계산
2. giant multi-row band 가 남아 있으면 먼저 row-range shell 로 정규화
...
2. 정규화 후 `.v102-frame-band[data-v106-normalized-band="true"]` 가 생성되는지 확인한다.
3. root 하위 shell 수가 증가하는지 확인한다.
4. `band-3-cell-2` 와 `band-4-cell-2` 가 더 이상 같은 giant shell 을 공유하지 않는지 확인한다.
...
- `CHK-RESTART-SEL-EDGE-007`
  - 목표: giant multi-row band 를 edge topology 이전에 row-range shell 로 정규화한다.
  - 완료 기준: giant shell 이 normalized shell 묶음으로 대체되고, 그 상태에서 selection/topology 가 동작한다.
...
4. 새 코드 반영 후 root 하위 shell 실측:
   - `.v102-frame-band` 총 20개
   - `.v102-frame-band[data-v106-normalized-band="true"]` 17개
5. click 검증:
   - `band-3-cell-2` 선택 후 `left edge` 1회 클릭 시 `connected`, edge 수 `2`, anchor `band-3-cell-2:left`
   - 같은 edge 2회 클릭 시 `isolated`, edge 수 `1`, anchor `band-3-cell-2:left`
6. drag 자동화는 devtools 가 직접 drag primitive 를 제공하지 않아 synthetic pointer / React capture handler 조합으로 시도했다.
7. synthetic drag 는 selection click/DOM normalization 확인에는 충분했지만, 최종 human drag 와 1:1 로 동일하다고 단정할 수 없어 수동 최종 확인이 필요하다.
