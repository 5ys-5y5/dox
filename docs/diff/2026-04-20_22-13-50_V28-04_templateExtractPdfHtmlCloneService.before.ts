...
const buildPositionedLineMeasurementKey = (pageNumber: number, lineIndex: number) =>
  `page-${pageNumber}-line-${lineIndex + 1}`;

const resolvePositionedFontFamily = (fontName: string | undefined, text: string) => {
  const normalizedFontName = String(fontName || '').toLowerCase();

  if (/times/i.test(normalizedFontName)) {
    return '"Times New Roman", Times, serif';
  }

  if (/courier/i.test(normalizedFontName)) {
...
const buildPositionedLineDescriptor = (
  ...
  lineIndex: number,
  options?: Pick<PositionedCloneOptions, 'measuredLetterSpacingByLine'>
) => {
  ...
  const fontSize = Number(
    Math.max(8.25, Math.min(line.fontSize || normalizeReplicaFontSize(height), Math.max(height * 1.08, 22))).toFixed(2)
  );
  const fontWeight = knownLabel ? 600 : 500;
  const fontFamily = resolvePositionedFontFamily(line.fontName, normalizedText);
  const measurementKey = buildPositionedLineMeasurementKey(page.pageNumber, lineIndex);
  const letterSpacing = Number(
    (
      options?.measuredLetterSpacingByLine?.[measurementKey] ??
      (inlineCompositeHtml || statusOptionHtml ? 0 : 0)
    ).toFixed(4)
  );
  ...
  if (Math.abs(letterSpacing) > 0.0001) {
    styleParts.push(`letter-spacing:${letterSpacing}px`);
  }
  ...
  return {
    ...
    renderItem: buildReplicaRenderTextItem(
      normalizedText,
      left,
      top,
      width,
...
const buildMaskReplicaRuleSegments = (page: TemplateExtractPdfPage, maskPage: TemplateExtractPdfMaskVectorPage) => {
  ...
  return mergeReplicaRuleSegments(
    buildMaskRuleSegments(maskPage).map((segment) =>
      segment.orientation === 'h'
        ? {
...
const buildReplicaRuleSegments = (
  ...
  if (maskPage) {
    const maskSegments = buildMaskReplicaRuleSegments(page, maskPage);
...
const buildReplicaRulesHtml = (
  ...
  buildReplicaRuleSegments(page, geometryPage, rulePage, maskPage)
    .map((segment) =>
      segment.orientation === 'h'
...
const buildReplicaFrameRenderSegments = (
  ...
  buildReplicaRuleSegments(page, geometryPage, rulePage, maskPage).map((segment) =>
    segment.orientation === 'h'
      ? {
          orientation: 'h',
          left: segment.left,
...
const buildPositionedPageHtml = (
  ...
  const lineDescriptors = page.lines
    ...
    .map((line, index) => buildPositionedLineDescriptor(page, line, index, options))
  ...
  const rulesHtml = buildReplicaRulesHtml(page, geometryPage, rulePage, maskPage, precisionScale);
  const frameSegments = buildReplicaFrameRenderSegments(page, geometryPage, rulePage, maskPage);
