...
### 2026-04-17 구현 메모 `ENHANCE-09`

1. `v21` 에 `hybrid bitmap/text-layer` 후보를 실제 코드로 추가한다.
2. `hybrid` 후보는 PDF raster 와 HTML raster 일치율이 `0.95` 미만이면 폐기한다.
3. `작업지시서_사일동 주상복합.pdf` 와 `사업자등록증.pdf` 는 회귀 우선 샘플로 고정한다.
4. `hybrid` 통과 시에는 기존 topology/legacy arbitration 보다 우선한다.
5. non-hybrid 후보의 시각 유사도는 아직 완전한 pixel gate 가 아니므로, 이 구간은 후속 턴에서 계속 대체한다.
