...
- PDF 는 반드시 `원문 페이지 렌더링 + value overlay` 구조로 가야 한다.
...
### BUILDHTML-05
목표:
- 특정 문서형 재조립이 아니라, 일반 PDF에도 적용 가능한 `rendered page + overlay mask` clone 을 도입한다.

완료 기준:
- PDF 결과가 텍스트 재배치 HTML 이 아니라, 원문 페이지 이미지를 배경으로 유지한 clone 으로 나온다.
- value 영역만 absolute overlay placeholder 로 덮여서, 사용자가 봤을 때 원문과 거의 같은 페이지처럼 보인다.
- work-order 류가 아닌 다른 PDF도 최소한 “원문 페이지 + 값 마스크” 구조로 표시된다.

### BUILDHTML-06
목표:
- PDF overlay 의 value grouping 과 candidate 추출 정확도를 일반 PDF 기준으로 높인다.

완료 기준:
- 병렬 섹션, 같은 줄 다중 value, 빈 value, 서명칸이 line parser 오차 없이 overlay block 으로 남는다.
...
따라서 후속 구현은 후보 개수 조정이나 label 튜닝보다, 반드시 `BUILDHTML-05`의 rendered page clone 을 먼저 성립시켜야 한다.
