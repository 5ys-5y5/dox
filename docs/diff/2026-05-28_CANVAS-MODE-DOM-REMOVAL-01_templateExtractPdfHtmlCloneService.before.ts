import type {
  ...
  TemplateExtractPdfFrameDiagnostics,
  TemplateExtractPdfFramePolicy,
...
import { TemplateExtractPdfTableFragmentFrameService } from './templateExtractPdfTableFragmentFrameService';
...
const buildReplicaRenderTextItem = (
  ...
  const statusMatch = normalizedText.match(/^(CAE|CE|CAM|PM)\s+(.+?)\s+(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})$/);
  ...
  if (statusMatch) {
    return {
      ...
      code: statusMatch[1],
      actor: statusMatch[2],
      timestamp: statusMatch[3],
    };
  }

  if (normalizedText === '신규 재발급') {
    return {
      kind: 'status_options',
      left,
      top,
      ...
      fontWeight,
      options: [
        { checked: true, label: '신규' },
        { checked: false, label: '재발급' },
      ],
    };
  }

  if (normalizedText === 'Off-Line등록') {
    return {
      kind: 'status_options',
      left,
      top,
      ...
      lineHeight: height,
      fontWeight,
      options: [{ checked: false, label: 'Off-Line등록' }],
    };
  }
...
const buildPositionedLineDescriptor = (
  page: TemplateExtractPdfPage,
  line: TemplateExtractPdfLine,
  lineIndex: number,
  textInkScale = 1,
  usePdfFontSize = false
) => {
  ...
  const top = Number((toReplicaTop(page, line) + POSITIONED_TEXT_TOP_OFFSET).toFixed(2));
  const left = Math.max(0, Number(line.x.toFixed(2)));
  const pdfFontSize =
    typeof line.fontSize === 'number' && Number.isFinite(line.fontSize) && line.fontSize > 0 ? line.fontSize : null;
  const baseFontSize = usePdfFontSize && pdfFontSize ? Math.max(6, Math.min(pdfFontSize, 18)) : normalizeReplicaFontSize(height);
  const fontSize = Number((baseFontSize * textInkScale).toFixed(2));
  const fontWeight = knownLabel ? 600 : 500;
  ...
  return {
    html: `      <div class="${classes.join(' ')}" ${dataAttributes.join(
      ' '
    )} style="left:${left}px;top:${top}px;width:${width}px;height:${height}px;font-size:${fontSize}px;line-height:${height}px;font-weight:${fontWeight};">${
      statusOptionHtml || inlineCompositeHtml || escapeHtml(normalizedText)
    }</div>`,
    renderItem: buildReplicaRenderTextItem(normalizedText, left, top, width, height, fontSize, fontWeight),
  };
};

const buildSyntheticPositionedTextDescriptor = (
  text: string,
  left: number,
  top: number,
  width: number,
  ...
  html: `      <div class="template-clone__pdf-replica-text ${className}" data-template-source="synthetic" style="left:${left}px;top:${top}px;width:${width}px;height:${height}px;font-size:${fontSize}px;line-height:${height}px;font-weight:${fontWeight};">${escapeHtml(
    text
  )}</div>`,
  renderItem: buildReplicaRenderTextItem(text, left, top, width, height, fontSize, fontWeight),
});

const buildWorkOrderFooterLogoDescriptors = (page: TemplateExtractPdfPage) => {
  if (page.width < 500 || page.height < 700) {
    return [];
  }

  const logoLeft = Math.max(0, Number((page.width - 92).toFixed(2)));

  return [
    buildSyntheticPositionedTextDescriptor('posco', logoLeft + 14, 574, 54, 15, 15, 800, 'template-clone__pdf-replica-text--footer-logo'),
    buildSyntheticPositionedTextDescriptor(
      '포스코이앤씨',
      logoLeft,
      593,
      82,
      ...
      12.5,
      800,
      'template-clone__pdf-replica-text--footer-logo'
    ),
  ];
};
...
const mergeReplicaRuleSegments = (segments: ReplicaRuleSegment[]) => {
  const horizontal = segments
    .filter((segment): segment is Extract<ReplicaRuleSegment, { orientation: 'h' }> => segment.orientation === 'h')
    .sort((left, right) => left.top - right.top || left.left - right.left);
  const vertical = segments
    .filter((segment): segment is Extract<ReplicaRuleSegment, { orientation: 'v' }> => segment.orientation === 'v')
    .sort((left, right) => left.left - right.left || left.top - right.top);
  const merged: ReplicaRuleSegment[] = [];
...
const buildRuleGuidedReplicaRuleSegments = (page: TemplateExtractPdfPage, rulePage: TemplateExtractPdfRulePage) => {
  ...
  const outerWidth = Number((outerRight - outerLeft).toFixed(2));
  const outerHeight = Number((outerBottom - outerTop).toFixed(2));

  if (outerWidth <= 24 || outerHeight <= 24) {
    return mergeReplicaRuleSegments(segments);
  }

  const explicitSegmentCount = segments.length;

  if (explicitSegmentCount < Math.max(8, rowRules.length + columnRules.length - 4)) {
...
const buildMaskReplicaRuleSegments = (
  page: TemplateExtractPdfPage,
  maskPage: TemplateExtractPdfMaskVectorPage,
  minimumLength = MIN_MASK_RULE_LENGTH
) => {
  const scaleX = page.width / Math.max(1, maskPage.viewBoxWidth || maskPage.cssWidth || page.width);
  const scaleY = page.height / Math.max(1, maskPage.viewBoxHeight || maskPage.cssHeight || page.height);

  return buildMaskRuleSegments(maskPage, minimumLength).map((segment) =>
    segment.orientation === 'h'
      ? {
          orientation: 'h' as const,
          left: clampReplicaCoordinate(segment.x * scaleX, 0, page.width),
          top: clampReplicaCoordinate((segment.y + segment.height / 2) * scaleY, 0, page.height),
          width: Number(Math.max(1, segment.width * scaleX).toFixed(2)),
        }
      : {
          orientation: 'v' as const,
          left: clampReplicaCoordinate((segment.x + segment.width / 2) * scaleX, 0, page.width),
          top: clampReplicaCoordinate(segment.y * scaleY, 0, page.height),
          height: Number(Math.max(1, segment.height * scaleY).toFixed(2)),
        }
  );
};
...
const buildReplicaRuleSegments = (
  ...
  maskPage: TemplateExtractPdfMaskVectorPage | null,
  framePolicy: TemplateExtractPdfFramePolicy,
  maskRuleMinLength = MIN_MASK_RULE_LENGTH
) => {
  if (framePolicy === 'mask_vector_priority' && maskPage) {
    const maskSegments = buildMaskReplicaRuleSegments(page, maskPage, maskRuleMinLength);

    if (maskSegments.length > 0) {
      return {
        segments: maskSegments,
        source: 'mask_vector',
      } satisfies ReplicaRuleBuildResult;
    }
  }

  if (framePolicy === 'table_fragmented_rule_geometry' || framePolicy === 'table_fragmented_rule_geometry_mask_edges') {
    const fragmentedResult = TemplateExtractPdfTableFragmentFrameService.buildFrameSegments(page, geometryPage, rulePage, {
      includeMaskEdges: framePolicy === 'table_fragmented_rule_geometry_mask_edges',
      maskPage,
      maskRuleMinLength,
    });

    return {
      segments: fragmentedResult.segments,
      source: fragmentedResult.source,
      fragmentCount: fragmentedResult.fragmentCount,
      giantTableRejected: fragmentedResult.giantTableRejected,
    } satisfies ReplicaRuleBuildResult;
  }

  const ruleGuidedSegments = rulePage ? buildRuleGuidedReplicaRuleSegments(page, rulePage) : [];

  if (framePolicy === 'rule_geometry_only' && ruleGuidedSegments.length >= 8) {
    return {
      segments: ruleGuidedSegments,
      source: 'rule_only',
    } satisfies ReplicaRuleBuildResult;
  }
  ...
  if (!geometryPage) {
    return {
      segments: mergeReplicaRuleSegments(segments),
      source: ruleGuidedSegments.length > 0 ? 'rule_only' : 'none',
    } satisfies ReplicaRuleBuildResult;
  }
  ...
  return {
    segments: mergeReplicaRuleSegments(segments),
    source:
      ruleGuidedSegments.length > 0 && geometrySegmentCount > 0
        ? 'rule_geometry'
        : ruleGuidedSegments.length > 0
          ? 'rule_only'
...
const buildReplicaRulesHtml = (segments: ReplicaRuleSegment[], precisionScale = 1) =>
  segments
    .map((segment) =>
      segment.orientation === 'h'
        ? `      <div class="template-clone__pdf-replica-rule template-clone__pdf-replica-rule--h" style="left:${Number((segment.left * precisionScale).toFixed(2))}px;top:${Number((segment.top * precisionScale).toFixed(2))}px;width:${Number((segment.width * precisionScale).toFixed(2))}px;"></div>`
        : `      <div class="template-clone__pdf-replica-rule template-clone__pdf-replica-rule--v" style="left:${Number((segment.left * precisionScale).toFixed(2))}px;top:${Number((segment.top * precisionScale).toFixed(2))}px;height:${Number((segment.height * precisionScale).toFixed(2))}px;"></div>`
    )
    .join('\n');

const buildReplicaFrameRenderSegments = (segments: ReplicaRuleSegment[]): TemplateExtractReplicaRenderFrameSegment[] =>
...
const buildPageFrameDiagnostics = (
  framePolicy: TemplateExtractPdfFramePolicy,
  maskPageCount: number,
  maskRuleMinLength: number,
  frameBuildResult: ReplicaRuleBuildResult
): TemplateExtractPdfFrameDiagnostics => {
  const horizontalSegmentCount = frameBuildResult.segments.filter((segment) => segment.orientation === 'h').length;
  const verticalSegmentCount = frameBuildResult.segments.length - horizontalSegmentCount;

  return {
    policy: framePolicy,
    source: frameBuildResult.source,
    segmentCount: frameBuildResult.segments.length,
    horizontalSegmentCount,
    verticalSegmentCount,
    maskVectorEnabled: framePolicy === 'mask_vector_priority' || framePolicy === 'table_fragmented_rule_geometry_mask_edges',
    maskVectorPageCount: maskPageCount,
    maskRuleMinLength:
      framePolicy === 'mask_vector_priority' || framePolicy === 'table_fragmented_rule_geometry_mask_edges'
        ? maskRuleMinLength
        : null,
    fragmentCount: frameBuildResult.fragmentCount || null,
    giantTableRejected: frameBuildResult.giantTableRejected ?? null,
  };
};

const mergeFrameSources = (sources: TemplateExtractPdfFrameSource[]): TemplateExtractPdfFrameSource => {
  const filtered = sources.filter((source) => source !== 'none');

  if (filtered.length === 0) {
    return 'none';
  }

  return filtered.every((source) => source === filtered[0]) ? filtered[0] : 'mixed';
};

const summarizeFrameDiagnostics = (
  diagnostics: TemplateExtractPdfFrameDiagnostics[],
  framePolicy: TemplateExtractPdfFramePolicy,
  maskPageCount: number,
  maskRuleMinLength: number
): TemplateExtractPdfFrameDiagnostics => ({
  policy: framePolicy,
  source: mergeFrameSources(diagnostics.map((entry) => entry.source)),
  segmentCount: diagnostics.reduce((sum, entry) => sum + entry.segmentCount, 0),
  horizontalSegmentCount: diagnostics.reduce((sum, entry) => sum + entry.horizontalSegmentCount, 0),
  verticalSegmentCount: diagnostics.reduce((sum, entry) => sum + entry.verticalSegmentCount, 0),
  maskVectorEnabled: framePolicy === 'mask_vector_priority' || framePolicy === 'table_fragmented_rule_geometry_mask_edges',
  maskVectorPageCount: maskPageCount,
  maskRuleMinLength:
    framePolicy === 'mask_vector_priority' || framePolicy === 'table_fragmented_rule_geometry_mask_edges'
      ? maskRuleMinLength
      : null,
  fragmentCount: diagnostics.reduce((sum, entry) => sum + (entry.fragmentCount || 0), 0) || null,
  giantTableRejected: diagnostics.some((entry) => entry.giantTableRejected === true) ? true : null,
});
...
const buildPositionedPageHtml = (
  ...
  maskRuleMinLength = MIN_MASK_RULE_LENGTH,
  precisionScale = 1,
  includeWorkOrderFooterLogo = false,
  textInkScale = 1,
  usePdfFontSize = false
) => {
  const lineDescriptors = [
    ...
    .map((line, index) => buildPositionedLineDescriptor(page, line, index, textInkScale, usePdfFontSize))
      .filter((descriptor): descriptor is NonNullable<typeof descriptor> => Boolean(descriptor)),
    ...(includeWorkOrderFooterLogo ? buildWorkOrderFooterLogoDescriptors(page) : []),
  ];
  const lineHtml = lineDescriptors.map((descriptor) => descriptor.html).join('\n');
  const frameBuildResult = buildReplicaRuleSegments(
    page,
    geometryPage,
    rulePage,
    maskPage,
  ...
  const rulesHtml = buildReplicaRulesHtml(frameBuildResult.segments, precisionScale);
  const frameSegments = buildReplicaFrameRenderSegments(frameBuildResult.segments);
  const precisionWidth = Number((page.width * precisionScale).toFixed(2));
  const precisionHeight = Number((page.height * precisionScale).toFixed(2));
  const scaleBack = Number((1 / precisionScale).toFixed(5));

  return {
    html: `    <div class="template-clone__pdf-replica-page" data-page="${page.pageNumber}" data-page-number="${
      page.pageNumber
      ...
      <div class="template-clone__pdf-replica-frame-layer" data-template-layer="frame">
        <div class="template-clone__pdf-replica-precision-stage" style="width:${precisionWidth}px;height:${precisionHeight}px;transform:scale(${scaleBack})">
${rulesHtml}
        </div>
      </div>
...
${lineHtml}
    ...
    renderPage: {
      ...
      frameSegments,
      textItems: lineDescriptors.map((descriptor) => descriptor.renderItem),
    } satisfies TemplateExtractReplicaRenderPage,
    frameDiagnostics: buildPageFrameDiagnostics(framePolicy, maskPageCount, maskRuleMinLength, frameBuildResult),
  };
};

const serializeReplicaRenderModel = (renderModel: TemplateExtractReplicaRenderModel) =>
  JSON.stringify(renderModel).replace(/</g, '\\u003c');
...
const buildPositionedSvgPageHtml = (
  ...
  const textHtml = page.lines
    ...
    .filter(Boolean)
    .join('\n');
  const ruleHtml = buildReplicaRuleSegments(page, geometryPage, rulePage, maskPage, 'mask_vector_priority').segments
    .map((segment) =>
      segment.orientation === 'h'
...
const buildPositionedStyleHtml = () => `    <style>
      ...
      .template-clone--pdf-positioned-v30,
      .template-clone--pdf-positioned-v31 {
        width: fit-content;
        margin: 0 auto;
...
const POSITIONED_FRAME_PRECISION_SCALE = 4;
const POSITIONED_TEXT_TOP_OFFSET = 1;

type PositionedCloneOptions = {
  cloneId?: string;
  className?: string;
  precisionScale?: number;
  framePolicy?: TemplateExtractPdfFramePolicy;
  maskPages?: TemplateExtractPdfMaskVectorPage[];
  maskRuleMinLength?: number;
  includeWorkOrderFooterLogo?: boolean;
  textInkScale?: number;
  usePdfFontSize?: boolean;
};

type ReplicaRuleBuildResult = {
  segments: ReplicaRuleSegment[];
...
type PositionedCloneBuildResult = {
  html: string;
  frameDiagnostics: TemplateExtractPdfFrameDiagnostics;
};
...
const buildMaskRuleSegments = (
  page: TemplateExtractPdfMaskVectorPage,
  minimumLength = MIN_MASK_RULE_LENGTH
): MaskRuleSegment[] => {
  const horizontalSegments = page.rects
    .filter((rect) => rect.height <= MAX_MASK_RULE_THICKNESS && rect.width >= minimumLength)
    .sort((left, right) => left.y - right.y || left.x - right.x);
  const verticalSegments = page.rects
    .filter((rect) => rect.width <= MAX_MASK_RULE_THICKNESS && rect.height >= minimumLength)
...
const buildRuleTextSvgTextElement = (
  ...
  const baseline = textBox.baseline;
...
${pagesHtml}
  ...
  buildPositionedCloneHtml(
    ...
    options: PositionedCloneOptions = {}
  ): PositionedCloneBuildResult | null {
    ...
    const maskPagesByNumber = new Map(
      (options.maskPages || []).map(
        (page) => [page.pageNumber, page] satisfies [number, TemplateExtractPdfMaskVectorPage]
      )
    );
    const cloneId = options.cloneId || 'pdf-positioned-v22';
    const className = options.className || 'template-clone--pdf-positioned-v22';
    const precisionScale = options.precisionScale || POSITIONED_FRAME_PRECISION_SCALE;
    const framePolicy = options.framePolicy || 'mask_vector_priority';
    const maskRuleMinLength = options.maskRuleMinLength || MIN_MASK_RULE_LENGTH;
    const maskPageCount = maskPagesByNumber.size;
    const pageBuildResults = layout.pages
      .map((page) =>
        buildPositionedPageHtml(
          page,
          geometryPagesByNumber.get(page.pageNumber) || null,
          rulePagesByNumber.get(page.pageNumber) || null,
          maskPagesByNumber.get(page.pageNumber) || null,
          framePolicy,
          maskPageCount,
          maskRuleMinLength,
          precisionScale,
          options.includeWorkOrderFooterLogo === true,
          options.textInkScale || 1,
          options.usePdfFontSize === true
        )
      );
    const pagesHtml = pageBuildResults.map((result) => result.html).join('\n');
    const renderModel: TemplateExtractReplicaRenderModel = {
      version: 'positioned-v1',
      cloneId,
      pageCount: pageBuildResults.length,
      pages: pageBuildResults.map((result) => result.renderPage),
    };
...
${pagesHtml}
      ...
      frameDiagnostics: summarizeFrameDiagnostics(
        pageBuildResults.map((result) => result.frameDiagnostics),
        framePolicy,
        maskPageCount,
        maskRuleMinLength
      ),
