...
export default function RequestLinksPage() {
  ...
  const [useDocumentSenderOverride, setUseDocumentSenderOverride] = React.useState(false);
  ...
  React.useEffect(() => {
    setUseDocumentSenderOverride(Boolean(selectedDocumentSenderBinding));
  }, [selectedDocumentSenderBinding?.id]);
  ...
  const handleSendSms = async () => {
    ...
    const senderIdToUse = useDocumentSenderOverride ? selectedDocumentSenderBinding?.senderId || null : null;
    const hasGlobalDefaultSender = Boolean(smsSettings?.defaultSenderPhoneNumber);

    if (!senderIdToUse && !hasGlobalDefaultSender) {
      setMessage('전역 기본 발신번호를 먼저 준비하세요. 문서별 기본 발신번호를 쓰려면 아래 체크를 유지하세요.');
      return;
    }
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">REQ-LINK-09</Badge>
          ...
          <p className="max-w-3xl text-sm text-slate-600">
            ...
            만료 검증, 1회성 사용, 제출 감사 로그를 유지합니다. 문자 발송은 전역 기본 발신번호를 쓰되, 문서별 기본
            발신번호가 등록된 경우 여기서 해제할 수 있습니다.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border-slate-200">
          <CardHeader>
            ...
            <CardDescription>
              ...
              관리합니다. 문서별 기본 발신번호도 `/messaging`에서 지정합니다. 이 페이지는 요청 링크 생성과 실제 발송 실행에만
              집중합니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-slate-700">
        ...
        <Card className="border-slate-200">
          <CardHeader>
            ...
            <CardDescription>
              선택한 요청 링크를 문자로 전달합니다. 수신번호는 매번 직접 고르고, 문서별 기본 발신번호가 있으면 기본 적용되지만
              아래에서 해제할 수 있습니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-slate-700">
            <div className="space-y-2">
              ...
              {selectedDocumentSenderBinding ? (
                <label className="flex items-center gap-2 text-xs font-medium text-slate-700">
                  <input
                    type="checkbox"
                    checked={useDocumentSenderOverride}
                    onChange={(event) => setUseDocumentSenderOverride(event.target.checked)}
                  />
                  문서 기본 발신번호 사용
                  <span className="text-slate-500">
                    ({selectedDocumentSenderBinding.senderPhoneNumber}
