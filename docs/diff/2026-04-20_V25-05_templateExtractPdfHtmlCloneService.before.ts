...
const resolvePdfFontFamily = (
  ...
  if (embeddedFont) {
    return `"${escapeAttribute(embeddedFont.cssFontFamily)}"`;
  }
...
const buildRuleTextSvgTextElement = (
  ...
  if (textLayoutMode === 'clustered_line_box' && clusteredTextNodes.length > 0) {
    ...
    return `        <text class="${textClasses.join(' ')}" ${dataAttributes.join(
      ' '
    )} ${fontAttributes} x="${Number(firstCluster.x.toFixed(2))}" y="${baseline}" font-size="${fontSize}" textLength="${width}" lengthAdjust="spacingAndGlyphs" xml:space="preserve">${escapeSvgText(
      rawText
    )}</text>`;
  }
