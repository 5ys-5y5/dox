...
export default function DocumentsPage() {
  ...
  const [quickAllowedLabelsText, setQuickAllowedLabelsText] = React.useState('manager_name');
  ...
  React.useEffect(() => {
    ...
    setQuickAllowedLabelsText(latestRequestLink.allowedLabels.join(', '));
  ...
  const handleCreateAndSendEmailRequest = async () => {
    ...
    const allowedLabels = quickAllowedLabelsText
      .split(',')
      .map((item) => item.trim())
      .filter(Boolean);

    if (allowedLabels.length === 0) {
      setMessage('허용할 항목을 하나 이상 입력하세요.');
      return;
    }
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <Badge variant="slate">DOC-CLOUD-07</Badge>
          <h1 className="text-3xl font-semibold text-slate-950">서류 클라우드 관리</h1>
          <p className="max-w-3xl text-sm text-slate-600">
            문서 한 건을 고른 뒤 지금 해야 할 작업을 바로 끝내는 화면입니다. 개발자용 등록·편집 기능은 아래 `고급 편집`
            안으로 숨기고, 기본 화면은 선택, 상태 확인, 전송·출력 실행에만 집중합니다.
          </p>
        </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          받는 사람만 입력하면 요청 링크를 만들고 이메일까지 한 번에 처리합니다.
                        </p>
                      </div>
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-slate-800">허용 항목</label>
                          <Input
                            value={quickAllowedLabelsText}
                            onChange={(event) => setQuickAllowedLabelsText(event.target.value)}
                            placeholder="manager_name, work_date"
                          />
                          <p className="text-xs text-slate-500">쉼표로 구분합니다.</p>
                        </div>
                        <div className="space-y-2">
