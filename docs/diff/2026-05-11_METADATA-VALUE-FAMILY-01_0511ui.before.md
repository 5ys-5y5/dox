...
| `METADATA-SELECTION-SCOPED-03` | 선택 value는 해당 value와 부모 key만, 선택 key는 해당 key와 소유 value 관계만 남긴다. | `TemplateEditWorkspace.before.tsx` | 완료 |
...
- 2026-05-11: 선택 항목이 key이면 해당 key와 연결된 value 관계만 남긴다. 선택 항목이 value이면 해당 value와 부모 key 관계만 남기며, 같은 key의 형제 value 관계는 표시하지 않는다.
