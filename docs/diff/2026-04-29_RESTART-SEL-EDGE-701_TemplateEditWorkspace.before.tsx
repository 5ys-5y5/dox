...
const buildSplitFrameBandGroups = (positions: TableCellLayoutPosition[]): SplitFrameBandGroup[] => {
  const groups = new Map<string, SplitFrameBandGroup>();

  positions.forEach((position) => {
    if (!position.cell.matches(RAW_FRAME_NODE_SELECTOR)) {
      return;
    }

    const groupKey = `${position.rowStart}:${position.rowEnd}`;
    const currentGroup = groups.get(groupKey);

    if (currentGroup) {
      currentGroup.colStart = Math.min(currentGroup.colStart, position.colStart);
      currentGroup.colEnd = Math.max(currentGroup.colEnd, position.colEnd);
      currentGroup.entries.push(position);
      return;
    }
...
const buildNormalizedFrameBandShell = (
  ...
  nextShell.setAttribute('data-v106-band-range', group.groupKey);
