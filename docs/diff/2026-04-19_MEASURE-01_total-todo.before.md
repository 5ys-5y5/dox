...
- [진행중] `MEASURE-01`
  ...
  - 다음 액션:
    - 렌더 기반 비교 경로를 먼저 확정
    - 그 전까지 heuristic pass/fail 을 승격 근거로 사용하지 않기
...
- [진행중] `ENHANCE-11`
  - 범위: `v21` HTML-only 후보 점수화 및 page metric 주입
  - 기준:
    - `legacy/generic/topology/rule` 중 이미지 없는 HTML만 후보로 삼을 것
    - 후보별 offline quality 를 계산해 최고점 산출물만 채택할 것
    - 동점이면 구조 보존력이 높은 topology 계열을 우선할 것
  - 우선 회귀 샘플:
    - `/Users/gy/Documents/dev/docs/docs/작업지시서_사일동 주상복합.pdf`
    - `/Users/gy/Documents/dev/docs/docs/사업자등록증.pdf`
  - 관련 diff:
...
- 날짜: 2026-04-15
  - 체크리스트 ID: `TPL-EXTRACT-PROGRESS-01`
  - 목표: `/templates/extract` 에서 `초안 생성` 실행 중 사용자가 업로드와 서버 분석 진행 상태를 구분해서 볼 수 있게 한다.
  - 수정 내용:
    - `src/app/templates/extract/page.tsx` 에 초안 생성 진행 상태를 추가했다.
    - 파일 업로드는 `XMLHttpRequest` 로 보내고 `upload.onprogress` 값을 진행률에 반영한다.
    - 수동 입력 경로는 실제 업로드가 없으므로 `문서를 분석하고 있습니다.` 단계형 진행률로 표시한다.
    - 완료 시 100%, 실패 시 실패 상태와 오류 문구를 그대로 진행 카드에 남긴다.
    - 진행 카드는 `초안 생성` 버튼 아래에서 `파일 업로드 -> 문서 분석 -> 초안 조립` 단계로 보인다.
  - 테스트:
    - `npx esbuild src/app/templates/extract/page.tsx --bundle --platform=browser --format=esm --outfile=/tmp/templates-extract-page-progress.mjs` 성공
    - `npm run check:no-shadow-app` 성공
  - Supabase MCP:
    - 이번 턴도 프로젝트 URL 응답만 확인했고 DB 수정은 없었다.
  - Chrome DevTools MCP:
