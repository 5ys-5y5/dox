...
const buildRuleTextSvgTextElement = (
  ...
  if (clusteredNodes.length > 0) {
    return clusteredNodes
      .map((node, index) => {
        ...
        const fontSize = Number(Math.max(9, Math.min(height * 1.02, 36)).toFixed(2));
        const baseline = Number((node.top + height - Math.max(1.2, height * 0.12)).toFixed(2));

        return `        <text class="${classes.join(' ')}" ${dataAttributes.join(
          ' '
        )} data-template-char="${index + 1}" x="${Number(node.x.toFixed(2))}" y="${baseline}" font-size="${fontSize}" textLength="${width}" lengthAdjust="spacingAndGlyphs" xml:space="preserve">${escapeSvgText(
          node.text
        )}</text>`;
      })
      .join('\n');
  ...
  const fontSize = Number(Math.max(9, Math.min(height * 0.98, 36)).toFixed(2));
...
const buildRuleTextSvgPageHtml = (maskPage: TemplateExtractPdfMaskVectorPage, page: TemplateExtractPdfPage) => {
  ...
  const textHtml = page.lines
    ...
    .map((line, index) => buildRuleTextSvgTextElement(maskPage, page, line, index))
    .filter(Boolean)
    .join('\n');
  ...
  buildMaskVectorCloneHtml(
    ...
    const pagesHtml = pages
      .map((maskPage) => {
        ...
        return layoutPage ? buildRuleTextSvgPageHtml(maskPage, layoutPage) : buildMaskVectorPageHtml(maskPage);
      })
      .join('\n');
...
${semanticHtml.trim()}
    ...
    return `<section data-template-extract-draft="true" data-template-clone="pdf-rule-text-svg-v24">
  <div class="template-clone template-clone--pdf-positioned-svg-v24">
${buildPositionedSvgStyleHtml()}
${pagesHtml}
