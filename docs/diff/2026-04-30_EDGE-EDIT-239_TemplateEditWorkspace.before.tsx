...
export default function TemplateEditWorkspace({ initialTemplateId = '' }: TemplateEditWorkspaceProps) {
  ...
  const collectDirectIsolatedWidthResizeTargets = React.useCallback(
    (root: HTMLElement, snapshot: TemplateEdgeTopologySnapshotDto, edgeId: string) => {
  ...
  const handlePreviewPointerMove = React.useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!resizeState && edgePressState && event.pointerId === edgePressState.pointerId) {
      ...
      const usesDirectIsolatedWidthPath =
        (edgePressState.direction === 'w' || edgePressState.direction === 'e') &&
        edgePressState.dragSelection.tokens.length === 1 &&
        edgePressState.dragSelection.tokens[0]?.mode === 'isolated' &&
        edgePressState.mutationEdgeIds.length === 1 &&
        !Object.values(edgePressState.edgeRoleById).includes('peer_edge');
      const resizeTargets = usesDirectIsolatedWidthPath
        ? collectDirectIsolatedWidthResizeTargets(
  ...
  }, [applyRuntimeSelectionUi, collectDirectIsolatedWidthResizeTargets, collectEdgeResizeTargets, getFrameNodes]);
