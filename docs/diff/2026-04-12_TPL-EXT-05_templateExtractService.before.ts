...
export const TemplateExtractService = {
  ...
  // 이번 단계는 HTML 또는 text 본문을 직접 입력받아 규칙 기반으로
  // draft 와 필드 후보를 생성하는 1차 골격입니다.
  // PDF/DOCX/HWP 업로드 파서와 실제 LLM 문서 이해는 다음 단계에서 이어집니다.
  async createDraft(params: TemplateExtractCreateInput): Promise<TemplateExtractDetailResult> {
    ...
    if (!['html', 'text'].includes(params.sourceKind)) {
      throw new Error('템플릿 추출 실패: sourceKind는 html 또는 text 여야 합니다.');
    }

    const analysis = analyzeSource(params.sourceKind, sourceTitle, sourceContent);
      ...
      .insert([
        {
          ...
          similar_template_ids: params.similarTemplateIds || [],
        },
      ])
      .select('*')
      .single();
