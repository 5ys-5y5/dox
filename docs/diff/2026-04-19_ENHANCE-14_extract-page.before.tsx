              ...
              {draftDetail ? (
                <>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {qualityReport ? (
                      <Badge variant={qualityReport.passed ? 'green' : 'slate'}>
                        {qualityReport.mode}:{qualityReport.passed ? 'pass' : 'review'}
                      </Badge>
                    ) : null}
                  ...
                  {qualityReport ? (
                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                      <p className="font-medium text-slate-900">Offline Heuristic Only</p>
                      <p className="text-[11px] text-amber-700">
                        이 값은 PDF-HTML 렌더 일치 측정이 아니라 구조 휴리스틱입니다.
                      </p>
                      <p>overallScore: {formatScore(qualityReport.summary.overallScore)}</p>
                      <p>pageCount: {qualityReport.summary.pageCount}</p>
                      <p>hardFailures: {qualityReport.summary.hardFailureCount}</p>
                      <p>maxMismatchPixelRatio: {formatScore(qualityReport.summary.maxMismatchPixelRatio)}</p>
