...
- 2026-05-13: `BOX-STYLE-COMPACT-ROWS-01` 구현 전 백업을 `docs/diff/2026-05-13_BOX_STYLE_COMPACT_ROWS-01_TemplateEditWorkspace.before.tsx`, `docs/diff/2026-05-13_BOX_STYLE_COMPACT_ROWS-01_uiupdate0511.before.md`에 생성했다.
- 2026-05-13: `BOX-STYLE-COMPACT-ROWS-01` 구현. `상자 스타일` 상단 controls를 한 줄 dense grid에서 세로 stack으로 재배치했다. 1행은 `선 두께`와 `코너 라운딩`, 2행은 `선 색`, `선 종류`, `선 정렬`로 분리했고, preview 내부 controls는 `배경` 1행과 `높이`, `너비` 2행으로 나눴다. `선 두께`, `코너 라운딩` input은 `maxLength=2`, `높이`, `너비` input은 `maxLength=3`을 적용하고 width class도 각각 줄여 전체 오버레이 폭을 축소했다.
- 2026-05-13: `BOX-STYLE-OVERLAY-WIDTH-STRETCH-FIX-01` 구현 전 백업을 `docs/diff/2026-05-13_BOX_STYLE_OVERLAY_WIDTH_STRETCH_FIX-01_TemplateEditWorkspace.before.tsx`, `docs/diff/2026-05-13_BOX_STYLE_OVERLAY_WIDTH_STRETCH_FIX-01_uiupdate0511.before.md`에 생성했다.
- 2026-05-13: `BOX-STYLE-OVERLAY-WIDTH-STRETCH-FIX-01` 구현. `상자 스타일` 오버레이 body wrapper에 `items-start`를 추가하고, 상단 toolbar에 `self-start`를 추가해 flex column의 cross-axis stretch를 끊었다. 이로써 `상자 스타일`의 자연 폭이 preview/autosize 구간의 stale width에 끌려 넓게 측정되던 현상을 줄이고, top toolbar의 실제 내용 폭을 기준으로 preview와 autosize 폭이 다시 좁아지도록 정리했다.
- 2026-05-13: `BOX-STYLE-AUTOSIZE-AUX-SECOND-ROW-01` 구현. `자동 높이 상자`, `자동 너비 상자`의 보조 버튼 3개를 main button 오른쪽 absolute tray에서 제거하고, 각 option wrapper 내부의 2행 tray로 재배치했다. main button은 40px 단독 행으로 유지하고, 그 아래 `h-7` tray shell 안에 `위/아래/너비 맞추기`, `좌/우/높이 맞추기`를 렌더한다. inactive 상태에서는 tray shell만 `opacity-0 pointer-events-none`으로 숨겨 즉시 반응 경로를 유지한다.
- 2026-05-13: `BOX-STYLE-AUTOSIZE-SINGLE-BOX-01` 구현 전 백업을 `docs/diff/2026-05-13_BOX_STYLE_AUTOSIZE_SINGLE_BOX-01_TemplateEditWorkspace.before.tsx`, `docs/diff/2026-05-13_BOX_STYLE_AUTOSIZE_SINGLE_BOX-01_uiupdate0511.before.md`에 생성했다.
- 2026-05-13: `BOX-STYLE-AUTOSIZE-SINGLE-BOX-01` 구현. `자동 높이 상자`, `자동 너비 상자`는 하나의 외곽선 박스 안에 1행 메인 버튼과 2행 보조 버튼이 같이 들어가도록 재구성했다. 바깥 wrapper는 `overflow-hidden rounded-md border border-slate-300 bg-white`를 유지하고, 메인 버튼은 border 없는 40px 상단 행, 보조 버튼은 활성 상태에서만 같은 wrapper 내부의 `h-7 border-t border-slate-300` 2행으로 펼쳐진다. 따라서 보조 버튼이 더 이상 박스 밖 sibling으로 분리되지 않고 하나의 단일 박스 안에서만 나타난다.
- 2026-05-13: `BOX-STYLE-AUTOSIZE-SINGLE-BOX-01` 보정. 2행 보조 버튼은 가운데 좁은 막대로 남기지 않고, 하단 행 전체 폭을 3등분하는 `grid-cols-3` segmented row로 바꿨다. 각 보조 버튼은 `w-full flex-1 h-7`로 같은 폭을 차지하고, 경계선은 `border-l`만으로 연결된다. 따라서 단일 박스 안에서 상단 라벨과 하단 보조 버튼 행이 시각적으로 하나의 자연스러운 2단 control처럼 보인다.
