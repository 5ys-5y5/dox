...
func extractSelectionCharacters(_ document: PDFDocument, page: PDFPage, selection: PDFSelection) -> [CharacterModel] {
  ...
  for rangeIndex in 0..<rangeCount {
    let textRange = selection.range(at: rangeIndex, on: page)

    if textRange.location == NSNotFound || textRange.length <= 0 {
      continue
    }

    for characterIndex in textRange.location..<(textRange.location + textRange.length) {
      guard let characterSelection = document.selection(
        from: page,
      ...
      let characterText = characterSelection.string?
        .replacingOccurrences(of: "\\r", with: "")
        .replacingOccurrences(of: "\\n", with: " ") ?? ""

      if characterText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        continue
      }

      let characterBounds = characterSelection.bounds(for: page)

      if characterBounds.width <= 0 || characterBounds.height <= 0 {
        continue
      }
      ...
      if let attributed = characterSelection.attributedString, attributed.length > 0 {
      ...
      characters.append(
        CharacterModel(
          text: characterText,
          x: Double(characterBounds.origin.x),
          y: Double(characterBounds.origin.y),
          width: Double(characterBounds.size.width),
          height: Double(characterBounds.size.height),
          fontName: fontName,
          fontSize: fontSize
        )
      )
    ...
    for lineSelection in lineSelections {
      ...
      extractedLines.append(
        Line(
          ...
          fontSize: fontSize,
          source: "text_layer",
          characters: extractSelectionCharacters(document, page: page, selection: lineSelection)
        )
      )
    }
  }
...
const buildPipelineTrace = (
  engineVersion: Extract<TemplateExtractEngineVersion, '19' | '20' | '21' | '22' | '23' | '24' | '25'>,
  detection: TemplateExtractPdfFamilyDetectionResult,
...
const overrideResolvedSourcePipelineVersion = (
  ...
  engineVersion: Extract<TemplateExtractEngineVersion, '23' | '24' | '25'>
): TemplateExtractResolvedSource => {
...
export const TemplateExtractPdfService = {
  ...
  async extractPdfSource(
    fileName: string,
    bytes: Uint8Array,
    version: Extract<TemplateExtractEngineVersion, '19' | '20' | '21' | '22' | '23' | '24' | '25' | '26'> = '19'
