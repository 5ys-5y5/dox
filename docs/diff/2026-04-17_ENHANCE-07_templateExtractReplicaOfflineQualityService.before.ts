...
const scoreVectorTopology = (
  ...
  const horizontalScore = scoreRelativeCount(
    topologySummary.horizontalSegmentCount,
    normalized.frameHorizontalCount + normalized.rowCount
  );
  const verticalScore = scoreRelativeCount(
    topologySummary.verticalSegmentCount,
    normalized.frameVerticalCount + normalized.tableCount
  );
  const cellScore = scoreRelativeCount(topologySummary.cellCandidateCount, normalized.cellCount);
...
export const TemplateExtractReplicaOfflineQualityService = {
  evaluate(input: OfflineQualityInput): TemplateExtractReplicaQualityReport {
    ...
    const pageReports = buildPageReports(input.sourcePages, normalized);
    const hardFailures = [
    ...
    const hardFailureCount =
      hardFailures.length +
      pageReports.reduce((count, pageReport) => count + pageReport.hardFailures.length, 0);
    const dimensionScore = averageScore(
      pageReports.map((pageReport) => clampScore(1 - pageReport.mismatchPixelRatio))
    );
    ...
    if (input.fallbackApplied) {
      overallScore = Math.min(overallScore, 0.94);
    }

    const maxMismatchPixelRatio = pageReports.reduce(
      (maxValue, pageReport) => Math.max(maxValue, pageReport.mismatchPixelRatio),
      0
    );
    const passed =
      !input.forceFailure &&
      ...
      hardFailureCount === 0 &&
      pageContractScore === 1 &&
      textAnchorScore >= 0.995 &&
      vectorTopologyScore >= 0.995 &&
      overallScore >= 0.995;
