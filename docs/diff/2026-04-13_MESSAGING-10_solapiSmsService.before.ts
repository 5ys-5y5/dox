...
export const SolapiSmsService = {
  ...
  async lookupMessagesByProviderIds(providerMessageIds: string[]) {
    ...
    params.set('limit', String(Math.max(dedupedIds.length, 1)));
    params.set('messageId[in]', JSON.stringify(dedupedIds));

    const response = await fetch(`${SOLAPI_MESSAGE_LIST_URL}?${params.toString()}`, {
      method: 'GET',
      headers: {
        Authorization: buildAuthorizationHeader(config.apiKey, config.apiSecret),
        'Content-Type': 'application/json',
    ...
    const rawMessageList =
      (payload.messageList as Record<string, Record<string, unknown>> | undefined) ||
      (payload.data as Record<string, Record<string, unknown>> | undefined) ||
      {};

    const results = dedupedIds.flatMap((providerMessageId) => {
      const entry = rawMessageList[providerMessageId];
