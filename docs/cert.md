...
작성일: 2026-04-10 (KST)
최종 업데이트: 2026-04-11 (KST) - 본인확인 골격을 `src` 운영 경로로 유지한 상태에서 서명 도메인 DB를 `signing` 스키마로 분리하기 시작함
...
- 현재 서비스의 Supabase 연결은 `/Users/gy/Documents/dev/docs/.env` 의 `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` 를 통해서만 사용한다.
- 전자서명 도메인 테이블의 정본 소유 위치는 `signing.sign_requests`, `signing.sign_authentications`, `signing.signatures`, `signing.signature_audit_logs` 이다.
- Supabase Dashboard > Project Settings > API > Exposed schemas 에 `signing` 을 추가해야 `src/services/signService.ts`, `src/services/signAuthService.ts` 의 `schema('signing')` 호출이 정상 동작한다.
- 기존 public 테이블을 쓰던 운영 DB는 먼저 `docs/applied/run-this-supabase-signing-schema-migration.sql` 을 실행한 뒤 `docs/applied/setup-db.sql` 을 다시 실행해야 한다.
- 현재 이 작업에서 운영 DB에는 `docs/applied/run-this-supabase-integrity.sql` 실행이 완료되었다.
...
| 저장값 | `signatures.document_hash` 저장은 적합하나, DB 레벨 불변성 제약이 없음 | `docs/applied/run-this-supabase-integrity.sql` 실행 완료 |
...
- 무결성 DB 보강은 `docs/applied/run-this-supabase-integrity.sql` 실행으로 완료되었다. 신규 DB는 `docs/applied/setup-db.sql` 전체를 실행한다.
- `be5f695b-e7a0-42d6-8628-06c3ccbd28b1` 검토 결과처럼 `sign_requests.document_hash`가 비어 있으면 요청 시점부터 서명 시점까지 동일 문서였음을 증명할 수 없으므로, 이후에는 `docs/applied/run-this-supabase-integrity-tighten.sql`까지 적용해 `signed` 전환 전 요청 해시와 서명 해시 메타데이터 일치를 DB에서 강제한다.
- `bcc0304f-6d2a-4514-a74d-8edf568e7bb7` 검토 결과처럼 구 서비스 코드가 `signatures`를 먼저 INSERT하고 `signed` 상태 업데이트에서 실패하면 `pending` 요청에 서명 row가 남을 수 있다. 이후에는 `docs/applied/run-this-supabase-integrity-signature-guard.sql`까지 적용해 `signatures` INSERT 시점부터 요청 해시와 서명 해시 메타데이터 일치를 DB에서 강제한다.
- `9854df9b-0eaf-44b1-9b82-da2156caec8a` 검토 결과처럼 signature guard는 불완전한 서명 row 저장을 막지만, 구 서비스 코드가 해시 없는 `pending` 요청을 만드는 것까지는 별도 차단이 필요하다. 이후에는 `docs/applied/run-this-supabase-integrity-request-guard.sql`까지 적용해 활성 `sign_requests` row 생성 시점부터 문서 해시 메타데이터를 강제한다.
...
- 2026-04-11 현재 `docs/applied/setup-db.sql`과 `docs/applied/run-this-supabase-auth-bootstrap.sql`에 `sign_authentications` 테이블, 상태 전이 제약, authenticated 경로 DB guard를 반영했다.
- 2026-04-11 현재 기존 DB의 public 전자서명 테이블을 `signing.*` 로 이동하는 `docs/applied/run-this-supabase-signing-schema-migration.sql` 과 검증용 `docs/applied/verify-signing-schema-migration.sql` 을 추가했다.
...
1. `docs/applied/run-this-supabase-integrity.sql` 실행 완료: `sign_requests` 문서 해시 컬럼, `signatures` 해시 메타데이터 컬럼, SHA-256 형식 체크, 요청당 단일 서명 unique index, 서명/감사 로그 append-only 트리거가 DB에 반영되었다.
2. `docs/applied/setup-db.sql`에도 동일한 신규 DB용 스키마를 반영했다.
...
7. `be5f695b-e7a0-42d6-8628-06c3ccbd28b1` 서명 검토용 SQL은 `docs/applied/verify-sign-integrity-be5f695b.sql`에 작성했다. Codex MCP 연결 프로젝트(`grfkmbrhbvcyahflqttl`)에는 `sign_requests` 테이블이 없어 실제 서명 데이터는 해당 서명이 존재하는 Supabase SQL Editor에서 확인해야 한다.
...
10. `docs/applied/run-this-supabase-integrity-tighten.sql`을 추가했다. 이 SQL은 임시 `pending -> signed` 전이는 유지하되, 요청 해시가 없거나 서명 해시 메타데이터와 맞지 않으면 `signed` 상태 전환을 DB에서 차단한다.
...
12. 재발 방지를 위해 `docs/applied/run-this-supabase-integrity-signature-guard.sql`을 추가했다. 이 SQL은 `signatures` INSERT 자체를 요청 해시와 메타데이터가 완전히 일치할 때만 허용한다. BaroCert/PASS 연동 후에는 이 함수의 `TODO(auth-gate)` 조건도 `authenticated`만 허용하도록 좁힌다.
...
14. 재발 방지를 위해 `docs/applied/run-this-supabase-integrity-request-guard.sql`을 추가했다. 이 SQL은 `pending`, `authenticating`, `authenticated`, `signed` 상태의 `sign_requests` row가 문서 해시 메타데이터 없이 생성 또는 유지되는 것을 DB에서 차단한다.
...
17. 실행 규칙을 명확히 하기 위해 `docs/applied/run-this-supabase-integrity.sql`에는 "기본 1회 실행 + 레거시 signatures 백필"이라는 주석과 append-only 트리거 감지 시 백필 `UPDATE signatures ...`를 건너뛰는 로직을 추가했다. 반면 `docs/applied/run-this-supabase-integrity-signature-guard.sql`은 추가 가드 스크립트이므로 재실행 가능하다는 주석을 추가했다.
18. 2026-04-11 기준 본인확인 구현 시작 상태:
    - `docs/applied/setup-db.sql`은 신규 DB용 전체 스키마에 `sign_authentications`와 authenticated 경로 guard를 포함한다.
    - `docs/applied/run-this-supabase-auth-bootstrap.sql`은 기존 DB 증분 적용용이다.
...
1. `sign_authentications` 테이블과 상태 전이 제약 추가. 현재 `docs/applied/setup-db.sql`, `docs/applied/run-this-supabase-auth-bootstrap.sql`에 반영 완료.
