...
2. `isolated` 는 “선택 edge token 1개”가 아니라 “선택한 boundary segment 1개”로 해석되어야 하며, 실제 mutation target 에는 같은 선분의 반대편 edge 가 반드시 포함되어야 한다.
3. `status-history-1` 처럼 같은 y 대역에 있어도 실제 boundary 선분을 공유하지 않는 spanning shell 은 mutation target 에 포함되면 안 된다.
4. 이후 selection / intent / resize 는 정규화된 cell shell 과 physical boundary peer 계약을 기준으로만 동작해야 한다.
...
9. 실패 이유 9: 높이 독립성을 “row propagation” 문제로 오판해, 실제로는 selected edge 의 반대편 physical boundary peer 를 mutation target 에 넣어야 한다는 핵심을 놓쳤다.
   - 재발 방지 규칙: top/bottom drag target 은 `same line coordinate + overlapping span + opposite side` 를 만족하는 peer edge 로 확장한다.
...
11. 실패 이유 11: `Math.round()` 로 normalized shell `left/top/width/height` 를 쓰면서 원래 0px 로 맞닿아 있던 shell 사이에 1~2px gap 이 생겼다.
   - 재발 방지 규칙: normalized shell geometry 는 소수점 좌표를 유지하고, shell outer rect 에 불필요한 반올림을 금지한다.
...
8. `isolated` drag 라도 실제 mutation target 은 selected edge 와 같은 physical boundary 의 반대편 peer edge 까지 포함해야 한다.
9. normalized shell outer rect 는 소수점 좌표를 유지하고, gap 방지를 위해 shell geometry 반올림을 금지한다.
...
4. cohort proof 생성
5. opposite-side physical peer edge 조회
...
2. `TemplateEdgeResizeIntentService.resolveResizeIntent()` 는 selected edge ids 를 그대로 넘기지 않고, opposite-side physical peer edge 를 포함한 `targetEdgeIds` 를 반환한다.
3. `collectEdgeResizeTargets()` 는 이 `targetEdgeIds` 를 handle 단위로 dedupe 하되, 같은 boundary 의 반대편 peer handle 은 유지한다.
...
4. 정규화된 shell 은 `rowRange + colRange + source` 메타데이터를 가져야 하며, 실제 높이 mutation 은 row propagation 이 아니라 opposite-side physical peer 포함 규칙으로 계산한다.
5. normalized shell outer rect 는 `Math.round()` 없이 소수점 좌표를 유지해야 한다.
...
1. `targetEdgeIds` 를 `Edge Mutation Handle Service` 로 전달
   - 이 집합은 selected edge ids 와 opposite-side physical peer edge ids 를 모두 포함한다.
...
5. drag 시 selected edge 와 같은 physical boundary 의 반대편 peer edge 만 함께 변하고, unrelated shell 은 변하지 않는지 확인
...
5. `band-3-cell-2` 의 `bottom` 과 `band-4-cell-2` 의 `top` 이 gap `0px` 로 맞닿아 있고, `status-history-1` 은 같은 boundary peer 로 잡히지 않는지 확인한다.
...
6. normalized shell geometry 실측:
   - `band-3-cell-2` rect `left=197`, `right=381.016`, `bottom=500`
   - `band-4-cell-2` rect `left=197`, `right=381.016`, `top=500`
   - 두 shell 의 shared boundary gap 은 `0px`
   - `status-history-1` 의 bottom 은 `547` 이므로 `band-3-cell-2:bottom` physical peer 가 아니다
7. synthetic pointer drag 는 safety / pointer-capture 제약 때문에 실제 resize capture 재현이 불가능했다.
8. 따라서 이번 MCP 기록의 최종 증빙 범위는 `실제 click 전이 + normalized shell geometry + physical peer 식별` 까지다.
