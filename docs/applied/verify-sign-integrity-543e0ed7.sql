-- sign_requests.id = 543e0ed7-9205-4814-b026-2dc99e7e5bd4 무결성 점검 SQL
-- 이 파일은 해당 서명이 존재하는 Supabase 프로젝트의 SQL Editor에서 실행하세요.
-- 전제: docs/run-this-supabase-integrity.sql,
--      docs/run-this-supabase-integrity-tighten.sql,
--      docs/run-this-supabase-integrity-signature-guard.sql,
--      docs/run-this-supabase-integrity-request-guard.sql 실행 완료

WITH target AS (
    SELECT '543e0ed7-9205-4814-b026-2dc99e7e5bd4'::uuid AS request_id
),
sr AS (
    SELECT sr.*
...
checks AS (
    ...
    SELECT
        ...
        CASE
            WHEN EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'require_sign_request_document_hash' AND NOT tgisinternal)
             AND EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'enforce_sign_request_document_hash_required')
            THEN 'PASS'
            ELSE 'FAIL'
        END,
        COALESCE((
            SELECT jsonb_agg(tgname)
            FROM pg_trigger
            WHERE tgname = 'require_sign_request_document_hash'
              AND NOT tgisinternal
        )::text, 'missing require_sign_request_document_hash trigger')
