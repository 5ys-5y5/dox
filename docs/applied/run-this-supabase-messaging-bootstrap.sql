...
create table if not exists messaging.sms_service_settings (
  ...
  default_recipient_ids text[] not null default '{}'::text[],
  message_prefix text null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

alter table messaging.sms_service_settings
  add column if not exists default_recipient_ids text[] not null default '{}'::text[];

create table if not exists messaging.sms_document_sender_registry (
  id uuid primary key default gen_random_uuid(),
  document_id uuid not null unique,
  sender_id uuid not null references messaging.sms_sender_registry(id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create table if not exists messaging.sms_document_recipient_registry (
  id uuid primary key default gen_random_uuid(),
  document_id uuid not null,
  recipient_id uuid not null references messaging.sms_recipient_registry(id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (document_id, recipient_id)
);

create table if not exists messaging.email_dispatch_registry (
  id uuid primary key default gen_random_uuid(),
  request_link_id uuid null,
  provider_key text not null default 'resend'
    check (provider_key in ('resend')),
  provider_message_id text null,
  status text not null
    check (
      status in (
...
create table if not exists messaging.email_dispatch_targets (
  id uuid primary key default gen_random_uuid(),
  dispatch_id uuid not null references messaging.email_dispatch_registry(id) on delete cascade,
  request_link_id uuid null,
...
create index if not exists idx_email_dispatch_targets_dispatch
  on messaging.email_dispatch_targets (dispatch_id);

create table if not exists messaging.email_dispatch_events (
  id uuid primary key default gen_random_uuid(),
  dispatch_id uuid not null references messaging.email_dispatch_registry(id) on delete cascade,
  event_type text not null
    check (event_type in ('dispatch_requested', 'provider_response', 'dispatch_failed')),
...
drop trigger if exists touch_sms_document_sender_updated_at on messaging.sms_document_sender_registry;
create trigger touch_sms_document_sender_updated_at
before update on messaging.sms_document_sender_registry
for each row execute function messaging.touch_updated_at();

drop trigger if exists touch_sms_document_recipient_updated_at on messaging.sms_document_recipient_registry;
create trigger touch_sms_document_recipient_updated_at
before update on messaging.sms_document_recipient_registry
for each row execute function messaging.touch_updated_at();

drop trigger if exists touch_email_dispatch_updated_at on messaging.email_dispatch_registry;
create trigger touch_email_dispatch_updated_at
before update on messaging.email_dispatch_registry
for each row execute function messaging.touch_updated_at();

drop trigger if exists touch_email_dispatch_targets_updated_at on messaging.email_dispatch_targets;
create trigger touch_email_dispatch_targets_updated_at
before update on messaging.email_dispatch_targets
for each row execute function messaging.touch_updated_at();
