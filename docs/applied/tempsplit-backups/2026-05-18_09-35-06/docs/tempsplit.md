...
- 현재 크기: `38,758` lines
- 내부 대형 경계:
  - `TemplateEditPreviewSurface` 시작: 대략 `1193` line
  - `TemplateEditWorkspace` 시작: 대략 `17376` line
- 현재 특징:
  - 상태, ref, effect, DOM 직접 조작, API 호출, 전역 이벤트, 캔버스 편집 로직이 한 파일에 몰려 있다.
  - 단순 JSX 분량 문제가 아니라 `UI + 상태기계 + DOM 엔진 + 저장/불러오기`가 결합된 구조다.
...
- 현재 진행 메모:
  ...
  - `TemplateEditWorkspace.tsx`에서 타입 묶음과 일부 순수 유틸을 외부 파일로 이동 완료
  - 이번 실행 기준 `TemplateEditWorkspace.tsx` 라인 수는 `38,758 -> 37,954`로 감소
  - 남은 일: selector/attribute 상수의 추가 분리
  - 남은 일: draft/default 값 상수의 추가 분리
  - 남은 일: 템플릿 실제 선택/불러오기 상호작용 검증
- 상태: `진행중`
...
| --- | --- | --- |
| P0 | 완료 | 기준선 문서 작성 완료 |
| P1 | 진행중 | `types.ts`, `utils.ts`, `constants.ts` 1차 분리 완료 |
| P2 | 대기 | persistence 경계 분리 전 |
| P3 | 대기 | preview surface 파일 분리 전 |
| P4 | 대기 | selection/canvas engine 분리 전 |
| P5 | 대기 | position 모듈 분리 전 |
| P6 | 대기 | metadata 모듈 분리 전 |
| P7 | 대기 | shell 경량화 전 |
| P8 | 대기 | 안정화 전 |
...
다음 실행은 `P1. 타입/상수/순수 유틸 분리`부터 시작한다.
