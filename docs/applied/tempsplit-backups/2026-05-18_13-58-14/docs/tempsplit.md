...
- 현재 크기: `28,583` lines
- `workspace/**` 총 크기: `12,756` lines
- 현재 모듈 총량: `41,339` lines
- 내부 대형 경계:
...
- 현재 특징:
  ...
  - position action, text size, text style, selection appearance overlay JSX는 `workspace/panels/**`로 이동했다.
...
- 현재 진행 메모:
  ...
  - `src/components/template/workspace/panels/TemplatePositionSideOverlays.tsx`로 position action, text size, text style overlay 이동 완료
