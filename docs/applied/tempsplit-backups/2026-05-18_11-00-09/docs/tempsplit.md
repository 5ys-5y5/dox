...
- 현재 특징:
  ...
  - 여전히 pointer down/move/up 본체와 position/metadata 계산 로직의 핵심은 메인 파일 안에 남아 있다.
...
- 현재 진행 메모:
  ...
  - 위치 그룹 계산과 상대 위치 계산 핵심은 아직 메인 파일에 남아 있다
- 상태: `진행중`
...
- 현재 진행 메모:
  ...
  - metadata draft 상태와 validation/review 계산 핵심은 아직 메인 파일에 남아 있다
- 상태: `진행중`
...
- 현재 진행 메모:
  ...
  - 상단 header와 좌측 persistence panel은 아직 메인 파일에 남아 있다
- 상태: `진행중`
...
| P5 | 진행중 | spacing panel, selection summary overlay 분리 완료. 계산 핵심 잔여 |
| P6 | 진행중 | metadata canvas overlay 분리 완료. draft/validation 계산 잔여 |
| P7 | 진행중 | canvas toolbar shell 분리 완료. 좌측/상단 shell 잔여 |
| P8 | 대기 | 안정화 전 |
...
다음 실행은 `P4/P5/P6/P7 진행 범위`에서 `pointer down/move/up 본체`와 `좌측 persistence panel` 분리를 이어간다.
