...
- 현재 특징:
  ...
  - position/metadata 계산 핵심 일부는 `workspace/position/positionComputations.ts`, `workspace/metadata/metadataConnectionComputations.ts`로 이동했다.
  - 여전히 pointer down/move/up 본체와 metadata action/draft 갱신의 핵심은 메인 파일 안에 남아 있다.
...
- 현재 진행 메모:
  ...
  - main file import/dead code 정리와 shell wiring 정리를 진행했다
  - pointer down/move/up 본체는 아직 메인 파일에 남아 있다
- 상태: `진행중`
...
- 현재 진행 메모:
  ...
  - `positionSpacingSettingRelations`와 일부 그룹/상대 위치 계산은 아직 메인 파일에 남아 있다
- 상태: `진행중`
...
- 현재 진행 메모:
  ...
  - metadata draft 상태와 validation/review 계산, connection apply/save action 핵심은 아직 메인 파일에 남아 있다
- 상태: `진행중`
...
| P4 | 진행중 | selection action, keyboard/observer/lifecycle hook 분리 완료. pointer 본체 잔여 |
| P5 | 진행중 | panel 분리 + relative/spacing core 계산 일부를 `workspace/position/positionComputations.ts`로 이동 |
| P6 | 진행중 | overlay 분리 + connection 계산을 `workspace/metadata/metadataConnectionComputations.ts`로 이동 |
| P7 | 완료 | canvas toolbar, 상단 header, 좌측 persistence panel shell 분리 완료 |
| P8 | 진행중 | dead import 정리, 문서/manifest 갱신, 번들 검증 진행 중 |
...
다음 실행은 `P4/P5/P6/P8 진행 범위`에서 `pointer down/move/up 본체`, `positionSpacingSettingRelations`, `metadata action/draft 갱신 본체` 분리를 이어간다.
