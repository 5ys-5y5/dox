...
- 현재 특징:
  - preview shell, persistence 상태 전이, metadata marker UI는 메인 파일 밖으로 분리되었다.
  - 여전히 상태, ref, effect, DOM 직접 조작, 캔버스 이벤트 엔진은 큰 비중을 차지한다.
  - 다음 병목은 `UI + 상태기계 + DOM 엔진` 중 DOM 입력 엔진과 position/metadata panel 오케스트레이션이다.
...
다음 실행은 `P4. 선택/리사이즈/캔버스 이벤트 엔진 분리`를 시작한다.
