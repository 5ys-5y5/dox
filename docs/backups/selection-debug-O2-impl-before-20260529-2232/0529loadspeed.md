...
## 추가 설계 O2: `allowCanvasBoxSelection` 허용 후 상자 선택 버그 재현/원인 추적
...
```text
http://localhost:3000/canvas?page=project&documentId=2f6d0be2-8ba5-4d69-aacf-845275a66908
```

### 문제 정의

`allowCanvasBoxSelection`을 허용하면 `canvasSelectionMode="box"` 정책이 적용되어 상자 선택 자체는 열리지만, 실제 선택 동작에 다음 이상이 있다.

1. 같은 마우스 위치를 반복 클릭할 때 순환 선택이 작동하지 않는다.
   - 기대: 최상위 그룹 -> 하위 그룹 -> 마우스 위치에 있는 최하위 상자 -> 최상위 그룹 -> ...
   - 현재 의심: hit-test 후보 목록 또는 순환 index가 실제 클릭 좌표/현재 선택 상태와 동기화되지 않는다.
2. 선택시 범위 밖 상자가 선택되고, 범위 안 상자가 선택되지 않는다.
   - 기대: 클릭 좌표 또는 드래그 선택 사각형과 실제 화면 좌표가 일치한다.
   - 현재 의심: preview scale, scroll offset, transform, iframe/HTML zoom, document viewport offset 중 하나가 hit-test 좌표 변환에 빠져 있다.

중요 전제:

- `/canvas`가 모든 편집 기능 설정과 구현을 책임진다.
- `/project` 전용 선택 보정 코드를 만들지 않는다.
- 이번 단계는 원인 식별과 재현 체크리스트 수립이 먼저이며, 원인이 확인되기 전까지 선택 로직을 수정하지 않는다.
- 브라우저에서 직접 재현하고, 각 체크 항목마다 "시도 완료 여부 / 실제 현상 / 원인 후보 / 다음 조치"를 남긴다.

### 실행 전 안전장치

- [ ] 현재 파일 백업을 남긴다.
- [ ] `allowCanvasBoxSelection`은 저장값을 바꾸지 않고 브라우저 draft 상태 또는 임시 local state로만 켠다.
- [ ] 테스트 후 `되돌리기` 또는 storage 확인으로 저장되지 않았음을 확인한다.
- [ ] 콘솔 오류와 runtime overlay를 먼저 확인한다.
- [ ] 테스트 대상 문서가 로드 완료된 뒤에만 선택 동작을 측정한다.
- [ ] 선택 테스트 중 발생한 저장, 자동 재조회, 문서 reload 여부를 별도 기록한다.
...
기대 선택:
실제 선택:
선택된 frameGroupId/valueKey/label:
순환 후보 목록:
좌표 변환 값(client/page/canvas/local):
판정:
다음 조치:
```
...
- [ ] 빈 영역 클릭: 기존 선택이 유지되는지, 해제되는지 확인한다.
- [ ] 최상위 그룹 내부 클릭: 최상위 그룹이 먼저 선택되는지 확인한다.
- [ ] 하위 그룹 내부 클릭: 하위 그룹 또는 최하위 상자가 잘못 우선되는지 확인한다.
- [ ] 최하위 value 상자 중앙 클릭: 실제 마우스 위치 상자가 선택되는지 확인한다.
- [ ] 텍스트/서명/사진/파일 등 role이 다른 상자별로 1회 클릭을 확인한다.
- [ ] 겹치는 상자 영역 클릭: hit-test 후보 순서가 시각적 z-index와 맞는지 확인한다.
- [ ] 화면 상단, 중앙, 하단, 좌측, 우측에 있는 상자를 각각 클릭해 좌표 오차 방향을 확인한다.
...
- [ ] 같은 좌표를 3회 클릭한다.
- [ ] 같은 좌표를 4회 클릭한다.
- [ ] 기대 순환: 최상위 그룹 -> 하위 그룹 -> 최하위 상자 -> 최상위 그룹.
- [ ] 실제 순환 후보가 고정되는지, 건너뛰는지, 범위 밖 상자로 튀는지 기록한다.
- [ ] 선택 후 탭 전환 없이 반복 클릭한다.
- [ ] 선택 후 다른 탭으로 이동한 뒤 같은 좌표 반복 클릭한다.
- [ ] ESC로 선택 해제 후 같은 좌표 반복 클릭한다.

#### C. 드래그 선택

- [ ] 작은 드래그 범위로 상자 1개만 포함한다.
- [ ] 중간 드래그 범위로 같은 그룹의 하위 상자 여러 개를 포함한다.
- [ ] 큰 드래그 범위로 최상위 그룹 전체를 포함한다.
- [ ] 좌상단 -> 우하단 방향 드래그를 확인한다.
- [ ] 우하단 -> 좌상단 방향 드래그를 확인한다.
- [ ] 범위 경계에 걸친 상자가 포함/제외되는 기준을 확인한다.
- [ ] 범위 밖 상자가 선택되는 경우 선택된 상자의 실제 DOM rect와 드래그 rect를 비교한다.
- [ ] 드래그 중 scroll이 발생하거나 viewport가 움직이는지 확인한다.

#### D. 선택 해제와 재선택

- [ ] ESC 1회로 선택이 해제되는지 확인한다.
- [ ] ESC 후 단일 클릭 선택이 정상인지 확인한다.
- [ ] ESC 후 반복 클릭 순환 선택이 정상인지 확인한다.
- [ ] 빈 영역 클릭 후 재선택이 정상인지 확인한다.
- [ ] 다른 상자 선택 후 원래 상자로 돌아왔을 때 순환 index가 초기화되는지 확인한다.

#### E. 편집 패널과 선택 상태 동기화

- [ ] 미리보기 탭에서 선택한 상자가 크기 및 위치 탭에서도 유지되는지 확인한다.
- [ ] 크기 및 위치 탭에서 선택한 상자가 속성 탭에서도 유지되는지 확인한다.
- [ ] 속성 탭에서 선택한 상자가 역할 탭에서도 유지되는지 확인한다.
- [ ] 역할 탭에서 선택한 상자가 미리보기 탭에서도 유지되는지 확인한다.
- [ ] 탭 전환 시 선택 해제 이벤트가 발생하는지 확인한다.
- [ ] 탭 전환 후 첫 클릭이 순환 선택의 다음 단계로 처리되는지, 새 선택으로 처리되는지 확인한다.

#### F. 확대/축소/스크롤 조건

- [ ] 기본 zoom 100%에서 클릭 좌표와 hit-test 좌표를 기록한다.
- [ ] zoom을 낮춘 뒤 같은 시각적 위치를 클릭한다.
- [ ] zoom을 높인 뒤 같은 시각적 위치를 클릭한다.
- [ ] 페이지 scroll이 0인 상태와 하단으로 scroll한 상태를 비교한다.
- [ ] 캔버스 내부 scroll이 있다면 내부 scroll 전/후를 비교한다.
- [ ] 전체 화면 모드 전/후 선택 결과를 비교한다.

### 계측 계획

브라우저 콘솔 또는 임시 개발 로그로 다음 값을 한 선택 이벤트 안에서 동시에 기록한다.

- pointer event: `clientX`, `clientY`, `pageX`, `pageY`
- canvas container rect: `left`, `top`, `width`, `height`
- preview scale 또는 zoom 값
- scroll offset: window scroll, canvas scroll, internal viewport scroll
- 변환된 local point
- hit-test 후보 목록: frameGroupId, parentGroupId, role, label, z-index 또는 DOM order, rect
- 실제 선택 결과: selectedFrameGroupIds, selectedEdgeSelectionState, selected box metadata
- 순환 선택 상태: 직전 click point, 직전 후보 목록 key, 현재 cycle index

계측은 원인 확인용으로만 둔다. 최종 수정에서는 불필요한 로그를 제거한다.

### 원인 후보 분류

1. 좌표 변환 문제
   - 증상: 클릭 위치와 선택 상자가 일정 방향으로 어긋난다.
   - 확인: client 좌표에서 canvas rect, zoom, scroll을 모두 뺀 local 좌표가 실제 DOM rect와 맞는지 본다.
2. hit-test 후보 정렬 문제
   - 증상: 범위 안 후보는 맞지만 최상위/하위/최하위 우선순위가 기대와 다르다.
   - 확인: 후보 목록의 depth, z-index, DOM order, group parent chain을 비교한다.
3. 순환 상태 초기화 문제
   - 증상: 같은 좌표 반복 클릭인데 매번 첫 후보만 선택되거나 엉뚱한 후보로 이동한다.
   - 확인: 직전 좌표 허용 오차, 후보 목록 key, cycle index가 클릭 사이에 유지되는지 본다.
4. 탭 전환 side effect
   - 증상: 특정 탭에서만 선택이 해제되거나 순환 index가 초기화된다.
   - 확인: 탭 전환 렌더링이 selection state 또는 selected box proxy를 덮어쓰는지 본다.
5. owner selection policy 문제
   - 증상: `/canvas?page=project...`에서만 발생하고 순수 `/canvas` 문서 편집에서는 다르다.
   - 확인: `canvasSelectionMode`, `canvasTextInteractionMode`, selected boxes prop, `onCanvasSelectionChange` 흐름을 비교한다.

### 해결 순서

1. 재현 체크리스트를 먼저 실행하고 결과를 이 문서에 기록한다.
2. 좌표 변환 로그로 "범위 밖 선택"이 좌표 문제인지 후보 정렬 문제인지 분리한다.
3. 순환 선택 로그로 후보 목록은 맞지만 index가 깨지는지, 후보 목록 자체가 잘못되는지 분리한다.
4. 탭별 선택 유지 테스트로 selection state가 어느 컴포넌트에서 초기화되는지 확인한다.
5. 원인이 확인된 최소 지점만 수정한다.
6. 수정 후 같은 체크리스트를 다시 실행한다.

### 완료 정의

- [ ] 모든 탭에서 단일 클릭 선택이 마우스 위치의 올바른 상자를 선택한다.
- [ ] 같은 좌표 반복 클릭 시 최상위 그룹 -> 하위 그룹 -> 최하위 상자 -> 최상위 그룹 순환이 재현된다.
- [ ] 드래그 선택이 범위 안 상자만 선택하고 범위 밖 상자를 선택하지 않는다.
- [ ] ESC 후 선택 해제와 재선택이 정상이다.
- [ ] 탭 전환 후에도 선택 상태와 순환 선택 정책이 의도대로 유지된다.
- [ ] zoom/scroll/fullscreen 조건에서 좌표 오차가 없다.
- [ ] `/project` 전용 우회 없이 `/canvas` 중앙 선택 로직에서 해결된다.
- [ ] 수정 후 불필요한 계측 로그가 남지 않는다.

사용자 확인 요청 문구:

```text
O2 상자 선택 좌표/순환 선택 수정이 끝났습니다. /canvas?page=project...에서 단일 선택, 반복 클릭 순환 선택, 드래그 선택, ESC 후 재선택, 탭 전환 후 선택 유지가 정상인지 확인해 주세요.
```
